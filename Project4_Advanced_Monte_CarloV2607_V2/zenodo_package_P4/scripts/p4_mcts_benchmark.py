...
def run_greedy(env, oracle, args):
    ...
    # Report the best reward seen on the trajectory, matching run_random (:147),
    # run_ga (:215) and run_mcts (whose MCTSAgent.search returns the best molecule seen
    # anywhere in the search). Returning oracle(state) here instead scored greedy on its
    # TERMINAL state alone, which is a different rule from every method it is tabulated
    # against: greedy walks downhill after its peak, so terminal scoring understated it
    # by 0.2395 (medium vocabulary) and 0.1857 (full vocabulary). See
    # scripts/p4_greedy_rescore_gate.py and the analysis-ledger greedy-rescore entry.
    return best_smiles, best_reward, time.perf_counter() - t0
...
def main() -> None:
    ...
    # The reward column is recomputed as oracle(best_smiles), which now agrees with the
    # benchmark CSV for every method: all four return the best reward seen on the
    # trajectory, and the oracle is deterministic. Before the greedy rescore the two
    # files disagreed for greedy alone -- the benchmark CSV held oracle(final state)
    # while this file held oracle(best intermediate), 0.4278 against 0.6672 on the
    # medium vocabulary. They were never equal: greedy walks downhill after its peak.
