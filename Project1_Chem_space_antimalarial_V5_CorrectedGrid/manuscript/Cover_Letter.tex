...
\noindent\textbf{V5 migration status --- internal draft, not submission-ready.} This cover letter is retained as inherited V4 submission material during the controlled migration. A 68-pair DiffDock raw run is now retained and independently audited, but its Vina score-only gate is blocked by a coordinate-frame/grid mismatch (10/68 rank-1 poses fully in-grid, 14/68 partial, 44/68 outside; 11/612 secondary confidence poses invalid). No V5 consensus, RRS, or PNS result has been accepted. Do not submit this letter until the coordinate gate, V5 manuscript compilation, provenance checks, and author approval are complete.
