...
0. **Consensus/RRS/PNS enforcement gate installed (08/08/2026):** `scripts/p1_v5_consensus_rrs_gate.py` is fail-closed — it refuses any consensus/RRS/PNS computation until `results/structural_pocket_independent_review.json` has `accepted_for_full_run=true` AND all three target decisions are `ACCEPTED`. Currently verified: FAIL-CLOSED (register PENDING_INDEPENDENT_REVIEW). Raw affinity consolidation (no scoring) available at `results/v5_three_target_vina_affinities.csv` (17×3, for reviewer use only).
