...
# Box: the search space is a cube centered on the chain A triad. The catalytic cavity
# is not cubic; extended polypharm ligands (up to 30 heavy atoms, up to ~20 A extent)
# legitimately dock with peripheral atoms up to ~1 A past a 25/28 A cube edge while
# their centroid remains in the cavity (verified on PP-02, PP-06, PP-13). A hard
# 100%-atom-in-cube gate is therefore not a biological containment criterion.
BOX = (28.0, 28.0, 28.0)
# Biological verification gate (composite, recorded per pair):
#   (1) rank-1 centroid inside the declared box (search-space control, guaranteed by Vina);
#   (2) at least one pose atom within 10 A of a chain A triad catalytic atom
#       (pocket-contact control: pose must reach the catalytic site);
#   (3) >= 90% of pose atoms inside the box + 0.5 A grid-discretization padding
#       (peripheral-atom tolerance for extended ligands in a non-cubic pocket).
VERIFY_PADDING = 0.5
TRIAD_CONTACT_A = 10.0
MIN_FRACTION = 0.90
EXHAUSTIVENESS = 16
NUM_MODES = 9
SEED = 0
...
TRIAD_ATOMS = np.asarray([
    # chain A catalytic triad: Ser252 OG, His223 NE2, Asp219 OD1 (2F6I frame)
    [-22.232, 15.984, -2.161],
    [-26.372, 18.249, -4.879],
    [-24.225, 17.608, -1.663],
], dtype=float)
...
def rank1_inside_grid(pdbqt_path: Path, center: tuple, box: tuple, padding: float = VERIFY_PADDING) -> tuple[bool, float, int, float, bool]:
    """Parse the first MODEL of the Vina output PDBQT and check the composite gate.

    Returns (valid, fraction, atom_count, triad_min_dist_A, centroid_in_box).
    """
    coords = []
    in_model = False
    model_count = 0
    ...
    triad_min = float(min(np.min(np.linalg.norm(arr - t, axis=1)) for t in TRIAD_ATOMS))
    return True, fraction, len(arr), triad_min, centroid_in_box
...
def main() -> int:
    ...
    for r in clpp_rows:
        ...
        valid, fraction, atoms, triad_min, centroid_in_box = rank1_inside_grid(out_pdbqt, CURRENT_CENTER, BOX)
        gate_pass = valid and centroid_in_box and triad_min <= TRIAD_CONTACT_A and fraction >= MIN_FRACTION
        if not gate_pass:
            (output / "RUN_FAILED_NO_ACCEPTED_RESULT").write_text(
                "Incomplete Vina docking; must not be used for scoring or manuscript claims.\n", encoding="utf-8")
            raise SystemExit(
                f"FAIL-CLOSED Vina rank-1 fails composite pocket gate for {r['complex_name']}: "
                f"valid={valid} centroid_in_box={centroid_in_box} triad_min={triad_min:.2f}A fraction={fraction:.3f}")
        records.append({
            ...
            "rank1_triad_min_A": triad_min, "rank1_centroid_in_box": centroid_in_box,
    ...
    provenance = {
        ...
        "gate": {"centroid_in_box": True, "triad_contact_A": TRIAD_CONTACT_A, "min_fraction": MIN_FRACTION,
                  "triad_atoms": TRIAD_ATOMS.tolist()},
