    ...
    for idx, orig_complex_graph in tqdm(enumerate(test_loader)):
        ...
        try:
            ...
            if args.fixed_pocket_center is not None:
                fixed_center = torch.tensor(args.fixed_pocket_center, dtype=orig_complex_graph['receptor'].pos.dtype, device=orig_complex_graph['receptor'].pos.device) - orig_complex_graph.original_center.squeeze(0)
            randomize_position(data_list, score_model_args.no_torsion, args.no_random or args.no_random_pocket,
                               score_model_args.tr_sigma_max if not args.fixed_pocket_center else args.pocket_tr_max,
                               args.pocket_knowledge and fixed_center is None, args.pocket_cutoff,
                               initial_noise_std_proportion=args.initial_noise_std_proportion,
                               choose_residue=args.choose_residue, fixed_center=fixed_center)
