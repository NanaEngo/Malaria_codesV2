...
**Citations enrichies (11 Aug 2026):** pre-existing `.bib` entries now cited in the main manuscript — `coconut_2021` (COCONUT, Introduction), `mmv2026dd4gh`/`deepmirror2026dd4gh`/`techinformed2026dd4gh` (dd4gh, Introduction), `charmmgui_2026` + `bioexcel_gromacs2026_webinar` (Methods). Recompiled RC=0, 0 undefined refs.
**Gaps identified (11 Aug 2026, vs `PAPERS_2_3_ROADMAP.md`):** two roadmap "supporting novelty" items are absent from all of P2 — (1) **DPFS (Dynamic Pharmacophore Fit Score)** ("captures transient pockets from MD"); could enrich the ongoing Set-C MD analysis (transient pockets across the 16 systems). (2) **Pathway enrichment (PlasmoDB)** ("connects targets to parasite biology", ~1 week); would complete the biological justification beyond docking. Status: identified, not implemented, optional for submission.
