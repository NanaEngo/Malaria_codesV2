...
**Generated:** July 29, 2026 — **Split from BMAD** August 1, 2026 — **Updated** August 8, 2026 (v12 public-activity) — **Refreshed** August 10, 2026 (cohérence manuscrit) benchmark; pre-activity Pareto provenance lock restored)
...
**Manuscript coherence refresh — 10 August 2026:** re-compilation of the P4 main (`P4_Pareto_MCTS_JoC_refined.tex`, 13 p.) verified clean (0 errors, 0 undefined refs); the v12-activity canonical numbers (Random 0.6724 > MCTS 0.6649 > GA 0.6453 > Greedy 0.4278) are identical between this report and the manuscript abstract/table/figure/SM/cover letter. No value changed.
...
| **v12-allfrag 20-seed benchmark (full 80-fragment vocabulary)** | `results/benchmark_v12_allfrag/` | Random (0.6701 ± 0.0108) > MCTS+ScafVAE (0.6488 ± 0.0152) > GA (0.6427 ± 0.0138) > Greedy (0.4593 ± 0.0000); Random > MCTS in 19/20 seeds, Δ = −0.0214 (wider gap than medium set) |
...
**Primary conclusion:** On the curated medium fragment set, the canonical v12-activity scalar benchmark gives Random (0.6724 ± 0.0056) > MCTS (0.6649 ± 0.0068) > GA (0.6453 ± 0.0124) > Greedy (0.4278 ± 0.0000); MCTS is below Random (paired t-test: t₁₉ = −4.97, p = 0.000085, 95% CI −0.0107 to −0.0043) and above GA (t₁₉ = 6.95, p < 0.0001). **Extending to the full 80-fragment vocabulary (v12-allfrag) does not narrow the deficit — it widens it:** Random (0.6701 ± 0.0108) > MCTS (0.6488 ± 0.0152) with Δ = −0.0214 and Random > MCTS in 19/20 seeds. This rules out vocabulary-induced reward flatness as the cause; the deficit is structural (tree convergence, see §1.12). Separately, the deposited pre-activity Pareto-MCTS front contains four non-dominated solutions with hypervolume 1.2366. The real value of MCTS lies in transparent multi-objective coverage, not in a higher scalar reward. The v11 re-benchmark (§1.8) remains a historical pre-activity sensitivity baseline.
...
### 1.12 v12-allfrag Benchmark — Root Cause of MCTS Deficit (August 10, 2026)

**Experiment:** Repeated the v12-activity 20-seed × 4-method benchmark with `FRAGMENT_SET=all` (80 fragments) instead of `medium` (37 fragments). Hypothesis: the medium vocabulary creates a flat reward landscape that prevents MCTS from demonstrating its tree-search advantage.

**Results (job 15133, outputs in `results/benchmark_v12_allfrag/`):**

| Method | Mean reward | Std | Min | Max |
|:------|:----------:|:---:|:---:|:---:|
| Random | 0.6701 | 0.0108 | 0.6505 | 0.6910 |
| MCTS+ScafVAE | 0.6488 | 0.0152 | 0.6190 | 0.6772 |
| GA | 0.6427 | 0.0138 | 0.6139 | 0.6663 |
| Greedy | 0.4593 | 0.0000 | 0.4593 | 0.4593 |

**Key finding:** The deficit **widens** with the larger vocabulary (Δ = −0.0214 vs −0.0075 on medium; Random > MCTS in 19/20 seeds). This rules out vocabulary-induced reward flatness as the cause.

**Structural root cause analysis:**

| Factor | MCTS | Random | Impact |
|--------|------|--------|--------|
| Oracle budget | ~10,000 calls | ~10,000 calls | Equal |
| Unique molecules evaluated | ~200–300 (tree convergence) | ~1,000 (fresh trajectory each iter) | **MCTS evaluates 3–5× fewer unique molecules** |
| `_visited_states` blocking | Prevents re-exploration | N/A | Reduces diversity |
| Progressive Widening | 5–20 actions/node (of 80) | All 80 equally likely | Narrows search |

**Code locations:**
- `_visited_states` blocking: `p4_mcts_agent.py:396–405` — once a state is visited, it cannot be expanded again by any branch. In molecular generation, the same intermediate can be reached by different fragment orders (A+B+C vs B+A+C); this policy blocks re-exploration.
- Progressive widening: `p4_mcts_agent.py:379–385` — `max(5, k·N^α)` with `k=1.0, α=0.5` means a node with 400 visits only tries 20 of 80 fragments.
- PUCT c_puct=5.0: `p4_mcts_benchmark.py:124` — aggressive exploration constant; tree converges to narrow paths after ~200 iterations.

**Conclusion:** MCTS concentrates its ~10,000 oracle evaluations on a narrow set of converged paths, while Random distributes them across 1,000 independent trajectories. In a relatively flat reward landscape (top methods span only 0.04 points), broad exploration by Random finds a slightly better molecule by chance, while MCTS's focused exploitation yields diminishing returns. The allfrag experiment confirms the issue is structural, not vocabulary-related.
...
## 6.14 Editorial prose and display reconciliation — 2026-08-10

The canonical JoC main manuscript, Supplementary Information and cover letter were revised after a final adversarial review. The title is now identical across the three documents. The scalar v12 benchmark remains explicitly separate from the historical pre-activity Pareto analysis. The canonical front is described as a post-hoc SYBA re-derivation with four points and hypervolume 1.2366; the secondary baseline front comparison (hypervolume 18.99 under a different common normalisation) is labelled as a re-scoring analysis and is not conflated with the canonical front. RRS and PNS are consistently described as computational proxies rather than biological validation. Greedy convergence is described as repeated path collapse within the evaluated fragment space, not proof of a globally unique attractor. No QMC/VMC/DMC claim is present in the canonical article files.

Final local checks: provenance checker PASS; canonical source row counts 80 benchmark records, 4 merged Pareto points and 5 front-summary rows; main/SM/cover compilation PASS with zero fatal errors and zero undefined references; figures are available at 300–400 dpi where rasterised. One minor overfull box remains in the cover-letter metadata and requires no scientific correction; human visual inspection remains a submission task.

## 6.15 Substantive scientific prose revision — 2026-08-10

The P4 manuscript was rewritten at the argument level rather than lexically. The central question is now whether a chemistry-informed Pareto archive characterises candidate-set geometry that is not captured by a single scalar reward within the constrained fragment space. The abstract now uses the journal-required `Contribution` heading and separates Background, Methods, Results and Conclusions. The title no longer implies causal preservation or algorithmic superiority. The introduction states two estimands—search efficiency and candidate-set geometry—and the discussion interprets the scalar cost and the multi-objective readout as complementary outcomes. Repeated audit-style caveats were consolidated into Methods and Limitations; the manuscript no longer foregrounds debugging history, administrative provenance or rejection-defensive language. No numerical result, protocol, or biological claim was expanded beyond the verified evidence.
