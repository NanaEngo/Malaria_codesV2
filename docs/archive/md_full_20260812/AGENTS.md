...
> **Current-state precedence — 12 August 2026:** For P2 Set-C operations, the live status is the isolated witness chain **15254 → 15259 → 15260** (`PP-01_PfDHFR_WT`). Earlier references to **15106/15111/15117** are retained as historical execution records and are **superseded**, not active jobs. The bounded 16-system pilot is prepared, but no full-panel production/QC/MD-RRS result has been promoted.

**Dernière mise à jour :** 11 août 2026 — **P2 manuscrit : 6 entrées .bib pré-existantes désormais citées dans le main** (`coconut_2021` + dd4gh ×3 en Introduction, `charmmgui_2026` + `bioexcel_gromacs2026_webinar` en Methods MD), recompilé RC=0, 0 undefined refs, 6/6 résolues dans le .bbl — trace ajoutée au P2 README + roadmaps (`P1_P5_RRS_POLYPHARMA_ROADMAP.md`, `docs/PAPERS_2_3_ROADMAP.md`) — 10 août 2026 : **P1 V6 SOUMISSION JCIM PRÊT — revue adverse croisée V4/V5/V6 terminée (`FINAL_CROSS_REVIEW_P1_V456.md`) : claims↔preuves conformes, prose publication, 0 erreur, package `submission_ACS_P1V6/` auto-contenu ; restent : ORCID ×5 + funding** — **Audit .bib étendu P2–P5 (10/08) : 270 entrées, 0 `journaltitle`, rendu journaux vérifié (.bbl + PDFs), aucune correction** — P1 V4 : remédiation 2F6I/PfClpP 484 COMPLÈTE (décompte final 484 = 458 PASS + 1 PENDING (171) + 15 exclusions boron + 5 gate-failed + 5 embed-failure ; array 15016 + aggregate 15044, 35/35 records, 0 missing ; `results/pfclpp_2f6i_484_final_accounting.{csv,json}` ; dossier v2.0 prêt-à-signer `PFCLPP_2F6I_484_INDEPENDENT_REVIEW_DOSSIER.md` ; registre `PENDING_INDEPENDENT_REVIEW`). **P2 Set-C : dépendance CGenFF non-installable — workaround OpenFF 2.2.0 (AM1-BCC) DOCUMENTÉ et approuvé PI** (16/16 systèmes préparés, `preparation_manifest.json` cohort `P2_SET_C_POLYPHARM_17`). **Cause racine équilibration identifiée (10/08, correction finale)** : un **désalignement d'ordre `[molecules]` vs coordonnées** dans `p2_setc_prepare_openff.py` (`add_ligand_to_topol` insérait MOL0 en premier alors que `merge_gro` mettait la protéine en premier) → grompp ré-attribuait silencieusement les atomes → genion mélangeait les coordonnées → **anneaux aromatiques effondrés (TYR34 CG–CE1 0.68 Å, TYR238 OH–CD2 1.0 Å) et chaînes latérales explosées (LYS104 HE1–HE2 65 Å)** dans `complex.gro` des 16 systèmes → eau en NaN → segfault non-déterministe du pair-search Verlet. **Fix : ordre `[molecules]` aligné sur les coordonnées (protéine d'abord, MOL0 ensuite) + régénération `ions.gro` depuis `solvated.gro` propre** — validé (K76T 0 anneau effondré vs 114, LYS104 1.63 Å vs 65 Å ; 16/16 systèmes vérifiés propres) ; **job 15106 en cours (16 systèmes, array `%4`, `-ntomp 8`), wave-0 (K76A/K76T/WT/C59R) en NVT, 0 failure** ; EM doux sans POSRES (`emstep=0.0001`) conservé. **Pipeline production prêt (10/08)** : `p2_setc_md_workflow.py` étendu au **mode array** (`--array-index` 1 système/tâche, `--ntomp`, manifests/status par tâche, flags mdrun CPU-only `-nb cpu -pme cpu -bonded cpu -update cpu` — l'A4000 crash GROMACS GPU) ; `p2_setc_production.sbatch` = array 0-15 `%4` 10 ns × 16 (~40 h) à lancer avec `sbatch --dependency=afterok:15106` ; **QC trajectoires** `p2_setc_trajectory_qc.py` (bound fraction < 5 Å, règle pré-déclarée `setc_p2_minheavy_5A_ge10percent_v1`, contrat CSV) ; **MD-RRS** `p2_setc_md_rrs.py` (100 × bound_frac_mut/WT, fail-closed sur lignes non-PASS) ; runbook `results/set_c_md/setc_md_rrs_execution_runbook.md` (stages 0-4). 09 août 2026 — **règle auteur-contrôlée : `development_execution_authorized=true`; aucun blocage éditorial ou de revue avant instruction explicite de réactivation ; science et QC continuent** — dossier de revue indépendante prêt-à-signer `Project1_Chem_space_antimalarial_V5_CorrectedGrid/results/INDEPENDENT_REVIEW_DOSSIER_v2.md` (§7 commandes openssl/gpg, procédure validée sur copie avec contrôle négatif tamper), auto-vérification machine `p1_v5_review_selfcheck.py` (6/6 : table 17×4, frames rmsd=0, provenance, identité, register PENDING), **gate-opener `p1_v5_apply_review_signature.py`** (vérifie la signature détachée contre la clé publique AVANT d'ouvrir le gate ; fail-closed testé) — **Règle actuelle (09/08) : la revue indépendante reste un statut de provenance et le gate de promotion reste dormant ; aucun blocage du développement pré-soumission avant instruction explicite de réactivation.** `development_execution_authorized=true`; Register `authorization_mode=INDEPENDENT_REVIEW_REQUIRED`, `internal_work_authorized=false` ; gate `p1_v5_consensus_rrs_gate.py` **strict fail-closed restauré** (fonction partagée `check_gate` : `accepted_for_full_run=true` + 4 cibles `ACCEPTED` ; réutilisée par `p1_v5_consensus_scoring.py`). Consensus calculé sous le bypass = **VOID** (CSV supprimé, provenance status VOID). Gate-opener `p1_v5_apply_review_signature.py` corrigé : pose `decision=ACCEPTED` sur chaque cible PASS. **Consensus/RRS/PNS et manuscrit peuvent progresser en mode exploratoire avant réactivation ; leur provenance non revue doit rester explicitement étiquetée.** **P1.4 package d'acceptation COMPLETÉ sur V4** : encadré « What is and is not validated » + table de reproductibilité SM S37 (12 sorties, SHA-256[12]) + plan prospectif ; compile main 41 p./SM 49 p., 0 erreur. **P4 : benchmark v12-activity TERMINÉ (job 12865)** — Random 0.6724 > MCTS 0.6649 > GA 0.6453 > Greedy 0.4278 (MCTS vs Random p=0.0001, d=−1.11) ; manuscrit P4 basculé sur v12-activity (abstract, table, figure, SM, cover letter, recompilé RC=0). **P3 : extval descripteurs job 12860 FAILED** (BrokenProcessPool numpy 2.x↔loky) → fixé (worker top-level + threading + cache disque) → relancé job 12891 ; **QKS extval pilote n=150 TERMINÉ (quantum 0.8385 ≈ RBF 0.8423, p=0.374 ns — verdict canonique répliqué sur dataset externe)**, run complet 12863 en cours (fold 3/5, SVM RBF lent). Reporting P4 séparé de BMAD : `P4_DATA_ANALYSIS_REPORT.md` est la boussole P4 (benchmark, Pareto, ablations, QMC) ; BMAD couvre P1–P3 uniquement. P3 : **benchmarks canoniques COMPLETS** (jobs 12698/12699/12700/12702) — classique n=19,836 (ECFP4 0.9475 ± 0.0045), **hybride canonique Hybrid RF AUC 0.8876 ± 0.0065** (p<0.0001 vs ECFP4), **ablation : QK = principal contributeur (Δ=−0.040)**, **QKS 6q C3-fix : quantum ≈ RBF à toutes les échelles** (n=5,000 p=0.419 ; n=19,849 p=0.060). **Manuscrit trimé 26→18 p.** (main 18 p. / 7 230 mots, SM 18 p., cover letter 1 p. ; fusion tables benchmark/hybrid, `tab:qkernel`→SM S13, titre harmonisé — BMAD §3.14, audit v3.1). **ChEMBL validation exécutée** : 10 leads queryés, analogues tous Inactive (Tanimoto 0.229–0.379) — résultat honnête négatif. ⚠️ **Fichiers canoniques P3 : `Paper3_Quantum_InspiredV2608.tex` (main) et `Paper3_Quantum_Inspired_SM_V2608.tex` (SM) — les anciennes versions (V2607, `_refined`) sont archivées dans `manuscript/LaTeX/archive/`; toute édition se fait sur les fichiers V2608 canoniques.
...
### Décision de workflow — développement pré-soumission (09 août 2026)

Tant que l’auteur n’a pas explicitement confirmé la soumission **et** demandé la réactivation des restrictions, aucun blocage éditorial, de revue indépendante, de signature ou de promotion n’arrête le travail scientifique P1–P5. Les calculs, reruns, figures, analyses RRS/PNS/ACSI et manuscrits peuvent progresser. Les registres restent honnêtement `PENDING_INDEPENDENT_REVIEW` / `accepted_for_full_run=false` et les sorties non revues gardent `NOT_SUBMISSION_READY`; ces labels ne sont pas des interdictions. Les contrôles d’identité, hashes, géométrie, cohorte, finitude, provenance et erreurs logicielles demeurent obligatoires. La soumission ne déclenche aucun changement automatique : seule l’instruction explicite de l’auteur active ultérieurement `POST_SUBMISSION_REVIEW`. Politique détaillée : `P1_INTERNAL_DEVELOPMENT_POLICY.md` et `P1_PRE_SUBMISSION_WORKFLOW_NOTICE.md`.
...
### P1 — Chemical Space & Docking 🔄 (author-controlled pre-submission development; scientific QC active; no editorial block)

> ⚠️ **Manuscrit P1 SOUMISSION = V6 (JCIM)** : `Project1_Chem_space_antimalarial_V6_CorrectedGrid/manuscript/P1_V6_Integrated_Polypharmacology_RRS.tex` (main 19 p.) + `_SM.tex` (5 p.) + cover letter ; **package ACS auto-contenu `submission_ACS_P1V6/`** (PDF + .bbl/.aux pour xr + .bib, suivis dans git). **V4/V5 = archives pré-soumission** (V4 : espace chimique + overlay intégré, remédiation 2F6I 484 ; V5 : docking ciblé 68 paires, 4 ancres biologiques). **Revue adverse croisée V4/V5/V6 TERMINÉE (10/08, `FINAL_CROSS_REVIEW_P1_V456.md`)** : claims↔preuves **toutes conformes** (recalculs CSV : RRS 68.18–111.65 mean 81.70, ACSI mean 0.543, PNS 1.04–6.00, classes A*:6/B:5/C:5/D:1), prose nettoyée (0 résidu registre interne), siunitx/cleveref OK, **0 erreur / 0 réf. indéfinie**, chiffres OMS corrigés (2023 ≈ 263 M cas / 597 k décès, WMR 2024). **V6 PRÊT pour Paragon Plus** — restent : ORCID ×5, funding, lecture auteur finale.
> ✅ **P1 V4 — Remédiation 2F6I/PfClpP TERMINÉE (09-10/08) :** la couche 4GM2/PfClpR (paralogue inactif) a été remplacée par 2F6I/PfClpP réel sur les 484 centroids. Uniform 449/35 (jobs 13451/13452) → rescue 3/32 → **rémédiation ciblée 35/35, 0 missing (array 15016 + aggregate 15044) : 10 PASS_RAW_VINA, 5 DOCKED_GATE_FAILED, 5 EMBED_FAILURE, 15 exclusions protocolaires boron**. Reclassements documentés : 373 rescue-pass → DOCKED_GATE_FAILED (triade 11.34 Å > 10 Å) ; 136/170 gate-fail → PASS. Décompte final **484 = 458 PASS + 1 PENDING (171) + 25 non-PASS** ; politique d'exclusion boron pré-spécifiée (§5 dossier) ; promoteur `accepted_for_promotion=false` jusqu'à revue indépendante. Artefacts : `results/pfclpp_2f6i_484_final_accounting.{csv,json}`, `results/PFCLPP_2F6I_484_INDEPENDENT_REVIEW_DOSSIER.md` (**v2.1, 10/08** : + §8 procédure de signature openssl/gpg + payload, + §9 validation contrôles positif/tamper/clé erronée tous PASS), registre `results/pfclpp_2f6i_484_independent_review.json`. **Gate-opener V4 `scripts/p1_v4_apply_review_signature.py` relié au final accounting** (484 rows, IDs 1–484, `final_counts` somme 484, hash canonical SMILES) — l'ancienne liaison `find_complete_audit` (aggregate uniforme 467 records) était **incompatible** et échouait FAIL-CLOSED ; corrigé et **validé sur copie 10/08** (positif RC=0 `INDEPENDENT_REVIEW_ACCEPTED` ; tamper `current_accounting_records`→483 rejeté ; mauvaise clé rejetée, registre intact).

> 🔶 **P1 V5 : données brutes 4 cibles complètes ; revue indépendante conservée comme provenance, sans blocage du développement pré-soumission** — 4 ancres vérifiées (PfDHFR/7F3Y MTX A702, PfCRT/6UKJ Y01 proxy, PfClpP/2F6I triade Ser252/His223/Asp219 — 4GM2=PfClpR corrigé, PfATP4/9N10 D451/DPPR), **table 17×4 COMPLETE** (`results/v5_four_target_vina_affinities.csv`, review table 4× COMPLETE_17, jobs 12854/12855/12859/12864), manuscrit V5 compile RC=0. **Règle actuelle : restrictions de soumission inactives jusqu'à instruction explicite de l'auteur ; le gate signé reste dormant pour la promotion soumission-facing** (register `authorization_mode=INDEPENDENT_REVIEW_REQUIRED`, `internal_work_authorized=false`). **Lorsque l'auteur réactivera explicitement les restrictions : 1) relecteur indépendant vérifie `results/INDEPENDENT_REVIEW_DOSSIER_v2.md` (checklist §5 + selfcheck `p1_v5_review_selfcheck.py` 6/6) ; 2) signe le register (openssl ed25519 ou gpg, commandes §7 du dossier) ; 3) `python scripts/p1_v5_apply_review_signature.py --reviewer "..." --date ... --sig <sig> --pubkey <pub>` → `accepted_for_full_run=true` + décisions `ACCEPTED` (corrigé) → gate `check_gate` PASS, consensus/RRS/PNS autorisés.** Pilote docking-RRS : jobs **12903** (PfDHFR 5×17) / **12904** (PfCRT 3×17 — TERMINÉ, `v5_mutant_vina_scores_PfCRT.csv` 51 records) + postprocess **12905** (merge + RRS par-cible, protocole P2 ; 6UKJ = 7G8 fond K76T → WT = révertant T76K). Bug Vina corrigé (chemins relatifs → absolus) ; dossier + gate-opener + postprocess committés/poussés.
...
| **Manuscrit V6 (SOUMISSION JCIM)** | ✅ **Prêt Paragon Plus (10/08)** | Étude intégrée RRS + polypharmacologie (Set C 17, classes A*:6/B:5/C:5/D:1, cross-metrics ns Bonferroni) ; **revue croisée `FINAL_CROSS_REVIEW_P1_V456.md`** : toutes valeurs conformes, prose publication, 0 erreur ; package `submission_ACS_P1V6/` auto-contenu ; restent : ORCID ×5, funding |
| **Manuscrit V4 (archive pré-soumission)** | ✅ **Remédiation 2F6I COMPLÈTE (09-10/08)** | **484 = 458 PASS + 1 PENDING (171) + 15 exclusions boron (AD4) + 5 DOCKED_GATE_FAILED + 5 EMBED_FAILURE** — récapitulatif `results/pfclpp_2f6i_484_final_accounting.csv/.json` + dossier v2.0 prêt-à-signer `PFCLPP_2F6I_484_INDEPENDENT_REVIEW_DOSSIER.md` ; registre PENDING (réactivation à la demande de l'auteur) |
...
> ⚠️ **Dépendance Set-C (10/08) : CGenFF INDISPONIBLE — workaround OpenFF documenté.** `cgenff`/`cgenff_charmm2gmx` est un binaire ParamChem **licencié**, non installable via pip/conda (vérifié 09/08). **16 systèmes Set-C préparés avec OpenFF 2.2.0 (AM1-BCC via AmberTools) + protéine CHARMM36m**, déviation de politique `DOCUMENTED_DEVIATION_OPENFF_2_2_AM1BCC` déclarée **et approuvée PI** dans chaque `forcefield_manifest.json` (`policy_deviation.declared=true`) et dans `results/md_systems/set_c/preparation_manifest.json` (cohort `P2_SET_C_POLYPHARM_17`, 16/16 préparés). Smoke validé de bout en bout sur PP-01_PfDHFR_WT (EM restreint + libre + NVT + NPT rc=0, EM final −5.21e6 kJ/mol, NPT 310.15 K/1 bar). **Cause racine équilibration identifiée (10/08, après 4 arrays 15054/15070/15081/15090 + diagnostic complet)** : **désalignement d'ordre `[molecules]` vs coordonnées** dans l'assemblage (`add_ligand_to_topol` : MOL0 en premier ; `merge_gro` : protéine en premier) → grompp ré-attribue silencieusement → genion corrompt les coordonnées (anneaux effondrés, chaînes explosées, eaux en NaN) → **segfault non-déterministe du pair-search Verlet**. Le smoke convergeait car ses coordonnées n'étaient pas corrompues. **Fix (10/08) : ordre `[molecules]` aligné (protéine d'abord, MOL0 ensuite) + régénération `ions.gro` depuis `solvated.gro` propre ; EM doux sans POSRES (`emstep=0.0001`, 3000 pas) conservé** — validé sur tous les cas (K76T −3.87e6, PP-02_WT −4.89e6, K76A −3.92e6, WT −5.14e6) ; **job 15106 en cours (16 systèmes, array `%4`, `-ntomp 8`), wave-0 à em 979-1585 pas = zone de crash dépassée, 0 failure json**. `p2_setc_md_workflow.py::validate_forcefield` n'accepte cette déviation que si `policy_deviation.approved=true` (fail-closed sinon). Doc : `results/set_c_md/set_c_preparation_remediation_20260809.md`.
> ⚠️ **Gaps P2 identifiés vs `PAPERS_2_3_ROADMAP.md` (11/08) :** deux « supporting novelty » listés dans la roadmap sont **absents de tout P2** — (1) **DPFS (Dynamic Pharmacophore Fit Score)** : « captures transient pockets from MD » ; à enrichir via l'analyse MD Set-C en cours (poches transitoires sur les 16 systèmes). (2) **Pathway enrichment (PlasmoDB)** : « connects targets to parasite biology », prévu ~1 semaine ; complèterait la justification biologique au-delà du docking. Statut : gap identifié, non implémenté, optionnel pour la soumission.
...
| **Manuscrit** | ✅ **Prêt soumission JCIM** | 17 candidats, 136 systèmes de docking, 40 ns MD ; RRS table 17 scaffolds ; compile propre (25 p., 0 erreur). **Citations enrichies (11/08)** : entrées .bib pré-existantes désormais citées dans le main — `coconut_2021` (COCONUT, Intr.), `mmv2026dd4gh`/`deepmirror2026dd4gh`/`techinformed2026dd4gh` (dd4gh, Intr.), `charmmgui_2026` + `bioexcel_gromacs2026_webinar` (Methods). Recompilé RC=0, 0 undefined refs, 6/6 résolues dans le .bbl |
| Set-C MD (pilot RRS/polypharm, 16 systèmes PP-01/PP-02 × PfDHFR/PfCRT WT+mutants) | 🔄 **16/16 préparés ; équilibration 15106 en cours (wave-0 en NVT, 0 failure) ; production array 10 ns × 16 prêt** | Workaround OpenFF 2.2.0 (AM1-BCC) à la place de CGenFF (binaire licencié indisponible — **déviation DOCUMENTÉE + approuvée PI** dans `forcefield_manifest.json::policy_deviation`). ⚠️ **Cause racine finale (10/08)** : **désalignement ordre `[molecules]` vs coordonnées** → genion corrompait les structures (anneaux effondrés, chaînes explosées) → segfault pair-search. **Fix : alignement d'ordre + régénération** → job 15106, wave-0 propre. **Pipeline MD-RRS complet** : workflow array + QC + MD-RRS + runbook (`setc_md_rrs_execution_runbook.md`) |

### P2 — Live Set-C execution checkpoint (12 August 2026)

The bounded **16-system Set-C pilot** (PP-01/PP-02 × PfDHFR/PfCRT mutation states) is prepared under the explicitly documented OpenFF 2.2.0 AM1-BCC + CHARMM36m/TIP3P policy deviation. The currently live chain is **not the full panel**: it is an isolated `PP-01_PfDHFR_WT` witness. Equilibration witness **15254** completed successfully with hashed `npt.gro`/`npt.cpt`; witness production **15259** is running (latest observed marker from `results/md_systems/set_c_publication_witness_v8_20260811/PP-01_PfDHFR_WT/runs/publication_witness_20260811/replicate_1/production.log`: step 378000, time 756 ps, of 5,000,000 steps / 10 ns); witness QC **15260** is pending with `afterok:15259`. The historical identifiers 15106/15111 are superseded. No full-panel production has passed QC and no Set-C MD-RRS result exists (`md_rrs_status=NOT_COMPUTED`); canonical RRS/ACSI/PNS remain docking-derived. The historical four-system parent-lead MD remains separate from this pilot and witness. This checkpoint records operational provenance only and does not promote a biological or manuscript claim.
...
### P5 — GNN/Transformer Drug Discovery ✅ (Benchmarks terminés — manuscript)
...
> 📄 **Plan stratégique P5 : `P5_STRATEGIC_PA90.md`** (v2, 08/08/2026 — phases 1-2 terminées, L5 Zenodo en attente).

| Composant | Statut | Résultat clé |
|-----------|:------:|:-------------|
| **Objectif** | ✅ **Benchmarks complets** | GNN (GIN) + fusion topologique (GIN-TFP/TNE) + transformers (ChemBERTa) — honest-negative verrouillé (L3) |
| **Data analysis report** | ✅ **Créé 01/08** | `P5_DATA_ANALYSIS_REPORT.md` — boussole P5 (panel canonique P3 n=19,836, splits random+scaffold, fusion TDA/TNE, SOTA 2025-26) |
| **Plan stratégique PA90** | ✅ **v2 (08/08)** | `P5_STRATEGIC_PA90.md` — phases 1-2 terminées, levier L5 (Zenodo) resté ouvert |
...
├── Project1_Chem_space_antimalarial_V6_CorrectedGrid/  ← P1 SOUMISSION JCIM (canonique)
│   ├── manuscript/                        ← P1_V6_Integrated_Polypharmacology_RRS.tex + _SM + cover letter
│   ├── submission_ACS_P1V6/               ← Package auto-contenu (PDF, .bbl, .aux, .bib)
│   ├── results/                           ← Tables 17×4, registres, dossiers revue
│   └── scripts/                           ← Scripts P1
├── Project1_Chem_space_antimalarial_V4_CorrectedGrid/  ← P1 V4 (archive pré-soumission)
├── Project1_Chem_space_antimalarial_V5_CorrectedGrid/  ← P1 V5 (archive méthodologie docking)
...
| `Project1_Chem_space_antimalarial_V6_CorrectedGrid/README.md` | Notes P1 (V6 = soumission JCIM) |
| `FINAL_CROSS_REVIEW_P1_V456.md` | **Revue adverse croisée V4/V5/V6 (10/08/2026)** — claims↔preuves vérifiés, prose publication, verdict : V6 prêt Paragon Plus ; **§7 audit .bib étendu P2–P5 : 270 entrées, 0 `journaltitle`, rendu journaux vérifié, aucune correction** |
...
| 12860 | `p3_extval` (descripteurs) | ❌ **FAILED 08/08** | BrokenProcessPool numpy 2.x↔loky (phase TNE); superseded by corrected rerun |
| 12891/13997 | `p3_extval` + complete-case sensitivity | ✅ **Terminé** | ITT n=22,447 with 351 TNE failures retained as explicit zero-vector penalties; complete-case n=22,096; ranking unchanged |
| 12863/13998 | `p3_qks_extval` + statistical audit | ✅ **Terminé** | QKS external AUC 0.8172 vs RBF 0.8466; corrected-resampled p=0.021, exploratory |
| 12864 | `p1_v5_vina_pfatp4` | ✅ **Terminé** | PfATP4/9N10 17 paires — 17/17 gate pass, aff −4.63 à −7.57 → table 17×4 COMPLETE |
| 12889 | `p5_cb_metrics` | ✅ **Arrêt volontaire documenté** | Scaffold `_metrics` complet et préservé (`0.7908 ± 0.0298`, 25/25 fold×seed); job `CANCELLED`, exit `0:15`, durée `05:59:35`; random `_metrics` volontairement abandonné; résultat canonique random job 12815 inchangé |
| 12907 | `p5_gnn_metrics` | ⚠️ **Artefacts présents / état SLURM non vérifiable** | Résultats/provenances P5 présents; aucun job actif; comptabilité terminale indisponible — ne pas appeler ce job `COMPLETED` sur la seule base des fichiers |
| 12908 | `p3_extval` | ⚠️ **Artefacts présents / état SLURM non vérifiable** | Validation externe P3 documentée par les artefacts 13997/13998; aucun job actif; comptabilité terminale indisponible |
| 12909 | `p4_pareto` | ⚠️ **Artefacts présents / état SLURM non vérifiable** | Sorties Pareto/benchmark présentes; aucun job actif; comptabilité terminale indisponible |
| 12865 | `p4_bench_v12` (array 0-19) | ✅ **Terminé** | Benchmark v12-activity 20 seeds : Random 0.6724 > MCTS 0.6649 > GA 0.6453 > Greedy 0.4278 |
| 12885/12889/12890 | P5 kNN/metrics | ⚠️ **Historique à réconcilier** | L’ancienne entrée groupée n’est plus un état live : 12889 est arrêté volontairement après le scaffold; consulter le snapshot P5 pour les états individuels |
| 15016 + 15044 | `p1_v4_remediate_2f6i` (array 35) + aggregate | ✅ **Terminé (09/08)** | Rémédiation 2F6I 35/35, 0 missing : 10 PASS_RAW_VINA, 5 DOCKED_GATE_FAILED, 5 EMBED_FAILURE, 15 exclusions boron → **décompte final 484 = 458 PASS + 1 PENDING (171) + 25 non-PASS** (`results/pfclpp_2f6i_484_final_accounting.{csv,json}`) |
| 15052→15053 | `p2_setc_prepare` (16 systèmes) | ✅ **Terminé (10/08)** | 16/16 systèmes Set-C préparés (OpenFF 2.2.0 AM1-BCC + CHARMM36m, déviation documentée approuvée) — `results/md_systems/set_c/preparation_manifest.json` |
| 15054/15070/15081/15090/15097 (arrays) | `p2_setc_equilibrate(_retry/_final)` | ❌ **Crashs successifs (10/08)** | Cause racine : **désalignement ordre `[molecules]` vs coordonnées** → genion corrompait les structures (anneaux effondrés TYR34/TYR238, LYS104 explosé, eaux NaN) → segfault pair-search. Arrays interrompus après régénération |
| 15106 (array 0-15 `%4`) | `p2_setc_equilibrate_final` | 🔄 **En cours (10/08)** | **Structures régénérées propres (0 anneau effondré, 16/16 vérifiés)** + ordre `[molecules]` corrigé + EM doux `emstep=0.0001` ; wave-0 (K76A/K76T/WT/C59R) en NVT, 0 failure json |
| À lancer (dep. 15106) | `p2_setc_production.sbatch` (array 0-15 `%4`) | ⏳ **Prêt** | **Production 10 ns × 16** via `p2_setc_md_workflow.py --array-index` (1 système/tâche, `-ntomp 8`, flags CPU-only) ; puis QC (`p2_setc_trajectory_qc.py`) → MD-RRS (`p2_setc_md_rrs.py`) |
