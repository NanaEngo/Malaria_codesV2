...
## 9 August 2026 — Concrete P1 V5/V6 scientific results generated

The pre-submission development run produced a complete, provenance-bound computational result package without changing any canonical submission-ready artifact. The locked P1 V5 panel contains **68/68 finite target-wise Vina records** (17 candidates × PfDHFR/PfCRT/PfClpP/PfATP4), with all pairs passing the declared geometric gate. The quarantined exploratory four-target ranking places PP-06 first (mean target-wise Vina score −7.122 kcal mol⁻¹), followed by PP-03 (−6.598) and PP-11 (−6.465); this arithmetic mean is retained only as an exploratory ranking because unlike receptor scores are not calibrated as a common affinity scale.

A separate V5 mutant docking-RRS pilot is complete at **136/136 finite scores** (17 candidates × PfDHFR WT + N51I/C59R/S108N/I164L and PfCRT WT + K76T/K76A). All 17 candidates met the WT binding threshold on both targets. The pilot yielded PfDHFR RRS 99.559 ± 0.841% (range 98.4–101.6%; A*:12, A:5) and PfCRT RRS 100.229 ± 1.032% (range 99.3–104.1%; A*:12, A:5). These are genuine computational outputs, but they are **exploratory V5 docking-RRS**, not MD-RRS and not independently reviewed.

**Concrete relative-polypharmacology result (9 August 2026).** Because raw Vina energies are not comparable across heterogeneous receptors, a target-normalised analysis ranked the 17 candidates within each target and computed within-cohort percentiles. **PP-06 was the only candidate in the top quartile on all four targets (4/4); PP-11 reached the top quartile on 3/4 targets.** The non-dominated Pareto front contained **PP-06, PP-03, PP-05, PP-13, and PP-10**. This is a relative multi-target breadth result within the locked 17-candidate cohort, not an absolute affinity or experimental polypharmacology claim. The machine-readable table, provenance, and 300-dpi figure are `Project1_Chem_space_antimalarial_V5_CorrectedGrid/results/exploratory/polypharmacology/v5_within_target_polypharmacology.csv`, `.json`, and `.png/.pdf`; the analysis status is `EXPLORATORY_RELATIVE_POLYPHARMACOLOGY_NOT_SUBMISSION_READY`.

The exact-SMILES V6 integration reproduced the 17-member P2 Set-C cohort and retained the canonical P2 RRS/ACSI/PNS source layer: RRS classes A*:6, B:5, C:5, D:1; RRS mean 81.698 ± 12.367 (range 68.175–111.653); ACSI mean 0.543 ± 0.153; PNS mean 3.571 ± 1.560; PNS–RRS Spearman ρ=−0.5588, p=0.019709; ACSI–RRS ρ=−0.1324, p=0.612593; high ACSI 2/17 (11.8%). Neither association survives Bonferroni α=0.017. The V5 pilot RRS distribution is not a replication of the P2 canonical RRS distribution: raw score files, receptor/preparation provenance, and mutation execution layers differ. This discrepancy is documented—not hidden—in `P1_V5_SCIENTIFIC_RESULTS_RECONCILIATION_20260809.md`; no V5 RRS class or correlation replaces the canonical P2 values.

**P3 external-validation integrity note (9 August 2026).** The descriptor external-validation script records **351 TNE embedding failures** on the 22,447-molecule public ChEMBL panel. The historical outputs were generated by an explicit zero-vector failure penalty (intention-to-evaluate analysis), but the script's original prose incorrectly described the finite-vector filter as a no-imputation complete-case intersection. A dedicated sensitivity script, `Project3_Quantum_Inspired_RepresentationsV2607/scripts/p3_external_validation_sensitivity.py`, separates the ITT result from a complete-case analysis on the 22,096 molecules without TNE failures; it writes only to `results/external_validation_sensitivity/` and does not overwrite the historical report. Job 13997 completed, and the complete-case values are now reported alongside the ITT values. The published external descriptor values remain explicitly **ITT/zero-failure-penalty results**, with complete-case values identified as sensitivity results.

**Independent external-enrichment result (9 August 2026).** A V5-local redérivation of the archived molecule-level DEKOIS PfDHFR panel (40 labelled actives, 1,200 property-matched decoys; ranking score `−Vina`) gave ROC-AUC **0.4964** (10,000-bootstrap 95% CI **0.4038–0.5893**), PR-AUC **0.0340**, and EF@1/5/10/20% **0.00/0.50/1.00/1.00**. Active and decoy mean scores were −6.989 and −7.018 kcal mol⁻¹, respectively. This is a concrete negative external-enrichment result: the archived Vina protocol did not discriminate the DEKOIS actives from decoys above chance. It constrains docking interpretation but does not show that the P1 candidates are inactive and does not generalise to the other three targets. The result is quarantined as `EXPLORATORY_EXTERNAL_ENRICHMENT_NOT_SUBMISSION_READY`; script, molecule-level scores, metrics, provenance, and figures are under `Project1_Chem_space_antimalarial_V5_CorrectedGrid/results/exploratory/external_validation/dekois_pf_dhfr/`. It reproduces the archived true-ROC value 0.49640625 and does not overwrite the historical V2 Vina-only summary (0.450104).

Artifacts: `Project1_Chem_space_antimalarial_V5_CorrectedGrid/results/exploratory/v5_consensus_four_target_exploratory.csv`, `results/exploratory/rrs_pilot/c_rrs_pilot_per_target_v5.csv`, and `Project1_Chem_space_antimalarial_V6_CorrectedGrid/results/exploratory/derived/v6_integrated_candidate_metrics.csv`. All carry `PRE_SUBMISSION_DEVELOPMENT_NOT_SUBMISSION_READY` or `EXPLORATORY_UNVERIFIED_VOID_FOR_SUBMISSION` provenance; scientific QC remains active and the review registers remain pending.

**V5–P2 raw-score reconciliation (9 August 2026).** A dedicated read-only reconciliation script (`Project1_Chem_space_antimalarial_V5_CorrectedGrid/scripts/p1_v5_reconcile_rrs_panels.py`) joined the exact 17 candidates and all **136/136** shared candidate–target–state rows after mapping P2 SMILES through the V6 manifest and normalising V5 `PfCRT_WT_K76`/`WT_K76` to `WT`. Overall concordance was weak despite a nominal rank association: Pearson *r* = **0.0227** (*p* = 0.793), Spearman ρ = **0.3114** (*p* = 0.000224), mean V5−P2 difference = **−1.082 kcal mol⁻¹**, MAE = **1.737 kcal mol⁻¹**, and RMSE = **3.035 kcal mol⁻¹**. The discrepancy was target-asymmetric: PfCRT showed concordance (Pearson *r* = 0.7597; Spearman ρ = 0.7629; mean difference +0.616 kcal mol⁻¹; MAE 0.792), whereas PfDHFR showed a systematic negative shift (Pearson *r* = −0.1341; Spearman ρ = 0.2679; mean difference −2.101 kcal mol⁻¹; MAE 2.304). The largest differences included P2 PfDHFR WT scores that were positive or near zero (PP-03: P2 +10.30 versus V5 −9.264; PP-01: +7.50 versus −7.883) and the previously identified PP-11 C59R case (P2 −0.02 versus V5 −7.931). Recomputed raw-panel RRS summaries were V5: 17/17 target-specific binders, candidate mean **99.885%** (SD 0.754); P2: 29 target-specific binder rows because five PfDHFR WT baselines were below the 5.0 kcal mol⁻¹ threshold, candidate mean **82.822%** (SD 10.757). These are not interchangeable evidence layers. The result is a **protocol/data-layer sensitivity finding**, not proof that either docking campaign is biologically correct; no V5 value, class, or correlation replaces canonical P2/V6 results. Full row-wise CSV and machine-readable provenance are quarantined at `Project1_Chem_space_antimalarial_V5_CorrectedGrid/results/exploratory/reconciliation/v5_p2_raw_score_reconciliation.csv` and `.json` (CSV SHA-256 `137b221a5eded818dcf228af3eed628506971ea5c35bc576d0aaf7b488daa266`; JSON SHA-256 `3bba94c5045a41a7057e0fc32d598be8ae329eda9a37f0fa57ad96aeb1739b14`). A new headline RRS claim requires one frozen rerun of the same 136 states with identical receptor/mutation/grid/ligand/Vina inputs, followed by one shared RRS implementation.

## 9 August 2026 — Pre-submission development policy updated

The P1 manuscripts remain under active scientific development and have not been submitted. The previous workflow incorrectly made the independent-review signature a computational execution blocker, which diverted effort from unresolved scientific questions. Effective immediately, the phase is `PRE_SUBMISSION_DEVELOPMENT` (root file `P1_DEVELOPMENT_PHASE.json`): V5/V6 consensus, docking-RRS pilot, and integrated exploratory analyses may run without a signed independent-review register. Outputs must remain explicitly labeled `PRE_SUBMISSION_DEVELOPMENT` / `NOT_SUBMISSION_READY`, and the register must remain truthful (`PENDING_INDEPENDENT_REVIEW`, `accepted_for_full_run=false`).

This is an execution-policy change, not an evidence upgrade. Structural identity, target-specific pocket definitions, input hashes, geometric gates, finite scores, candidate-set checks, and trajectory/QC requirements remain active. No result is silently treated as independently reviewed. During author-controlled pre-submission development, no editorial restriction blocks calculation or drafting. Submission does not switch the workflow automatically: only explicit author confirmation of submission plus an explicit reactivation request may change `P1_DEVELOPMENT_PHASE.json` and activate signed review for submission-facing claims, revisions, or reviewer responses.

## 9 August 2026 — P1 V5/V6 descriptive data and figure package generated

A provenance-bound descriptive package was generated from the locked P1 V5 four-target raw Vina table and the existing P2 source-panel RRS/ACSI/PNS tables. The V5 generator is `Project1_Chem_space_antimalarial_V5_CorrectedGrid/scripts/p1_v5_generate_figures.py`; the V6 generator is `Project1_Chem_space_antimalarial_V6_CorrectedGrid/scripts/v6_generate_integrated_data_figures.py`. V5 outputs include a 68-row target-wise long table, a four-row target summary, 68 geometric-gate records, within-target ranks, and three publication-quality figure families (target-wise scores, within-target ranks, and geometric QC) in both PDF and 300-dpi PNG. V6 outputs include a 17-row canonical-SMILES-joined exploratory metrics table and three figure families (RRS mutation profiles, exploratory RRS--PNS/ACSI relationships, and target-wise Vina profile summary) in PDF/PNG.

The derivation remains scientifically bounded: `mean_completed_targets` was excluded from descriptive data and figures; Vina scores remain target-specific; within-target ranks are not a potency consensus; geometric gates are pose/provenance QC rather than biological validation; and every pre-submission consensus/RRS/PNS output is labeled as development-only until the manuscript freeze. Source SHA-256 hashes are recorded in `v5_vina_derivation_provenance.json` and `v6_integrated_data_figures_provenance.json`. The registers retain `PENDING_INDEPENDENT_REVIEW` / `accepted_for_full_run=false`; these statuses are not computational blockers during the current development phase and do not constitute acceptance. The figures are descriptive/reproducibility assets, not independent biological validation.

The generated PDF figures were integrated into the V5/V6 canonical manuscript and Supporting Information sources through the local `manuscript/Graphics/` directories. V5 main now contains the target-wise score figure; V5 SI contains the within-target rank and geometric-QC figures; V6 main contains the RRS profile and exploratory cross-metric figures; and V6 SI contains the target-wise descriptive profile. Captions explicitly state score non-comparability, exploratory status, and pending independent structural review. Dependency-aware clean compilation of V5/V6 SI followed by main returned `0/0/0/0` for all four sources, with no fatal errors, missing figures, or undefined citations/references.
...
The safe backport scope is limited to terminology and caveats: 4GM2 analyses are PfClpR-specific, the historical parent-MD `164` label is not accepted as structural PfClpP validation, and no V5 pending-review result is imported into V4. **Fresh V4 replacement attempt (9 August 2026):** isolated directory `pfclpp_2f6i_484_uniform_seed20260809_run13999` contains 450 raw result records and 34 explicit failures, accounting for all 484 attempted centroids. A record-level inventory confirms unique IDs 0–483, no result/failure overlap, and no missing IDs; the aggregate directory is not populated, so no V4 numerical or manuscript result is updated. **Mapping resolution (9 August 2026):** the V4 V5–P2 overlay was rebuilt from the V6 source-bound candidate manifest. Row-wise RDKit canonical-SMILES checks now match each `PP-01`–`PP-17` identifier to its V5 four-target manifest rows and its P2 Set-C source row; the provenance sidecar records the V6 audit/validator hashes. The mapping is no longer order-only, but it remains a computational provenance correction and does not authorize biological or downstream consensus claims. The complete replacement run requires the canonical 484 centroid ligand mapping, 2F6I receptor/PDBQT frame verification, target-specific grid definition, fail-closed per-record accounting, and an updated downstream consensus only after independent review. The uniform array `13451` produced all 484 centroid directories, but its fail-closed audit `13972` recorded 449 independently passing records and 35 worker failures; no partial result was promoted. The separate rescue layer `pfclpp_2f6i_484_rescue_seed20260809_v2` produced 3 raw rescue results and 32 remaining worker failures. The corrected independent audit recovers all 35 requested IDs, reports zero unaccounted records, returns `RESCUE_SENSITIVITY_INCOMPLETE`, and cannot repair the canonical panel. An older cross-pass audit covers 17 exclusions with 16 IDs overlapping rescue-v2 and one cross-pass-only ID (171); the scopes are explicitly reconciled in `results/pfclpp_2f6i_audit_reconciliation_20260809.json` and are not pooled. The V4-specific review register is `results/pfclpp_2f6i_484_independent_review.json`, distinct from the V5/V6 four-target register.

**Computational-rescue authorization (9 August 2026; historical protocol record).** The first raw array produced 449 `PASS_RAW_VINA` records and 35 explicit failures (10 deterministic embedding failures, 8 non-finite Gasteiger-charge failures, 7 Vina exit-code failures, 9 composite biological-gate failures, and 1 multi-fragment preparation failure). A versioned rescue array is authorized only for these failed IDs. It may use an alternate deterministic conformer seed and higher Vina exhaustiveness, but it must preserve the canonical SMILES, receptor frame, fixed 2F6I triad-centered box, and the original biological gate. It may not enlarge/translate the box, relax containment/contact thresholds, or overwrite the first-run outputs. Rescue outputs are a sensitivity/provenance layer until the combined 484-record aggregate passes independent review; unresolved failures remain explicit and block promotion. The stale dependency job 12966 is superseded by the independent retry/merge chain documented in `Project1_Chem_space_antimalarial_V4_CorrectedGrid/scripts/p1_v4_retry_clpp_2f6i.sbatch`. To remove protocol heterogeneity rather than merely rescue failed records, a separate uniform 484-centroid run with the rescue protocol is also authorized; it writes to `pfclpp_2f6i_484_uniform_seed20260809` and is aggregated only through the parameterized fail-closed aggregator. The uniform run is a candidate replacement panel, not an accepted manuscript result, until independent review.
...
**Implementation status before full execution (9 August 2026).** A V4-local worker (`scripts/p1_v4_revalidate_clpp_2f6i.py`) and SLURM array wrapper (`scripts/p1_v4_revalidate_clpp_2f6i.sbatch`) were created. The worker binds each zero-based row of the 484-row `cluster_representatives_smiles.csv` to genuine PfClpP structure 2F6I, derives the chain-A catalytic-triad center from the PDB, verifies PDB/PDBQT frame equivalence, uses deterministic RDKit/Meeko preparation and seeded Vina, and writes only raw per-centroid provenance. It has no consensus/RRS/PNS side effects. A centroid-0 smoke initially failed because the worker passed a relative ligand path to Vina; the path handling was corrected, and a fresh smoke then passed (`PASS_RAW_VINA`, raw affinity −6.627 kcal mol−1, composite gate passed). This smoke is a technical QC check only and is not a manuscript result. The first submission, SLURM array **13999_[0-483]**, failed closed before Vina because all 484 target directories already contained prior artifacts (`non-empty output exists`); its outputs are therefore not treated as a new run. A fresh array **14483_[0-483]** was then submitted on 9 August 2026 to the previously absent directory `results/pfclpp_2f6i_484_uniform_seed20260809_run13999`. The corrected worker passed local compilation and verified the canonical header-plus-484-row mapping (zero-based IDs 0–483). Scientific promotion requires complete accounting and transparent evidence reconciliation from the fresh directory. No consensus/RRS/PNS or manuscript update is performed during the run, and no editorial restriction blocks continuation before explicit author reactivation.
...
**P1 V5 continuation.** The V5 canonical Main, Supplementary Material, and Cover Letter compile in a full-tree temporary copy (Main/SM LaTeX return code 0; Cover LaTeX return code 0, BibTeX return code 2 because no bibliography is defined). V5 remains in scientific QC development for new DiffDock/Vina execution: 4GM2 is PfClpR rather than PfClpP, fixed-center rank-1 containment is 79.17%, and the independent structural-review register remains pending. These are technical/provenance findings to resolve, not an editorial block; exploratory consensus, RRS, and PNS work may continue with explicit status labels.

**P2 guarded execution.** The four-system input/provenance preflight `scripts/p2_parent_md_preflight.py` now resolves historical aliases to canonical directories and reports **0/4 READY, 4/4 BLOCKED**: all four systems lack `forcefield_manifest.json`, and `438_PfATP4` additionally lacks `npt.gro` and `npt.cpt`. The canonical parent pipeline and direct execution wrappers invoke this input-read-only/report-writing gate before any GROMACS stage. Historical direct-call utilities are catalogued as non-canonical in `results/metrics/p2_gromacs_entrypoint_audit.json` and cannot support new publication-grade claims. GROMACS 2025.4-conda_forge remains installed at `/home/nanaengo/miniforge3/envs/malaria_md/bin.AVX2_256/gmx_mpi`. The Set-C preflight independently selected 16 systems, found 0 READY and 16 BLOCKED, and reported `gromacs_launched: false`.

**P2 historical diagnostic continuation (9 August 2026).** The first submission `14968_[0-1]` failed before GROMACS because the batch script was resolved under `/var/lib/slurm/slurmd` and inferred an unusable project path; no scientific output was produced. The launcher was corrected to require the explicit repository path via `P2_PROJECT_DIR`/`SLURM_SUBMIT_DIR`, to record machine-readable exclusions and input/dependency hashes, and to update provenance to `FAILED` on any non-zero exit. The corrected author-authorized background array **`14970_[0-1]`** was then submitted through `scripts/p2_historical_diagnostic_md.sbatch` for **201–PfDHFR and 214–PfCRT only**. Both systems passed a temporary-copy `grompp` smoke-test; the diagnostic attempted to use the existing historical CHARMM36-jul2022/GAFF2-ACPYPE NPT states, 10 ns per system at 310.15 K, and the isolated output root `results/diagnostic_md_20260809/`. However, both machine-readable provenance records report `status=FAILED` at `failed_stage=mdrun`; therefore the trajectory/analysis files are retained as diagnostic artifacts, not as a successful new MD run. It explicitly excludes 438–PfATP4 (missing NPT state) and 164–4GM2 (4GM2 is PfClpR, not PfClpP). This is not the publication-grade CHARMM36m+CGenFF reconstruction, not independent validation, not a new MM-GBSA estimate, and not MD-RRS. The canonical 4-system preflight remains `0/4 READY`; no publication-facing result is promoted.
...
**DiffDock V5 input preflight (Aug 7, 2026):** the local `diffdock` environment was inspected read-only (`torch 1.13.1+cu117`, one CUDA device visible), and the local DiffDock source/checkpoints were found under `/home/nanaengo/software/DiffDock`. The canonical P2 Set-C file contains 17 valid, unique SMILES and the four target PDB files are present and non-empty. The V5-only generator created a deterministic 68-row input manifest at `Project1_Chem_space_antimalarial_V5_CorrectedGrid/results/diffdock_polypharm/` using stable IDs `PP-01`–`PP-17`, absolute protein paths, canonical SMILES, candidate-file hash, protein hashes, inference-source/tree hash, configuration and normalization-array hashes, and explicit score/confidence checkpoint hashes. The directory contains a `NO_INFERENCE_SENTINEL`; it launches no DiffDock inference, writes no poses, and produces no score/RRS/PNS result. The manifest, runner, and raw output directories remain subject to scientific integrity checks. Their review status is recorded for provenance, but no editorial restriction blocks further development before explicit author reactivation. **Execution authorization (Aug 7, 2026):** the user explicitly authorized continuation through the full workflow. The sequence is fail-closed: one real smoke-test pair first; only if its raw SDF output, confidence filename, log, and identity pass verification may the 68-pair run proceed. No hardcoded confidence/Vina values are permitted. Any derived consensus/RRS/PNS output must state whether its raw-output verification is complete and retain exploratory provenance when it is not.
...
**Set-C MD workflow implementation (Aug 7, 2026):** `scripts/p2_setc_md_workflow.py` now provides the canonical auto-approved, non-interactive orchestration path for a bounded PP-01/PP-02 pilot. It validates the 136-row docking panel, candidate/target/mutation identities, candidate-specific `system_manifest.json`, CHARMM36m+CGenFF force-field manifests, and topology/coordinate hashes before any GROMACS call. It writes `results/set_c_md/set_c_md_status.csv` and `set_c_md_execution_manifest.json`, joins only docking-derived RRS/ACSI/PNS labels, and explicitly leaves `md_rrs_status=NOT_COMPUTED` until trajectory QC and a pre-specified MD analysis are complete. The companion `scripts/p2_setc_md_rrs.py` is the strict post-run MD-RRS analyzer: it accepts only a complete PASS-QC bound-fraction panel with trajectory hashes, candidate hash, and analysis-rule ID, and never overwrites docking-RRS. **STATUS UPDATE (Aug 10, 2026) — preparation COMPLETE, equilibration running, production pipeline ready:** 16/16 systems prepared with OpenFF 2.2.0 (AM1-BCC) ligand FF + CHARMM36m/TIP3P (PI-approved documented deviation, `policy_deviation.approved=true`). Root cause of the earlier equilibration crashes (arrays 15054/15070/15081/15090/15097) identified and fixed — **ROOT CAUSE v3 (10/08)** : `[molecules]` order misaligned with coordinates in `add_ligand_to_topol` (MOL0 first vs protein-first coords) → grompp silently mis-assigned → genion corrupted structures (collapsed aromatic rings TYR34 CG–CE1 0.68 Å / TYR238 OH–CD2 1.0 Å, exploded side chains LYS104 HE1–HE2 65 Å) → water NaN → non-deterministic libgomp segfault in Verlet pair-search. Fix: `[molecules]` order aligned (protein first, MOL0 second) + `ions.gro` regenerated from clean `solvated.gro` (validated: K76T 0 collapsed rings vs 114, LYS104 1.63 Å vs 65 Å; 16/16 verified clean). **Job 15106** (array 0-15 `%4`, `-ntomp 8`, EM doux `emstep=0.0001` sans POSRES) en cours : wave-0 (K76A/K76T/WT/C59R) en NVT, 0 failure, ETA ~12-16 h. **Production prête (10/08)** : `p2_setc_md_workflow.py` étendu au mode array (`--array-index` 1 système/tâche, `--ntomp`, manifests/status par tâche, flags mdrun CPU-only `-nb cpu -pme cpu -bonded cpu -update cpu` — l'A4000 crash GROMACS GPU) ; `p2_setc_production.sbatch` = array 0-15 `%4` 10 ns × 16 (~40 h) à lancer via `sbatch --dependency=afterok:15106` ; QC trajectoires `p2_setc_trajectory_qc.py` (bound fraction < 5 Å, règle pré-déclarée `setc_p2_minheavy_5A_ge10percent_v1`, MDAnalysis, contrat CSV) ; runbook `results/set_c_md/setc_md_rrs_execution_runbook.md` (stages 0-4). `md_rrs_status` restera `NOT_COMPUTED` tant que les trajectoires 10 ns n'auront pas passé le QC. Doc complète : `results/set_c_md/set_c_preparation_remediation_20260809.md` (ROOT CAUSE v3) + AGENTS.md.

**P2 Set-C MD execution checkpoint (12 August 2026; live HPC status):** Preparation of the **16/16 systems in the bounded Set-C pilot (PP-01/PP-02 × PfDHFR/PfCRT mutation states)** is complete under the PI-approved, explicitly documented force-field-policy deviation (OpenFF 2.2.0 AM1-BCC ligand parameters with CHARMM36m/TIP3P). The currently active chain is a **single isolated witness for `PP-01_PfDHFR_WT`**, not the full 16-system panel: equilibration witness job **15254** completed successfully and produced hashed `npt.gro` and `npt.cpt` states after the internal TER/chain-gap and molecule-order fixes; this is an equilibration/preparation result, not a binding or resistance result. Witness production job **15259** is currently running with a 10-ns target (5,000,000 integration steps); the latest observed marker was approximately **374,700/5,000,000 steps (~7.5%)**. Witness QC job **15260** is queued with `afterok:15259`. The earlier 16-system production/equilibration identifiers (15106/15111) are historical/superseded and do not represent the present live chain. No complete Set-C production panel has passed QC, and **no Set-C MD-RRS value, class, or manuscript claim has been generated** (`md_rrs_status=NOT_COMPUTED`). The historical four-system parent-lead MD remains a separate evidence layer and must not be pooled with this Set-C preparation or witness. Until the full candidate-specific production/QC workflow is completed, the canonical Set-C RRS/ACSI/PNS results remain docking-derived and unchanged. This checkpoint records execution provenance only; it does not upgrade the biological status of the PfCRT loop model or any candidate-specific binding claim.

**PfCRT gap-model biological gate (11 August 2026):** The experimental PfCRT model PDB 6UKJ (RCSB validation record: 3.3 Å; Kim et al., *Nature* 576, 315–320 (2019), DOI `10.1038/s41586-019-1795-x`, which reports approximately 3.2 Å) reports missing coordinates for residues 114–122 between GLY113 and HIS123. The local Set-C pilot therefore inserted an explicit `TER` at this unresolved transition and regenerated a CHARMM36m topology. This produced a technically consistent topology (`grompp -maxwarn 0` PASS), full isolated EM PASS, progressive constrained NVT stability at 0.2/0.5/1/2 fs, and an isolated 2-ps NPT witness. However, the `TER` split creates an artificial internal `COOH` terminus at GLY113 and `NH2` terminus at HIS123; the regenerated protein charge is therefore a model-policy consequence, not an experimentally established state. The complete local `PfCRT_K76A.pdb` and `PfCRT_K76T.pdb` files contain coordinates for residues 114–122 but are marked as computational OpenMM-generated models, not independent experimental structures; their sequence/numbering and loop geometry still require explicit reconciliation with PDB 6UKJ, so coordinate presence alone is not validation. Accordingly, the pilot is classified as **technical topology/stability evidence only**, not biological validation. No PfCRT WT/K76A/K76T extension, production MD, trajectory QC, or MD-RRS is promoted from this pilot. The next valid gate is either a validated loop-reconstruction protocol with local geometry/ensemble checks or an explicit literature-supported justification of the internal terminal treatment, followed by charge/patch, hash, and structural validation. An independent UniProt archive and mapping were added on 11 August 2026: reviewed accession `W7FI62` (7G8, 424 aa) is stored with hashed FASTA/flat-file provenance, and `p2_pfcrt_map_uniprot_to_pdb.py` reports zero sequence mismatches, UniProt/PDB residue 76 = T/THR, and all PDB 114--122 coordinates absent. The candidate audit (`Project2_Polypharmacology_MD_ValidationV2607/scripts/p2_pfcrt_gap_model_audit.py`) consumes this mapping and is now explicitly `BLOCKED_NO_PRELIMINARY_ELIGIBLE_CANDIDATE`: neither OpenMM-generated K76A/K76T candidate contains SEQRES records or a demonstrably mapped K76A/K76T identity at PDB residue 76, and their sequence/numbering cannot be reconciled to 6UKJ without an independent mapping. This is a provenance-screen block, not a definitive biological invalidation of every possible future model. Both are rejected as loop templates. The reconstruction boundary and promotion criteria are specified in `Project2_Polypharmacology_MD_ValidationV2607/P2_PFCRT_LOOP_RECONSTRUCTION_PLAN_20260811.md`. No candidate PDB or canonical Set-C preparation was modified.

**PfCRT GPU reconstruction and isolated stability-witness checkpoint (11 August 2026).** The initial CPU-only ColabFold attempt was stopped and preserved as operational provenance; the repaired `pfcrt_colabfold` environment used JAX CUDA 12 and job 15236 confirmed `CudaDevice(id=0)`. Ensemble job 15241 generated 15 AlphaFold2-PTM model/score pairs plus relaxation outputs. The corrected graft audit (15243) evaluated 16 PDB records and found 0 admissible grafted candidates because the predicted loop could not be joined continuously to the experimental GLY113/HIS123 anchors and several candidates contained steric clashes. The graft route remains rejected and no candidate was promoted to Set-C. A bounded computational result was then obtained from one complete relaxed full-length W7FI62 prediction (not the grafted model): hardened OpenMM witness job 15249 used OpenMM 8.5.2 CUDA, matched the input model's SHA-256 to the `declared_model_sha256` in the 15241 ensemble manifest, completed minimization plus 10,000 steps (20 ps), produced finite final coordinates and 10 finite StateDataReporter rows, and detected no NaN/Inf, CUDA, exception, segmentation-fault, fatal-error, or LINCS markers. Initial/minimized/final potential energies were −1,349,819.26/−2,371,367.22/−1,882,743.81 kJ mol−1; the observed temperature range was 257.11–311.69 K, so this is a startup/stability witness rather than an equilibrated thermodynamic trajectory. The manifest records the prepared-model hash and the explicit Amber14-all/TIP3P deviation from the P2 CHARMM36m+CGenFF/OpenFF policy. The status is `TECHNICAL_MODEL_STABILITY_PASS` only: the witness trajectory is an isolated technical artifact and does not validate the unresolved 6UKJ loop, PfCRT pocket, ligand binding, mutation resilience, production MD, canonical Set-C MD, QC, or MD-RRS. **PfCRT provenance-gate hardening checkpoint (11 August 2026).** The mapping and candidate-audit scripts were hardened after final read-only review: DBREF PDB/UniProt intervals are checked against the archived FASTA bounds; the expected PDB ID (`6UKJ`) and accession (`W7FI62`) are enforced; raw PDB/FASTA hashes are recomputed when the audit consumes the JSON artifacts; critical checks (K76=T/THR, complete PDB gap 114–122, zero sequence mismatches) are required rather than trusted from copied fields; and model/altloc/insertion-code ambiguity is fail-closed. The rerun passed compilation and mapping (`0` sequence mismatches; K76=T/THR; missing coordinates 114–122) and returned `BLOCKED_NO_PRELIMINARY_ELIGIBLE_CANDIDATE` for both OpenMM-generated mutant files. The final mapper also reparses the archived UniProt flat-file and requires accession/sequence agreement with the FASTA; the candidate-audit index is derived from the verified PDB-76 mapping rather than a standalone SEQRES-79 assumption, and duplicate atom records are rejected. No SLURM job, MD stage, QC, or MD-RRS was launched. This remains a provenance-screen conclusion, not a biological invalidation of future sequence-faithful models. **Loop-backend preflight (11 August 2026):** PDBFixer detected exactly one intended internal gap (chain-A key `(0,67)`, sequence `ASN-LYS-LYS-GLY-ASN-SER-LYS-GLU-ARG`), but the available environment lacks MODELLER/Rosetta or another supported internal-loop backend; the preflight therefore returned `BLOCKED_NO_LOOP_MODELING_BACKEND`, generated no candidate coordinates, and launched no MD/QC/MD-RRS stage. The next dependency is an explicitly versioned and hashed external loop-modeling backend; this is not a biological invalidation. **GPU infrastructure follow-up (11 August 2026):** the isolated ColabFold environment was repaired with CUDA-enabled JAX and verified on the RTX A4000 (`CUDA_VISIBLE_DEVICES=0`, `CudaDevice(id=0)`). GPU job 15236 produced one AlphaFold2-PTM model (pLDDT 74.2; pTM 0.717), but the isolated graft audit (job 15238) found zero geometry-passing candidates: the expected loop sequence was present, while anchor RMSD was 2.188 Å, mean loop pLDDT 47.43, and peptide-bond distances were implausible. The final structural status is `BLOCKED_INSUFFICIENT_PASSING_ENSEMBLE`; the PDB remains quarantined and no topology, Set-C system, MD, QC, or MD-RRS was launched. Earlier CPU-only attempts produced no coordinates and are operational failures only. GPU activation therefore solved the infrastructure bottleneck but did not upgrade the biological/structural evidence.
...
## P3 External Validation on the Public ChEMBL Malaria Dataset — COMPLETE (2026-08-09, jobs 12858→12860→12891→13997→13998)

> 🟢 **Action à fort impact d'acceptation, documentée AVANT exécution (règle workflow) :** nouveau script `Project3_Quantum_Inspired_RepresentationsV2607/scripts/p3_external_validation.py` — benchmark des descripteurs P3 (ECFP4, FCFP4, MACCS, AP, PHCO, BPF, **TFP 78-d**, **TNE 192-d**, **Hybrid TFP+TNE**) sur le **dataset public externe ChEMBL malaria IC50/EC50** (22,447 molécules ; 19,321 actives / 3,126 inactives — le même jeu brut que la validation externe P5, `Project5_GNN_Transformer_DrugDiscovery/results/p5_public_chembl_malaria.csv`). Pour P3, ce panel est externe au jeu d’étiquettes eOS80CH, mais sa disjointeté moléculaire avec le panel interne n’a pas été établie et n’est pas revendiquée ; P5 utilise séparément le sous-ensemble disjoint de 22,267 molécules après exclusion de 180 recouvrements. Protocole identique au benchmark canonique (5-fold StratifiedKFold 42, RF 200 arbres, ACT_THRESHOLD 0.5, paired-t per-fold + BH-FDR) ; panel ITT commun = 22,447 RDKit-parseable molecules; TNE failures are retained as explicit zero-vector penalties, while a separate complete-case sensitivity uses 22,096 molecules. Répond à la question « les descripteurs topologiques généralisent-ils hors du panel interne ? » (le verdict interne TFP 0.876 / TNE 0.722 / ECFP4 0.948 doit être confirmé ou nuancé sur données indépendantes). Sorties : `results/p3_external_validation_report.json` + `_summary.txt` + `_csv`. **Note environnement local : pennylane→jax 0.10.2 requiert numpy≥2.0 alors que malaria_md a numpy 1.26.4 → import pennylane cassé localement ; le script copie donc les helpers canoniques (ECFP4, CV) sans pennylane (aucun code quantique n'est exécuté par la validation externe) ; les jobs QKS canoniques restent inchangés (env HPC).** Soumis job 12858 (16 CPU, budget 8h) → **relancé job 12860** après fix de thrashing ripser/BLAS (exports `OMP/MKL/OPENBLAS_NUM_THREADS=1` manquants dans le sbatch initial — débit ×2 ; committé `bf18bae58`).
>
...
> 📊 **Status final (9 August 2026):** the descriptor ITT output is complete for 22,447 molecules, with 351 explicit TNE failures retained as zero-vector penalties. The complete-case sensitivity (job 13997) is also complete on 22,096 molecules and gives ECFP4 0.9601, TFP 0.8611, TNE 0.6437, and TFP+TNE 0.7981, versus ITT values 0.9601, 0.8645, 0.6448, and 0.8000. The ranking and scientific conclusion are unchanged. The deterministic post-processing audit (job 13998; `results/p3_external_validation_statistical_audit.json`) migrated explicit failure-mask provenance and replaced naive fold-level inference with corrected-resampled exploratory statistics; no claim is labelled confirmatory. Job 13996 failed before execution because the CPU-only wrapper sourced GMXRC under nounset; it produced no scientific output. The historical output is therefore correctly described as ITT/zero-vector-penalty, not complete-case.

## P3 QKS External Validation on the Public ChEMBL Malaria Dataset — COMPLETE (job 12863; statistical audit job 13998)
...
>
> **Interprétation finale :** le pilote n=150 avait suggéré quantum ≈ RBF, mais le run complet n=22,447 (job 12863) donne quantum 0.8172 contre RBF 0.8466. Le test corrigé-resampled est exploratoire ($p=0.021$, BH $q=0.021$); le $p=0.0053$ du t-test naïf sur cinq folds est conservé comme comparateur d'audit, non comme preuve confirmatoire. Quantum et RBF restent supérieurs au noyau linéaire dans la même analyse, mais le résultat ne démontre aucun avantage quantique spécifique. Panel complet : 19,321 actifs/3,126 inactifs, 5-fold StratifiedKFold(42), SVM pré-calculé vs RBF/linéaire, protocole C3. La comparaison avec ECFP4-RF conserve le caveat de classifieur différent.
