...
**Manuscript coherence refresh — 10 August 2026:** `P5_manuscript_V2608` re-checked against this report — external ChEMBL transfer analysis (n=22,267: scaffold ECFP4-RF 0.9190 > GIN 0.8843, Δ=+0.0346; random 0.9547 vs 0.9237), the paired-t (df=4) + BH-FDR statistics, and audit v2 (§10–11 of `P5_ADVERSARIAL_AUDIT_MITIGATION.md`) are consistent with the manuscript claims. The manuscript now uses five per-seed means for the displayed SD/CI estimand and explicitly distinguishes the external split from the frozen internal folds.
...
| v4 | Aug 8, 2026 | **Réplication indépendante + validation externe MoleculeNet malaria** | random + scaffold | **Réplication ECFP4-RF (jobs 12844/12845)** : verdict **PASS** (écart 0.000 vs committé, tol. 0.005) — protocole ECFP4-RF (RDKit Morgan r=2, 2048bits, RF 500, splits figés) verrouillé (`p5_replication_verification.json`). **Validation externe MoleculeNet malaria n=22,267 (jobs 12846-12848, GPU)** : scaffold **ECFP4-RF 0.9190 > GIN 0.8843 (Δ=+0.0346, p<0.0001)** ; random 0.9547 vs 0.9237 (Δ=+0.031, p=0.0001). Le verdict « fingerprints > GNN » est reproduit sur un panel public indépendant → **L3 renforcé hors panel canonique**. **Manuscrit `P5_manuscript_V2608.pdf` compilé (08/08 06:58)** : 0 erreur/undefined/TODO, cover letter JoC. **Job 12889:** cancelled deliberately after the complete scaffold `_metrics` replication; scaffold output retained (`0.7908 ± 0.0298`), redundant random `_metrics` step abandoned, canonical random result from job 12815 unchanged. | `p5_replication_verification.json`, `p5_public_malaria_report.json`, `P5_manuscript_V2608.pdf`, `results/p5_chemberta_scaffold_results_metrics.csv`, `logs/p5_cb_metrics_12889.log` |
