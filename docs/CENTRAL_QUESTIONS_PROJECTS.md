...
4. Not yet computed. Scaffold-held-out runs for molecular arms remain `NOT_COMPUTED`, along with per-label calibration plots and QKS sensitivity analysis.

### Scientific interpretation and limits

P6 is an honest-negative benchmark: molecular representations that succeed on chemical-activity tasks (P3, P5) do not transfer to MoA classification when the prediction target is phenotype-like and collision groups enforce drug-level separation. The collision-group split prevents information leakage between branded/generic variants of the same drug. The 794 groups (from 1,722 unique molecules) provide a meaningful but modest evaluation set. Conclusions are bounded by the PRISM mapping scope, the collision-group split design, and the absence of external validation. The result is hypothesis-generating: it suggests MoA classification from structure alone is harder than activity prediction for this cohort.
