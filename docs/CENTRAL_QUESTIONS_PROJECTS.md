...
This document states the scientific questions addressed by Projects P1–P6 and the evidence-bounded answers currently supported by the canonical analyses. Docking, scoring functions, molecular descriptors, and model outputs are computational evidence; they do not by themselves establish experimental potency, target engagement, binding, or resistance circumvention.
...
2. How are mutation-resilience estimates distributed across the PfDHFR and PfCRT mutant panels when the RRS denominator is defined per target and non-binding wild-type targets are excluded? (The mutant states are WHO-listed molecular markers of antimalarial resistance [WHO Compendium](https://www.who.int/tools/compendium-of-molecular-markers-for-antimalarial-drug-resistance), giving the panel a clinical anchor.)
...
1. The Set-C cohort was selected as a polypharmacology-oriented, enriched hypothesis cohort rather than a prevalence sample. The four-target wild-type matrix contains 68 candidate–target records that passed the defined geometric pose-quality gate. PP-06 is among the strongest target-profile candidates and is top-quartile across all four target profiles. The joint breadth-plus-A* RRS readout identifies PP-15, PP-06, and PP-11 as the three candidates favourable on all four target profiles and assigned to class A*. These are computational docking profiles, not experimentally demonstrated multi-target binding. Under the single-drug-multi-target (SDMT) rationale, such breadth combined with per-target mutational retention is precisely the design objective proposed to raise the resistance barrier [Trends Parasitol 2026](https://www.cell.com/trends/parasitology/abstract/S1471-4922(26)00040-1); the class-A* readout is reported as an SDMT-style hypothesis, not as measured multi-target engagement.
2. Per-target RRS classified the 17 candidates as **A\***: 6, **B**: 5, **C**: 5, and **D**: 1, with an observed range of 68.2–111.7%. The RRS calculation used genuine wild-type binders only (\(|S_{\mathrm{Vina,WT}}| \geq 5.0\) kcal mol\(^{-1}\)); non-binding wild-type targets were not used as denominators. The PfDHFR and PfCRT mutant panels therefore provide a mutation-resilience analysis separate from the four-target wild-type docking matrix. This per-target definition aligns with the single-drug-multi-target (SDMT) paradigm where multi-target activity is pursued to raise the resistance barrier beyond combination-like effects [Trends Parasitol 2026](https://www.cell.com/trends/parasitology/abstract/S1471-4922(26)00040-1), and with recent PfCRT/PfDHFR mutational docking/MD showing target-specific resilience shifts that require a WT-binder denominator [Comput. PfCRT/PfDHFR 2025](https://pubmed.ncbi.nlm.nih.gov/40915500/).
3. PNS–RRS showed a negative exploratory association (Spearman \(\rho=-0.559\), \(p=0.020\), \(n=17\)), but it did **not** survive the Bonferroni threshold of approximately 0.017. The relationship is therefore a hypothesis-generating trend, not a confirmed association. ACSI–RRS was also not significant (\(\rho=-0.132\), \(p=0.613\)). Network-pharmacology scores are themselves sensitive to database coverage and hub bias [Trends and Pitfalls 2025](https://pmc.ncbi.nlm.nih.gov/articles/PMC12030339/), so the PNS result is reported as a bounded ranking heuristic, not as a validated network property.
...
P2 is a computational validation and quality-control study centred on polypharmacology and resistance resilience. RRS, PNS, ACSI, docking, MD, and MM-GBSA address different evidence layers and should not be treated as interchangeable measures. Resistance-aware design now formalizes mutational liabilities as design-time objectives — site-aware mapping, Pareto K\* positive/negative design and hotspot→distributed robustness [ResistAgent 2026](https://doi.org/10.64898/2026.06.16.732538), [RESISTOR/OSPREY](https://pmc.ncbi.nlm.nih.gov/articles/PMC9807075/), [CAPTURE](https://doi.org/10.48550/arxiv.2403.02603) — our RRS/MD/MM-GBSA layers instantiate this separation for a 4-target antimalarial panel (PfDHFR/PfCRT/PfATP4/PfClpP as clinically relevant mechanistic classes [RSC Adv 2026](https://pubs.rsc.org/en/content/articlehtml/2026/ra/d5ra08585c)). The cohort is enriched, the MD trajectories are short, and experimental potency and target engagement remain unmeasured. The bounded 16-system Set-C pilot completed 18 Aug 2026 (job 15320 — 16/16 trajectories; QC `exit 0`, MD-RRS `COMPUTED_WITH_COHORT_CONTRACT` for PP-01/PP-02, MM-GBSA 16/16); full-panel (17×8=136) MD-RRS remains `NOT_COMPUTED` by design (pilot scope only).
...
P5 supports ECFP4–RF as a strong baseline and treats TFP/TNE fusion as complementary descriptive modelling rather than an automatic route to higher predictive performance. The scaffold split is the principal generalisation test; the results do not establish universal superiority of fingerprints or universal inferiority of deep models. The V2 manuscript (16p main + 4p SI) is ready for author review with JCAMD as the target venue.

## P6: LISH-MoA Structure–Phenotype Prediction

### Central question

Can molecular-representation-based models (ECFP4, GNN, ChemBERTa, topological) predict drug mechanism-of-action classifications for antimalarials better than a phenotype-only baseline when drug-level collision groups are held out?

### Subquestions

1. Can a versioned, auditable drug_id → SMILES mapping cover the full LISH-MoA cohort (3,289 drugs, 206 labels)?
2. Do structure-only molecular arms outperform the phenotype collision-group baseline under an honest, drug-grouped split?
3. Does a phenotype + ECFP4 fusion improve over the phenotype-only baseline?
4. Are the molecular-arm results robust under a scaffold-held-out sensitivity analysis?

### Answering results

1. Yes. The PRISM strategy B mapping achieved 100% coverage of 3,289 drugs, yielding 1,722 unique SMILES and 794 collision groups. The frozen contract is `drugid_to_smiles_v2_validated.csv` (25 August 2026).
2. No. Under the collision-group split, all four molecular arms (GIN 0.02419, GIN-TFP 0.02334, GIN-TNE 0.02434, ChemBERTa 0.02549 log loss) showed macro-AUROCs near 0.50–0.51, at chance level. The phenotype collision-group baseline achieved log loss 0.02428 and AUROC 0.6362. No molecular arm beat the baseline on either log loss or AUROC.
3. No. The fusion arm (phenotype + ECFP4) achieved AUROC 0.5832, also below the phenotype-only baseline. Adding structure information degraded performance.
4. Yes (robust honest-negative). Under the scaffold-held-out split all four molecular arms remained near chance (GIN 0.50642, GIN-TFP 0.50077, GIN-TNE 0.50609, ChemBERTa 0.50344 macro-AUROC, all below the phenotype reference 0.64023). See §4.6 of the P6 DAR for the corrected scaffold table.
5. (Decision 30 Aug 2026) A true cross-modal attention-fusion arm is **deferred, not run**: the `COMPUTED` late-concat fusion already answers the central question's second branch as an honest-negative (fusion below phenotype under both splits), structure-only arms are near chance, so attention has no additional structure signal to attend over; and the single GPU is saturated by the active M1 production job. Status: `PLANNED_SECONDARY`, unchanged. See `P6_DATA_ANALYSIS_REPORT.md` §1 / §4.6 decision record.

### Scientific interpretation and limits

P6 is an honest-negative benchmark: molecular representations that succeed on chemical-activity tasks (P3, P5) do not transfer to MoA classification when the prediction target is phenotype-like and collision groups enforce drug-level separation. The collision-group split prevents information leakage between branded/generic variants of the same drug. The 794 groups (from 1,722 unique molecules) provide a meaningful but modest evaluation set. The honest-negative conclusion is stable under both the collision-group and the scaffold-held-out splits, and the late-concat fusion arm does not cross the phenotype reference either. Conclusions are bounded by the PRISM mapping scope, the collision-group split design, and the absence of external validation. The result is hypothesis-generating: it suggests MoA classification from structure alone is harder than activity prediction for this cohort. Updated 30 Aug 2026 to reflect the completed scaffold benchmark and the deferred attention-fusion decision record (see P6 DAR §1/§4.6).

## Cross-project synthesis

Across P1–P6, the programme produces a consistent methodological message:

1. **Separate evidence layers.** Docking scores, RRS, PNS, ACSI, MD, MM-GBSA, molecular descriptors, phenotype features, and predictive AUCs answer different questions.
2. **Use strong baselines.** ECFP4 remains difficult to exceed in P3 and P5; random search remains difficult to exceed in P4; the phenotype-only baseline remains difficult to exceed in P6.
3. **Treat negative results as results.** Null DEKOIS enrichment, non-significant PNS–RRS association, no quantum advantage, the failure of larger models to beat ECFP4, and the failure of structure-only models to beat phenotype-based MoA prediction define the limits of the tested approaches.
4. **Prioritise transparent decision support.** P1/P2 provide target- and mutation-aware hypotheses; P4 provides Pareto trade-off visibility; P3/P5/P6 identify when complex representations do not improve prediction.
5. **Bound all claims.** The projects generate computational hypotheses and reproducible comparisons; experimental potency, target engagement, resistance circumvention, MoA validation, and prospective clinical utility remain to be tested.
