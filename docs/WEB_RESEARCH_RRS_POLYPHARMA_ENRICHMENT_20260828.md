...
> **Implementation (28 Aug 2026, evening):** S1–S4, S6, S7 applied to the P2 main manuscript (`Polypharmacology_MD_Validation_V2607.tex`) with 11 bib keys appended to `Bibliography_P2.bib`; P2 main (29 p.) + SM (15 p.) recompiled with 0 errors / 0 undefined refs. S7 (artemisinin context) was already present in the P2 Introduction; the WHO Q\&A citation was added.
>
> **S5 — COMPUTED (28 Aug 2026):** `scripts/p2_gnina_consensus_rescore.py` ran on the full 312-pose external panel (array 15605 complete; EXT-039 declared EMBED_FAILURE per the frozen repair ledger, so the panel is 39 ligands × 8 states = 312, not 320). GNINA 1.3.2 `--score_only` rescored all 312 MODEL-1 Vina poses; non-AD generic atom types (`CG0/CG1/G0/G1`) were normalized to `C` in the type column (Meeko/OpenBabel output). **312/312 finite CNN scores, 0 failures** → `results/robustness_transfer_20260827/gnina_consensus_20260828/`. `scripts/p2_gnina_consensus_rrs.py` then recomputed the frozen RRS estimand under both scoring layers (target eligibility decided once on the canonical Vina scale): **38/38 eligible ligands class A under both Vina and CNN, 0 class discordances**; RRS mean Vina 100.4 vs CNN 103.6; Spearman (ligand mean RRS) = 0.558; per-mutant ρ K76T 0.644, K76A 0.486, N51I 0.271, S108N 0.204, I164L 0.040, C59R −0.076. Status `COMPUTED_CONSENSUS_RRS_SENSITIVITY`; Vina-only remains canonical.
