...
\caption{\textbf{Explicit-Solvent Molecular Dynamics Trajectory Stability Time-Series (\qty{10}{\ns}).} RMSD trajectories (\si{\angstrom}) over \qty{10}{\ns} production simulations for (A) \textit{Pf}DHFR complexes (wild-type backbone \qty{1.42 \pm 0.15}{\angstrom}, N51I backbone \qty{1.55 \pm 0.18}{\angstrom}, \textbf{PP-01} ligand \qty{0.85 \pm 0.12}{\angstrom}); and (B) \textit{Pf}CRT transporter complexes (wild-type backbone \qty{2.15 \pm 0.22}{\angstrom}, K76T backbone \qty{2.28 \pm 0.25}{\angstrom}, \textbf{PP-01} ligand \qty{1.15 \pm 0.14}{\angstrom}). All trajectories reach rapid thermal equilibration ($< \qty{2}{ns}$) and remain tightly anchored without dissociation. Rendered using \texttt{SciencePlots} publication styling.}
...
\caption{\textbf{ProLIF Non-Covalent Interaction Persistence Heatmaps Across Explicit-Solvent Trajectories.} Interaction fingerprint occupancy (\% of \qty{10}{\ns} production trajectory, 100 snapshots per system) quantified for (A) \textit{Pf}CRT central cavity anchor residue Tyr16 across six mutant states (including K76T and K76A), demonstrating persistent $\pi$--$\pi$ stacking (\qty{96}{--}\qty{100}{\percent}) and hydrophobic contact persistence; and (B) \textit{Pf}DHFR binding pocket hotspots (Asp54 salt-bridge anchor, Leu46, Met55, and Ile14) across five mutant states (WT, N51I, C59R, S108N, and I164L), illustrating near-total contact persistence (\qty{89}{--}\qty{100}{\percent}) for candidates \textbf{PP-01} and \textbf{PP-02}. Rendered using \texttt{SciencePlots} publication styling.}
