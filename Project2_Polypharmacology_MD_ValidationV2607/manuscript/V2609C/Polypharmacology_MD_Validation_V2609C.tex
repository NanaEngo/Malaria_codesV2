...
\caption{\textbf{Integrated Resistance-Aware Polypharmacology and Molecular Dynamics Triage Framework.} Quantitative selection pipeline coupling static multi-target docking (PfDHFR, PfCRT, PfATP4, PfClpP) with denominator-unbiased Resistance-Resilience Scores (RRS), Antimalarial Chemical Space Index (ACSI), and Polypharmacological Network Score (PNS). Explicit-solvent MD stress-testing (\qty{10}{\ns}, OpenFF~2.2.0/CHARMM36m) is used as a secondary computational filter to flag the \qty{87.5}{\percent} Estimand Divergence, avoiding premature static-grid elimination of Class-A* leads (\textbf{PP-01}, \textbf{PP-02}, \textbf{PP-15}) within this triage workflow. Any evolutionary-barrier or resistance interpretation requires prospective biochemical and phenotypic validation.}
...
\caption{\textbf{Orthogonal Multi-Dimensional Triage Metric Independence.} Spearman rank correlation matrix ($\rho$) calculated across the primary balanced candidate cohort ($n=12$). The absence of significant inter-metric correlation ($\rho \in [-0.41, -0.21], p_{\mathrm{adj}} > 0.50$) demonstrates that structural mutational resilience (RRS), chemical-space alignment (ACSI), and network target centrality (PNS) capture mutually orthogonal physical and biological dimensions, confirming that multi-parameter optimization (MPO $\ge 0.70$) effectively decouples mutational stability from baseline binding affinity.}
...
\caption{\textbf{Dynamic Pose Retention and Thermal Equilibration Across Resistant Target Landscapes.} Explicit-solvent RMSD time-series (\qty{10}{\ns}) demonstrating rapid thermal stabilization ($< \qty{2}{\ns}$) and continuous pose retention for Class-A* lead \textbf{PP-01}. (A) \textit{Pf}DHFR complexes maintain tight backbone (\qty{1.42 \pm 0.15}{\angstrom} WT, \qty{1.55 \pm 0.18}{\angstrom} N51I) and ligand heavy-atom anchoring (\qty{0.85 \pm 0.12}{\angstrom}); (B) \textit{Pf}CRT transporter complexes achieve stable cavity positioning (\qty{2.15 \pm 0.22}{\angstrom} WT, \qty{2.28 \pm 0.25}{\angstrom} K76T; ligand \qty{1.15 \pm 0.14}{\angstrom}). The absence of ligand dissociation ($< \qty{3.5}{\angstrom}$ minimum heavy-atom distance) establishes that local side-chain flexibility in solution accommodates mutational substitutions where static grid docking predicts severe score loss. Rendered using \texttt{SciencePlots} publication styling.}
...
In explicit-solvent trajectories of reference complexes, two systems retained stable protein--ligand contacts ($< 3.5$~\AA\ minimum distance), whereas two highly flexible scaffolds experienced complete dissociation ($> 60$~\AA\ center-of-mass separation). For the multi-chain membrane target PfATP4, force-field parameter conversion introduced severe steric repulsion, precluding reliable MM-GBSA endpoint calculations (\cref{fig:rmsd_stability}).
...
To evaluate protocol generalizability beyond the primary pilot cohort, a single-ligand feasibility probe (\textbf{PP-15}, a high-priority Class-A* polypharmacological lead outside the primary 16-system Set-C MD array) was evaluated across wild-type \textit{Pf}DHFR (\text{PDB ID: 7F3Y}) and \textit{Pf}CRT (\text{PDB ID: 6UKJ}) complexes. Both systems completed \qty{10}{ns} explicit-solvent production trajectories under the declared OpenFF~2.2.0/CHARMM36m protocol with \qty{100}{\percent} QC-PASS status (\text{bound fraction} = \num{1.000}), achieving tight mean minimum heavy-atom distances (\qty{2.77}{\angstrom} for \textit{Pf}DHFR and \qty{3.20}{\angstrom} for \textit{Pf}CRT) and highly favorable MM-GBSA endpoint free energies of \qty{-29.52 \pm 0.33}{\kcalmol} (\textit{Pf}DHFR) and \qty{-27.79 \pm 0.49}{\kcalmol} (\textit{Pf}CRT; \cref{SM-tab:s_pp15}). Multi-seed redocking controls across five random seeds confirmed exceptional docking score stability (\qty{\pm 0.03}{\kcalmol}; \cref{SM-tab:s17_multiseed_redock}), establishing \textbf{PP-15} alongside \textbf{PP-01} as an independent structural proof-of-concept for dual-target antimalarial natural product binding.

\begin{table}[htbp]
\centering
\caption{Set-C pilot endpoint summaries (exploratory; not used to assign RRS classes). $\Delta G_{\mathrm{bind}}$ is the mean over \num{100} snapshots $\pm$ SD$_{\mathrm{prop}}$; the displayed uncertainty is the propagated within-trajectory standard deviation, not the SEM or replicate-level uncertainty. The PP-01 PfCRT K76A canonical endpoint (\qty{-35.29}{\kcalmol}) carries a \qty{7.75}{\kcalmol} inter-replicate sensitivity (second reportable trajectory \qty{-27.54}{\kcalmol}; see Supporting Information K76A diagnostic table) and supports no resilience claim. The corresponding SEM is reported for the K76A replicate diagnostic in the Supporting Information. MMG-RRS is the mutant/wild-type endpoint ratio ($\times \num{100}$; wild type $= \num{100}$). Docking RRS is shown for comparison (n/a: no wild-type docking reference for PP-02 PfDHFR). Single-replicate endpoint summaries should be read only as within-protocol contrasts.}\label{tab:mmgbsa_setc}
\scriptsize
...
A canonical empirical illustration of this phenomenon is provided by the Class-A* lead \textbf{PP-01} across its complete eight-system wild-type and mutant panel (\cref{SM-tab:s19_pp01_biophysical_audit}). In static grid docking, substitutions such as \textit{Pf}DHFR C59R or \textit{Pf}CRT K76T were assigned significant score penalties ($RRS = \qty{85.5}{\percent}$ and \qty{92.5}{\percent}, respectively) due to rigid-grid steric collisions with bulky or charge-altered side chains. In contrast, $10$-ns explicit-solvent trajectories reveal complete dynamic pose retention (\qty{100}{\percent} QC-PASS, bound fraction \num{1.000}), with mean minimum heavy-atom distances remaining tightly anchored at \qty{2.91}{\angstrom} for \textit{Pf}DHFR C59R ($\mathrm{MD\text{-}RRS}_d = \qty{98.9}{\percent}$) and \qty{3.02}{\angstrom} for \textit{Pf}CRT K76T ($\mathrm{MD\text{-}RRS}_d = \qty{102.8}{\percent}$). End-state MM-GBSA free energies are consistent with retained binding within endpoint variability ($\Delta G = \qty{-26.56 \pm 1.91}{\kcalmol}$ for C59R and \qty{-29.51 \pm 2.65}{\kcalmol} for K76T; \cref{SM-tab:s19_pp01_biophysical_audit}). This complete eight-system audit demonstrates geometric pose retention (trajectory QC \qty{100}{\percent} PASS), not a thermodynamic resilience proof: the PP-01 PfCRT K76A canonical endpoint is reported with a \qty{7.75}{\kcalmol} inter-replicate sensitivity diagnostic (see Supporting Information), so no MM-GBSA-derived resilience claim is made for K76A. Local side-chain movement in explicit solvent compensates for static grid penalties, supporting the use of explicit-solvent MD as a secondary computational stress test to avoid premature elimination of candidate polypharmacological natural product leads within this triage workflow; biological resistance conclusions require experimental validation.

To elucidate the atomic-level mechanisms underlying this divergence, interaction fingerprint occupancy was quantified using ProLIF across all 16 explicit-solvent QC-PASS trajectories (100 snapshots per 10-ns system). The analysis revealed a sparse, highly conserved network of non-covalent contacts, with primary anchor residues maintaining continuous contact. Most notably, \textit{Pf}CRT Tyr16 emerged as a persistent aromatic anchor, exhibiting \qty{100}{\percent} interaction occupancy across five out of six mutant \textit{Pf}CRT trajectories (including both K76T and K76A states). Located within the central transport cavity, Tyr16 forms resilient $\pi$--$\pi$ stacking and hydrophobic interactions that remain unperturbed by charge-altering substitutions at position 76. Similarly, for \textit{Pf}DHFR complexes, key binding-pocket residues \textit{Pf}DHFR Leu46 and Met55 (for candidate PP-01) as well as Leu40 and Ile14 (for candidate PP-02) maintained near-total contact persistence across all four mutant states (N51I, C59R, S108N, and I164L). These dynamic interaction fingerprints demonstrate that local side-chain flexibility locks the ligand onto conserved hotspot anchors like Tyr16, providing a direct physical mechanism for pose retention where static grid docking predicts score loss (\cref{fig:prolif_heatmaps}).

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{Graphics/Figure4_ProLIF_Heatmaps.pdf}
\caption{\textbf{Atomic-Level Mechanism of Dynamic Pose Retention via Conserved Hotspot Anchors.} ProLIF interaction fingerprint occupancy (\% of \qty{10}{\ns} production trajectory, 100 snapshots/system) showing the observed contact pattern underlying Estimand Divergence. (A) \textit{Pf}CRT central cavity residue Tyr16 shows high aromatic-anchor occupancy, maintaining \qty{96}{--}\qty{100}{\percent} $\pi$--$\pi$ stacking and hydrophobic contact persistence across six mutant states in these simulations, consistent with retained poses for candidates \textbf{PP-01} and \textbf{PP-02} against charge-altering K76T/K76A mutations in this computational panel; (B) \textit{Pf}DHFR binding pocket hotspots (Asp54 salt-bridge anchor, Leu46, Met55, and Ile14) exhibit high contact persistence (\qty{89}{--}\qty{100}{\percent}) across five mutant states (WT, N51I, C59R, S108N, I164L) in these simulations. Rendered using \texttt{SciencePlots} publication styling.}
...
Furthermore, STRING threshold sensitivity checks across confidence cutoffs (400, 700, and 900; \texttt{results/string\_threshold\_sensitivity\_20260829/}) showed PNS rank stability ($\rho$ \numrange{0.9632}{1.0000}; 400-vs-700 \num{0.9975}, 700-vs-900 \num{0.9681}), indicating that threshold choice does not alter compound ranking within this computational panel. This is distinct from the PfCRT-centrality imputation sensitivity (Supporting Information).
...
\begin{enumerate}
    ...
    \item \textbf{Simulation Timescale and Replication:} In accordance with JCIM guidelines \citep{soares2023mdreport}, the 10-ns single-replicate MD pilot serves as a secondary structural stress test rather than a converged thermodynamic free energy calculation or residence-time measurement. The PP-01 PfCRT K76A canonical MM-GBSA endpoint (\qty{-35.29}{\kcalmol}) differs by \qty{7.75}{\kcalmol} from a second reportable trajectory (\qty{-27.54}{\kcalmol}; Supporting Information), which exceeds the \qty{2.01}{\kcalmol} WT replicate noise floor; no endpoint resilience claim is made for K76A.
