...
\begin{table}[htbp]
\centering
\caption{Sensitivity of the PNS ranking to the imputed PfCRT centrality. The reference value is the observed network-mean imputation ($C_{\mathrm{PfCRT}}=\num{0.151}$). Alternative values were zero, one-half, one-and-a-half, and twice the reference value. Spearman $\rho$ is calculated against the reference 17-candidate ranking; all candidates have two target entries in the PNS record. The ranking is sensitive to this network-data choice; target essentiality and biological polypharmacology were not assessed.}
