%% ============================================================================
%% Paper 2: Estimand Divergence as a Discovery Signal for African Antimalarial Natural Products
...
\title{Estimand Divergence as a Discovery Signal for African Antimalarial Natural Products}
...
\begin{abstract}
No study has systematically calibrated docking reliability for African antimalarial natural products, a chemical class whose rigid polycyclic frameworks and rare functional groups may challenge scoring functions. We computed docking-derived resistance-retention scores (RRS) on \num{136} PfDHFR and PfCRT wild-type and mutant systems for a \num{17}-candidate cohort from \num{65856} molecules derived from African natural products. The primary analysis on \num{12} two-target candidates produced a five-class stratification that separates predicted wild-type potency from predicted mutant retention. A \num{16}-system single-replicate pilot on two candidates revealed that docking-RRS and short-MD geometry metrics diverged in direction for seven of eight mutant comparisons (87.5\% divergence rate). This elevated divergence rate constitutes a diagnostic signal identifying candidates whose docking ranking is likely an artifact of static pose scoring. An external \num{39}-ligand panel confirmed that docking-score retention is protocol-dependent. The study supports computational prioritization as a hypothesis-generating tool but does not establish biological target engagement, affinity, or resistance resilience.
\end{abstract}
...
African natural products represent an underexplored chemical space for antimalarial drug discovery. A systematic review identified 652 plant taxa with documented antimalarial activity, yielding 134 prioritized compounds from 39 species~\citep{moyo2023prioritised}, yet fewer than 5\% of African NPs have been validated computationally~\citep{african_np_underexplored}. This gap is critical because African NPs present molecular characteristics---rigid polycyclic frameworks, macrocyclic systems, and rare functional groups---that challenge scoring functions calibrated for drug-like molecules~\citep{ramirez2018docking,ancajas2024np_sar}. Under the single-drug--multi-target (SDMT) paradigm, simultaneous loss of activity at several relevant targets raises the mutational barrier to treatment failure~\citep{sdmt2026trends,sdmt_trends_parasitology_2026}, and natural products are attractive starting points because their stereochemical complexity supports activity across multiple protein classes~\citep{polypharmacology_malaria_2026,value_addition_african_np_2025}. However, a list of predicted protein contacts is not a mechanism-of-action assignment; causal network analyses require explicit perturbation models and evidence that predicted interactions propagate to a phenotype~\citep{Trapotsi2022MoA}. No systematic calibration of docking reliability for African antimalarial NPs exists, leaving a blind spot in the computational selection pipeline.
...
Workflows that combine a docking-derived retention metric, network pharmacology and short MM-GBSA endpoints are sometimes presented as joint evidence for resistance-resilient polypharmacology, but each layer measures a different quantity and has its own scope of inference. Treating them as independent evidence of mechanism, rather than as parallel ranking signals with non-overlapping failure modes, inflates the apparent epistemic reach of a single short MD pilot. We make this distinction explicit throughout.

The central question is whether the docking-MD divergence rate constitutes a reliable diagnostic signal for African antimalarial natural products. Specifically: (i) whether resistance-aware polypharmacology docking produces a discriminant prioritization that separates candidates by their predicted retention across clinically relevant mutant panels, and (ii) whether the directional divergence between docking-RRS and short-MD geometry metrics can be used as a diagnostic signal to identify candidates whose docking ranking is likely an artifact of static pose scoring. The first question addresses whether RRS-based stratification is informative enough to generate hypotheses for downstream experimental testing. The second addresses a critical gap in the literature: no study has systematically quantified the docking-MD divergence rate for African antimalarial NPs, despite evidence that scoring functions struggle with the structural complexity of natural products \citep{ramirez2018docking,ancajas2024np_sar}. The primary analysis used \num{12} candidates with eligible PfDHFR and PfCRT wild-type scores; five PfCRT-only candidates formed a coverage-limited sensitivity set. RRS was calculated from empirical Vina scores, whereas PNS and ACSI provided complementary ranking dimensions. Four parent-study wild-type complexes---chemically and numerically distinct from the 17 Set-C candidates---underwent \qty{10}{\nano\second} simulations as a methodological pose-retention check; a 16-system Set-C pilot provided the only trajectory-level comparison against docking-RRS for members of the prioritization cohort. Biological polypharmacology and resistance circumvention were not tested.
...
Seventeen candidates were selected from a library of \num{65856} molecules \citep{temgoua2026chemrxiv} by sequential filtering: MPO $\geq$ \num{0.70} (\num{19913} primary leads retained), SYBA $>$ \num{0} (synthesisable), SI $>$ \num{10} (selectively antiparasitic), multi-target engagement of at least two docking targets, and scaffold diversity $\leq$ three compounds per Bemis--Murcko scaffold. Class frequencies are conditional on these filters and should not be extrapolated to the full library. The selection includes one previously named consensus lead (Ligand 87, PfDHFR). Four additional parent-study consensus leads (Ligands 201, 214, 438, 164) were not part of the 17-candidate docking set but were used for the production-MD wild-type complexes; the 164 system is PfClpR (PDB 4GM2, not PfClpP). Five approved antimalarial drugs (artemisinin, pyrimethamine, chloroquine, lumefantrine, cipargamin) were reference controls (\cref{tab:system_breakdown}).
...
Requiring Class~A or A*, ACSI $>$ \num{0.5}, and above-mean PNS leaves PP-15 on the two-target panel; PP-15 was, however, included only as a feasibility probe outside the \num{17}-candidate Set-C panel (the PP-15 feasibility details are provided in the Supporting Information) and so is not a product of the Set-C prioritisation pipeline. That intersection is reported as a shortlist for experiment, not as evidence of activity or resistance resilience.
...
Two of four systems (PfCRT--214, PfATP4--438) retained their ligands, while the 164-labelled PfClpR and PfDHFR--201 ligands dissociated completely despite stable protein conformations (\cref{tab:md_stability}). Only PfCRT--214 yielded an interpretable MM-GBSA endpoint ($\Delta G_{\mathrm{bind}} = \qty{-18.25 \pm 0.40}{\kcalmol}$; \cref{tab:mmgbsa}). The dissociation of two docked poses under explicit solvent illustrates why static docking scores alone are insufficient to predict binding persistence---the same limitation that motivates the divergence analysis in the Set-C pilot.

\subsection{Docking-RRS and short-MD structural retention are non-equivalent quantities (central result)}
...
\subsection{External docking replication (sensitivity check)}
...
\subsection{The docking-RRS signal is reproducible but not a calibrated resistance predictor}

The docking-derived RRS stratifies the 17-candidate Set-C cohort into five classes (A*--D) that separate predicted wild-type potency from predicted mutant retention. Under the declared protocol, this signal is operationally reproducible (seed dispersion $\pm\qty{0.05}{\kcalmol}$, 77.9\% class stability under $\pm\qty{1}{\kcalmol}$ perturbation). However, the docking-derived triage is the baseline against which the diagnostic value of divergence is tested, not the paper's principal contribution. The classes are combinatorially defined and not evidence of resistance circumvention; they can be aligned with regional resistance surveillance in a future decision analysis, but they do not justify clinical deployment on their own \citep{wwarn_2026}.

In the current epidemiological context, where the 2025 World Malaria Report documents declining artemisinin partner-drug efficacy alongside the spread of \textit{Pfkelch13}-mediated partial resistance concentrated in the African Region \citep{who_malaria_2025}, resistance-aware prioritization is a useful design criterion---though computational scores alone cannot establish whether a given compound is matched to the resistance states relevant in a parasite population.

The external docking panel (mean RRS $\approx\num{100}$) underscores the same boundary: mutant and wild-type Vina scores remained nearly proportional on an independent ligand set. The DEKOIS 2.0 PfDHFR benchmark (AUC \num{0.45}, 95\% CI \numrange{0.37}{0.53}) confirms that this protocol does not recover known actives above chance. A GNINA cross-scoring analysis reproduced the class-level picture (all \num{38} eligible ligands class~A under both scoring layers; \cref{SM-tab:s15_robustness_transfer}), indicating the retention-not-gain pattern is not a single-scoring-function artefact, though per-mutant rank transfer was weak or null (e.g.\ C59R $-0.076$). These observations caution against treating RRS as a calibrated resistance predictor outside the primary cohort.

\subsection{The 87.5\% divergence rate is a diagnostic signal for African antimalarial NPs}

The central question asked whether docking-RRS and short-MD structural retention are interchangeable estimates of mutant binding for African antimalarial NPs. The answer is no: seven of eight matched mutant--wild-type comparisons showed directional disagreement. This non-equivalence is not a failure of either method but a consequence of what each estimates---docking-RRS reflects a static score ratio between mutant and wild-type poses, while MD-RRS captures dynamic binding persistence under explicit solvent and protein fluctuations. Failure-mode analyses of docking confirm that cross-target score comparisons under non-comparable states and over-reading scores as affinities are recurrent pitfalls~\citep{kittelson2026docking,ramirez2018docking}. The two quantities diverge when conformational selection or induced-fit effects alter the binding mode upon mutation, which is precisely the regime relevant to drug resistance.

The 87.5\% divergence rate is high, and while general docking benchmarks document pose-reliability limitations, they rarely quantify docking--MD directional agreement for any specific chemical class \citep{ramirez2018docking}. Whether the elevated rate reflects properties specific to African natural products---rigid polycyclic frameworks, macrocyclic systems, and rare functional groups \citep{moyo2023prioritised}---or a more general artefact of the short-MD protocol cannot be resolved within this study. No prior study has systematically quantified this divergence rate for African antimalarial NPs; the diagnostic signal it provides is the central finding of P2.

Short MD on the parent-study complexes shows both the value and the limit of structural follow-up when the compounds are not the ranked Set-C set. Two of four systems kept a bound ligand; the 164-labelled PfClpR and PfDHFR--201 systems did not. Only PfCRT--214 passed the checks needed for MM-GBSA, giving $\qty{-18.25 \pm 0.40}{\kcalmol}$---a system-specific endpoint that cannot calibrate Set-C scores \citep{wang2019_mmgbsa,gmx_mmpbsa_2021}. The Set-C pilot is the only trajectory comparison with docking-RRS for compounds on the prioritization list. Mutant MM-GBSA did not recover the docking-predicted weakening, so whether RRS tracks mutant-state free-energy differences remains unresolved. The directional mismatch observed in most mutant--wild-type pairs persists even where the class-level retention gate was satisfied (PP-01).

For antimalarial lead prioritization, this means that docking-based RRS serves as a first-pass computational filter but is not a validated prediction of resistance resilience. MD stress tests, even short ones, provide a complementary but non-redundant signal that probes a different aspect of the binding event. Computational prioritization and structural scrutiny should therefore be reported as parallel ranking dimensions with non-overlapping failure modes, not as convergent evidence of resistance resilience. The most direct validation would be matched IC$_{50}$ measurements on wild-type and mutant PfDHFR, PfCRT binding or transport assays at digestive-vacuole pH, and short growth-inhibition tests to determine whether multi-target computational signals translate to a measurable phenotype \citep{mlsypredc2024}.

\subsection{Chemical-space, network, and structural context}

ACSI places candidates relative to approved-drug and African natural-product chemical space. The mean was \num{0.543}, and two of \num{17} candidates (\qty{11.8}{\percent}) exceeded \num{0.70}. PNS is a network-weighted rank; on the two-target set it associated only weakly with RRS ($\rho=\num{-0.2098}$, adjusted $p=\num{1.0000}$). At $n=\num{12}$ the correlations mainly describe how the ranks line up in this cohort; the wide bootstrap intervals indicate that the three ranking dimensions capture distinct information. ACSI, PNS, and RRS are complementary ranking tools, not independent measures of biology, and their meaning depends on the chosen reference spaces, network, and targets \citep{anpdb_2026,value_addition_african_np_2025,Sentinel2025Offtarget}.

Topological associations reported in a companion TDA analysis \citep{temgoua2026quantum} used overlapping cohorts and are not independent validation; prospective testing in larger panels with experimental measurements is needed before topological descriptors can serve as design rules.
...
\subsection{Positioning within multi-target and resistance-aware approaches}

Polypharmacological drug design has gained traction to overcome resistance in infectious diseases~\citep{hegedus2025polypharmacology,rafehi2024medicinal}, including malaria~\citep{polypharmacology_malaria_2026,multi_target_antimalarials_2025}. Conventional multi-target docking ranks compounds by aggregated wild-type scores and does not treat mutant-state retention as a primary quantity. Network pharmacology often weights targets by centrality without comparing wild-type and resistance-associated structures under one scoring protocol~\citep{Sentinel2025Offtarget,abdelhamid2025polypharmacology_ai}. Under the SDMT frame, the present class-A* readout combines exactly the two axes such a strategy requires---target breadth in the four-target wild-type matrix and per-target mutational retention in the RRS panel---and is therefore reported as an SDMT-style hypothesis, not as demonstrated multi-target engagement~\citep{sdmt2026trends}. Free-energy methods such as FEP or TI report mutant binding differences with clearer physical content but remain costly for early triage. RRS is an explicit mutant/wild-type docking-score ratio that sits between those extremes, while ACSI, PNS, and short MD remain separate layers. None replaces IC$_{50}$ panels, transporter assays, or converged free-energy calculations; they are intended only to shortlist compounds for those experiments~\citep{temgoua2026quantum,multi_target_antimalarials_2025,ghosh2025pfcrt_pfdhfr}.

\subsection{Limitations}

Mutant receptors were homology models; all six met QMEAN, GMQE, Ramachandran, and RMSD thresholds, yet model quality is not experimental accuracy. MD was limited to 10~ns on four parent-study complexes and a single-replicate 16-system Set-C pilot; neither addresses long residence times or convergence. The JCIM MD-reporting guidelines recommend at least three replicates per system \citep{soares2023mdreport}; no new replicate campaign was launched, and the pilot is therefore a secondary structural stress test, not a thermodynamic or kinetic measurement. The 87.5\% divergence rate rests on eight matched comparisons from two candidates---the small denominator limits precision and prevents extrapolation to the full library. An independent PP-01 PfCRT wild-type run differed from the reference by \qty{2.01}{\kcalmol} (\cref{SM-tab:setc_wt_replicate_consistency}); single-replicate endpoints are sensitive to frame choice and periodic-boundary handling. MM-GBSA omits configurational entropy; only PfCRT--214 satisfied the bound-state and topology requirements. The four-target panel is not a full parasite targetome.

RRS class frequencies depend on the selection filters and unequal target coverage and should not be extrapolated to the full library. The cut-offs follow the Vina score distribution and have not been calibrated to biochemical resistance or clinical outcome (\cref{SM-tab:s11_rrs_threshold_sensitivity}). PfCRT network centrality was imputed (\cref{SM-tab:s7_pns_sensitivity}) and PNS is sensitive to database coverage and hub bias \citep{panossian2025pitfalls}. Force fields differed between cohorts, so absolute MM-GBSA values are not compared across protocols. Cross-metric correlations rest on $n=12$; the wide bootstrap intervals \textbf{indicate} that the three ranking dimensions capture distinct information. PfCRT was prepared at physiological pH rather than digestive-vacuole pH \citep{temgoua2026chemrxiv}. Five candidates lacked an eligible PfDHFR wild-type score and appear only in the PfCRT-only analysis.
...
This study asks whether docking-derived resistance-retention scores and short-MD structural retention are interchangeable estimates of mutant binding for African antimalarial natural products. The answer is no: seven of eight matched mutant--wild-type comparisons showed directional disagreement (87.5\% divergence rate). This elevated divergence rate suggests that African NPs---with their rigid polycyclic frameworks, macrocyclic systems, and rare functional groups---may be susceptible to static-pose artifacts, though whether the rate reflects properties specific to this chemical class or a more general artefact of the short-MD protocol cannot be resolved here. The divergence constitutes a diagnostic signal identifying candidates whose docking ranking requires experimental validation before further prioritization. Docking-RRS and MD-RRS should be reported as parallel ranking dimensions, not as convergent evidence of resistance resilience. The docking-derived five-class stratification is operationally reproducible under the declared protocol and serves as a hypothesis-generating prioritization. It does not constitute a validated triage procedure, nor does it demonstrate biological polypharmacology or experimental resistance circumvention. PP-01 \textbf{merits} experimental follow-up; PP-02 was not promoted by the pilot gate. Target-engagement assays, phenotypic tests, and prospective experiments in larger panels of African antimalarial NPs remain necessary before any biological claim can be advanced.
