...
\begin{table}[htbp]
\centering
\caption{Endpoint-analysis status for the four parent-study consensus complexes used for production MD. These systems are distinct from the 17 set-C polypharmacology candidates. Mean $\pm$ standard deviation is shown only for the single interpretable MM-GBSA result from 101 snapshots; dissociated or conversion-corrupted systems are reported as N/A. Mutant RRS is a docking-based analysis of set C and is not assigned to these MD systems.}
\label{tab:s3_mmgbsa_mutants}
\begin{tabular}{l l S[table-format=-2.2] S[table-format=1.2] l}
\toprule
Ligand & Target & {$\Delta G_{\mathrm{bind}}$} & {Std. dev.} & Status \\
\midrule
164 & Historical PfClpP cohort label$^\ast$-WT & {--} & {--} & N/A; ligand dissociated$^\ddagger$ \\
...
\bottomrule
\multicolumn{5}{l}{$^\ast$The 164--PfClpP label is historical; PDB 4GM2 is PfClpR rather than PfClpP and this system is not PfClpP structural validation.} \\
