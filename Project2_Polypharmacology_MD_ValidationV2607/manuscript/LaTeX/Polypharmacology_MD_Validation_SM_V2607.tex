...
\caption{Per-compound, docking-derived RRS profiles across six resistance mutations for the 17 Set-C candidates. Missing axes denote an ineligible target under the $\SI{5.0}{\kcalmol}$ wild-type docking-score threshold and are not treated as zero. Concentric rings indicate \qty{50}{\percent}, \qty{100}{\percent}, and \qty{150}{\percent} RRS; the plot does not encode absolute binding potency.}
\label{fig:s3_rrs_radar}
\end{figure}

\noindent\textit{Scope of the MD evidence.} The parent-study trajectories provide a targeted, non-membrane explicit-solvent check on four wild-type complexes; they do not constitute candidate-specific Set-C production MD or a physiologically complete model for the membrane targets. A 16-system Set-C MD pilot (PP-01 and PP-02 against the six-mutant panel, \SI{10}{\nano\second} each) passed the prespecified trajectory-QC gate and is reported as a secondary analysis; its trajectory-derived MD-RRS and mutant-state MM-GBSA estimates are summarised in \cref{tab:s8_setc_mmgbsa} and compared with docking RRS in \cref{fig:s4_setc_rrs_scatter}.
...
\begin{table}[htbp]
\centering
\caption{Set-C MD pilot endpoint estimates (secondary, exploratory analysis). All \num{16} systems passed the prespecified trajectory-QC gate; $\Delta G_{\mathrm{bind}}$ is an MM-GBSA endpoint estimate averaged over \num{100} snapshots with within-trajectory standard deviation, not a replicate uncertainty or a thermodynamic confidence interval. MMG-RRS is the ratio of absolute endpoint magnitudes relative to the corresponding WT system (WT = \num{100}); it is not a free-energy resilience measurement. n/a = no WT docking reference for PP-02 PfDHFR. The two systems flagged with $^\ast$ failed the first MM-GBSA pass (BOND/UB overflow from a protein split across the periodic boundary) and were re-run on PBC-whole trajectories.}\label{tab:s8_setc_mmgbsa}
\scriptsize
