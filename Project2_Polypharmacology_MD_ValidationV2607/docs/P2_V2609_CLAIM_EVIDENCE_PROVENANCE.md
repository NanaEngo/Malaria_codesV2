...
**Date:** 2026-09-10
**Scope:** Reconfigured P2 manuscript (estimand-calibration and computational-triage study)
...
| 3 | **SECONDARY** | Sensitivity, transfer, and cross-metric diagnostics | Independent biological replication |
| 4 | **TECHNICAL** | QC, hashes, manifests, force-field and topology checks | Scientific validity of the biological hypothesis |
| 5 | **NOT_COMPUTED** | Intentionally absent; must remain explicit | All claims requiring these quantities |
| 6 | **UNOBSERVED** | No canonical measurements available | All biological resistance conclusions |
...
### 3.3 SECONDARY — Sensitivity, transfer, and cross-metric diagnostics

| ID | Claim | Manuscript location | Evidence / provenance | Level | Boundary |
|---|---|---|---|---|---|
| C08 | PNS–RRS association is weak in the primary cohort (ρ = −0.2098, adjusted p = 1.0000, n=12) | Secondary Results | `results/cross_metric_statistical_audit.{csv,json}`; DAR §2–3 | SECONDARY | Descriptive; not evidence of independence or no relationship; post-selection |
| C09 | ACSI is a post-selection, cohort-dependent ranking descriptor | Secondary Results (SI formulas) | `results/c_acsi_scores.csv`; DAR §2 | SECONDARY | Not a causal biological measure; formulas moved to SI |
| C10 | GNINA CNN rescoring reproduces class-level retention on 38/38 eligible external ligands | Robustness/Discussion | GNINA consensus manifests; DAR §8quater.1 | SECONDARY | Same poses; not independent biological validation |
| C11 | External 39-ligand/312-state panel shows mean RRS ≈ 100.45; class-A fraction 0.974 | External docking | `external_vs_primary_bootstrap_20260828.json`; DAR §8 | SECONDARY | Computational transfer under different provenance; not experimental validation |
| C12 | DEKOIS 2.0 PfDHFR ROC-AUC is 0.45 [0.37, 0.53] | Discussion (protocol boundary) | DAR §3; P1 V8 SM | SECONDARY | Upstream companion context; protocol-transfer boundary; not P2 biological validation |

### 3.4 TECHNICAL — QC, endpoint diagnostics, and protocol records

| ID | Claim | Manuscript location | Evidence / provenance | Level | Boundary |
|---|---|---|---|---|---|
| C13 | 16/16 Set-C pilot trajectories pass geometric QC | Methods/Results (pilot) | `results/set_c_md/*` manifests; DAR §5 | TECHNICAL | Procedural reproducibility; not scientific validity |
| C14 | 16 finite MM-GBSA endpoint rows: 8/12 mutant ratios exceed 100% | Results (endpoint diagnostics) | `mmgbsa_summary_pilot.csv`; DAR §5.3 | TECHNICAL | Single-replicate endpoint ratios; not free-energy resilience; PP-01 PfCRT K76A inter-replicate Δ 7.75 kcal/mol demonstrates within-trajectory error ≠ replicate uncertainty |
| C15 | Parent-study MD: 4 complexes, 2 dissociated, 1 corrupted MM-GBSA, 1 interpretable endpoint | Discussion (protocol boundary) | parent-study manifests; DAR §4 | TECHNICAL | Structural check; not Set-C validation |
| C16 | STRING threshold sensitivity: PNS ranking robust at 400/700/900 (ρ = 0.9681–0.9975) | SI | `results/string_threshold_sensitivity_20260829/`; DAR §9 | TECHNICAL | Operational sensitivity; not biological network validation |

### 3.5 P1 BOUNDARY — Non-independence declarations

| ID | Claim | Manuscript location | Evidence / provenance | Level | Boundary |
|---|---|---|---|---|---|
| C17 | P2 Set-C is derived from P1; P2 is not an independent validation of P1 | Methods (estimand separation), Discussion | Crosswalk §2 above; DAR §0, §8 | PRIMARY | P2 may cite P1 as upstream context; P2 must not call any P1-derived result "independent P2 validation" |

### 3.6 NOT_COMPUTED — Intentionally absent quantities

| ID | Claim | Status | Required treatment |
|---|---|---|---|
| C18 | Full-panel MD-RRS (17 × 8 = 136 systems) | `NOT_COMPUTED` by design | Must remain explicit in all manuscript versions; cannot be promoted without a separate production contract |
| C19 | M1 replicated MD (12 trajectories × 100 ns) | `NOT_COMPUTED / NOT_REPORTABLE` | No completed trajectory set; partial artifacts are archival-only pending DAR reconciliation; no biological or convergence claim may be derived |
| C20 | Experimental PfDHFR/PfCRT WT-mutant activity (IC₅₀, SPR, transport) | `UNOBSERVED` | No canonical measurements; all biological resistance conclusions are outside P2 scope |

### 3.7 FORBIDDEN — Claims P2 must not make
...
- [ ] C08–C12 (SECONDARY) are traceable with explicit non-independence labels;
- [ ] C13–C16 (TECHNICAL) are documented with QC status;
- [ ] C17 (P1 boundary) crosswalk is complete and visible in Methods;
- [ ] C18–C20 (NOT_COMPUTED/UNOBSERVED) remain explicit and unviolated;
