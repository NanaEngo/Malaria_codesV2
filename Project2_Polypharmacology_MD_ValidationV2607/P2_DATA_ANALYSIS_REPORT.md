...
- **Completion (verified 16 Sept, `penavoraserver`, GROMACS 2025.4 CUDA):** tasks 0, 1, 4, 5, 6, 7, 8, 9 reached full 2 ns (`Finished mdrun`; 201 xtc frames × 10 ps; ~31–33 ns/day DHFR systems, ~43 ns/day CRT systems). Task 10 (`PP-02_PfDHFR_C59R`) running (started ~05:52 UTC, 1,000,000 steps / 2000 ps target); tasks 11–15 queued (throttle `%1`); relaunches 15851/15852 queued behind the array on the single GPU (`Priority`/`Resources`).
...
- **Thermostat/barostat sanity (completed runs, `gmx energy` averages):** T = 310.14–310.16 K; P = 1.07–1.73 bar (task 9 N51I: T = 310.15 K, P = 1.73 bar, potential −4.336×10⁶ kJ/mol); potentials stable within target family (DHFR ≈ −4.34×10⁶, CRT ≈ −3.28×10⁶ kJ/mol); conserved-energy drift negligible.
- **Backbone RMSD, fit = backbone, full 2 ns (mean / last-frame, nm):** PP-01 WT 0.172/0.211; N51I 0.154/0.173; I164L 0.186/0.200; CRT WT 0.181/0.215; K76T 0.171/0.197; K76A 0.210/0.238; PP-02 WT 0.158/0.192; PP-02 N51I 0.201/0.230 (n = 201 frames, min 0.001, max 0.306 nm). All < 0.25 nm — no unfolding. (S108N partial 0.137/0.151 nm on 0.4 ns recorded for completeness only, excluded from conclusions.)
- **Ligand RMSD after backbone fit (MOL0, 40 atoms; mean / last-frame, nm):** PP-01 WT 0.246/0.201; N51I 0.577/0.647; I164L 0.363/0.362; CRT WT 0.666/0.646; K76T 0.345/0.505; K76A 0.387/0.703; PP-02 WT 0.380/0.395; PP-02 N51I 0.293/0.217 (min 0.000, max 0.530 nm — stably bound). Elevated drift (N51I, CRT WT means > 0.5 nm; K76A last frame 0.703 nm) is an observation on 2 ns exploratory runs, NOT a binding, resistance, or pose-loss conclusion; no claim promoted.
- **Artifacts (HPC, per completed run dir `runs/short_15840_<task>/`):** `rmsd_backbone.xvg`, `rmsd_ligand.xvg`, `rmsd_idx.ndx` (MOL0 = group 13; task 9: xvg pair only, no ndx — same numeric groups 4/13, default index).
