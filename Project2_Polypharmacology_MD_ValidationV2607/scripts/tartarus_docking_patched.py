...
def get_score(smi, docking_target='1syh', verbose=False, scratch='/tmp'): 
    ...
    try: 
        ...
        # Limit smina CPU to avoid over-subscription (each worker spawns its own smina)
        # Use SMINA_CPU env var or default to 2 CPUs per job
        cpu_count = int(os.environ.get('SMINA_CPU', 2))
