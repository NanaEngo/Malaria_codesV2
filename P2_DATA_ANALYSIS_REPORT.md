...
## 8. Manuscript status (19 Aug)

- **Manuscript**: `manuscript/LaTeX/Polypharmacology_MD_Validation_V2607.tex` (main) + `_SM_V2607.tex` (SI). Target: JCIM (ACS).
- 19 Aug revision: Set-C MD pilot integrated as an explicitly secondary analysis with discriminative MD-RRS and 16-system MM-GBSA; the current rigor pass separates target-balanced and coverage-sensitive RRS estimands, corrects the weakest-WT potency discriminator, and adds permutation/bootstrap uncertainty and PNS-imputation sensitivity.
- All numbers in text/tables trace to JSON/CSV data files (manifests listed above).
- Unit tests: `tests/` — 18/18 PASS, including MD manifests, MM-GBSA aggregation, RRS class definitions, target coverage, and regenerated statistical outputs.
...
- [x] Unit tests: `tests/` — 18/18 PASS (incl. MM-GBSA aggregation + PBC-fix tests); rigorous audit tests cover class definitions, target coverage, and regenerated outputs.
