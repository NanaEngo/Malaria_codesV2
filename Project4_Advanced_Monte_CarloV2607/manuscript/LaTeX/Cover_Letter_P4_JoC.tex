...
A four-method, 20-seed benchmark under a fixed search budget and a public-activity-augmented reward yields a transparent ranking: random search achieves the highest mean scalar reward (0.6724), followed by MCTS at its screened optimal configuration (0.6649; mean gap 0.0075, paired $t_{19}=-4.97$, $p=0.0001$), a genetic algorithm (0.6453) and deterministic greedy search (0.4278). MCTS is therefore not a single-objective optimiser and, under the multi-objective protocol, returns a Pareto front of non-dominated candidates (four solutions, hypervolume 1.2366) spanning the potency--accessibility trade-off, including a high-potency / low-accessibility extreme that scalar optimisation misses. Component ablations identify the chemistry-informed policy and the Pareto front itself as the dominant performance drivers.
