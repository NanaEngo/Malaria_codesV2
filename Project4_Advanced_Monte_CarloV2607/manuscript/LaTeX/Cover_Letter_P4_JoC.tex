...
A four-method, 20-seed benchmark under a fixed oracle budget yields a deliberately honest ranking: random search achieves the highest mean scalar reward (0.734), followed closely by MCTS at its screened optimal configuration (0.728; mean gap 0.006), a genetic algorithm (0.703) and deterministic greedy search (0.615). MCTS is therefore competitive on single-objective performance, and is the only method that systematically returns a multi-objective Pareto front of non-dominated candidates (four solutions, hypervolume 1.237) spanning the potency--accessibility trade-off, including concave regions that scalar optimisation misses. Component ablations identify the chemistry-informed policy and the Pareto front itself as the dominant performance drivers.
