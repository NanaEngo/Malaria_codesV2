...
All data supporting the conclusions will be deposited on Zenodo and mirrored in the project repository under the MIT licence upon acceptance. The deposit will include: (i)~the canonical 20-seed four-method benchmark tables and best-in-seed SMILES (\texttt{results/benchmark\_molecules\_opt/}); (ii)~per-seed and merged Pareto fronts with the post-hoc SYBA recomputation log (\texttt{results/pareto/}); (iii)~diversity metrics and MDS coordinates (\texttt{results/diversity/}); (iv)~component and vocabulary ablation summaries; and (v)~analysis and provenance-check scripts. Reviewers may request access to the private repository prior to public release.
