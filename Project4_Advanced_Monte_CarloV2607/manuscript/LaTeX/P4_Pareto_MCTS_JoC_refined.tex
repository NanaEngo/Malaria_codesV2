...
\begin{itemize}
\item Pareto-guided MCTS is transparent objective-space exploration, not scalar superiority: below random on scalar reward (mean $\Delta = -\num{0.0075}$, $p = \num{0.000085}$)
\item The post-hoc front (hypervolume \num{1.2366}) spans the potency--accessibility trade-off, including a high-MPO/low-SYBA extreme discarded by fixed-weight aggregation
\item RRS- and PNS-informed chemotype/docking proxies are integrated as design dimensions; their biological validation remains future work
\item A public-activity proximity term enables a fair scalar comparison with random, greedy and genetic baselines across \num{20} seeds
\end{itemize}
...
A second limitation is unguided exploration: many methods expand molecular graphs without chemical priors, spending budget on inaccessible regions of chemical space. Chemistry-informed priors improve sample efficiency~\citep{Jensen2019,Olivecrona2017}. For antimalarial leads in particular, the objectives that matter---potency, synthetic accessibility, resistance-informed chemotype similarity and docking-derived proxy values---are genuinely conflicting. The P4 RRS-informed proxy encodes Morgan/Tanimoto similarity to P2 reference chemotypes used as resistance-resilience exemplars, whereas the PNS-informed proxy aggregates normalized docking scores from the three detected Tartarus columns (\texttt{score\_1syh}, \texttt{score\_6y2f}, \texttt{score\_4lde}). Neither should be read as a direct WT/mutant binding panel or as a four-target PfDHFR/PfCRT/PfATP4/PfClpR validation. We are not aware of a multi-objective generation study that exposes these resistance-related and docking-derived proxy signals inside the search objective rather than collapsing them into a post-hoc ranking.

The concrete design problem motivating this work is the following. A medicinal chemist generating antimalarial candidates must simultaneously consider: (i)~potency against the target (here proxied by multi-parameter optimisation, MPO); (ii)~synthetic accessibility (SYBA), because a potent compound that cannot be synthesised is useless; (iii)~resistance resilience (RRS), because the candidate must retain activity against mutant parasites; and (iv)~polypharmacological engagement (PNS), because single-target agents are vulnerable to resistance emergence. These objectives are genuinely conflicting: the most potent candidates may be synthetically intractable, and the most accessible may lack resistance coverage. A scalar reward that averages these dimensions with fixed weights conceals this trade-off and can silently discard candidates that are Pareto-optimal on a subset of objectives. This paper asks whether Pareto-guided tree search can expose these trade-offs while maintaining competitive scalar performance.
...
The present work differs from prior Pareto-MCTS studies in three respects. First, we integrate domain-specific resistance-related objectives (RRS-informed and PNS-informed proxies) directly into the search, rather than using only generic property objectives (potency, drug-likeness, accessibility). Second, we pair the Pareto analysis with a rigorous scalar benchmark against three baselines (random, greedy, genetic) across 20 seeds, establishing the cost of multi-objective exploration in scalar terms. Third, we apply the framework to a specific antimalarial design programme with a curated fragment vocabulary, making the results reproducible within a constrained but chemically meaningful search space.

We address both limitations with a Pareto-guided MCTS agent that:
\begin{enumerate}[label=\textbf{C\arabic*:},leftmargin=*]
  \item maintains a non-dominated candidate pool using the informative pre-activity signals and re-derives the reported front after an explicit SYBA recomputation, so the provenance of resistance-informed chemotype similarity, docking-derived proxies and accessibility is inspectable rather than hidden in a post-hoc ranking;
...
The study is designed around a deliberate distinction. The scalar benchmark asks which method obtains the largest single reward under a fixed budget; the pre-activity analysis asks how the discovered candidates are distributed when chemically and domain-informed signals are reconstructed as a vector. Random is the strongest scalar baseline, whereas MCTS is evaluated for objective-space coverage. To make that comparison inspectable, \Cref{sec:front_comparison} re-scores the recorded best molecules from all methods with the common pre-activity oracle. This separation prevents a scalar ranking from being misread as a test of multi-objective coverage.
...
We next analysed the MCTS outputs under a pre-activity vector schema comprising MPO, SYBA, the RRS-informed chemotype proxy and the PNS-informed Tartarus docking proxy. The per-seed search records show that SYBA was constant at zero; the reported front is consequently a transparent post-hoc SYBA re-derivation from the discovered candidates, not evidence that the search optimised an informative SYBA signal. Across \num{20} independent seeds, the merged front contains four mutually non-dominated solutions with hypervolume \num{1.2366} (\Cref{fig:pareto}, \Cref{tab:pareto}).
...
\begin{table}[htbp]
  \centering
  \caption{Canonical \textbf{pre-activity} merged Pareto front from \num{20} independent Pareto-MCTS seeds. All four points are mutually non-dominated in the post-hoc re-derived schema MPO, recomputed SYBA, RRS and PNS (SA is constant at \num{3.0} and excluded). SYBA was recomputed with the lich/conda classifier because the original run returned a constant fallback of zero; this table is not evidence of informative four-way optimisation during the search. SMILES and score tables are provided with the source data.}
  \label{tab:pareto}
  \begin{tabular}{lccccc}
    \toprule
...
This ranking is consistent with recent observations that simple baselines remain competitive in constrained fragment spaces~\citep{Jensen2019} and that the primary advantage of multi-objective MCTS is coverage of the non-dominated surface~\citep{Mothra2024}. For practitioners, three recommendations follow: \emph{First}, if the goal is to maximise a single scalar reward under a fixed budget, random search remains the strongest baseline in constrained fragment spaces; more sophisticated methods should be benchmarked against it before claims of superiority are made. \emph{Second}, if the goal is to explore trade-offs between conflicting objectives (e.g., potency vs accessibility vs resistance coverage), Pareto-guided MCTS provides transparent, auditable candidate provenance that scalar aggregation cannot; the Pareto front preserves candidates that would be silently discarded by any fixed weighting, including high-potency/low-accessibility extremes that may be chemically valuable. \emph{Third}, the global best-molecule tracking fix is essential: without it, MCTS reward collapses by a factor of approximately \num{2.6}, and all prior benchmark versions that lack this fix should be re-evaluated.

\subsection{Why greedy collapses under activity-augmented reward}

The most striking result in \Cref{tab:benchmark} is the deterministic greedy baseline scoring \num{0.4278}---\num{0.2446} below random search, with zero variance across all 20 seeds. This extreme is not a random artefact: greedy deterministically converges to the same local optimum regardless of initial conditions. The mechanism is straightforward. Greedy always appends the fragment that maximises the immediate oracle reward at each step. Under the activity-augmented reward (v12), the activity term penalises distance from known actives; greedy therefore converges to a single, locally optimal molecule that is structurally similar to known actives but scores poorly on the other reward components (MPO, docking). Once stuck in this local optimum, greedy cannot escape because it has no stochastic exploration mechanism. The zero standard deviation confirms that the reward landscape has a single dominant attractor for deterministic hill-climbing within this fragment vocabulary. This result has a practical implication: in multi-objective molecular design, deterministic optimisers are unreliable even when they appear principled, because the reward landscape is rugged and the global optimum is not accessible by local moves alone.
...
Several limitations must be stated explicitly. First, the fragment vocabulary (approximately \num{108} fragments in the full set; medium subset used for the main benchmark) constrains accessible chemical space; results should not be extrapolated to open-ended SMILES generation. Second, molecular-diversity analysis is reported in the Supplementary Information in \Cref{SM-fig:sm_mds,SM-tab:sm_diversity}, where it is computedfrom the recorded best-in-seed moleculesets of the canonical benchmark; the main text emphasises the scalar ranking and the Pareto front, both reproducible from the reported source data. In particular, greedy search is deterministic here---it returns the identical molecule across all \num{20} seeds---so its recorded set is a single point rather than a diverse panel. Third, SYBA scores on the Pareto front were recomputed post-hoc after the original search returned a constant zero fallback; the recomputation, the resulting front and the verification scripts are documented with the source data. This means the four-point Pareto front is a post-hoc reconstruction, not evidence that the historical search simultaneously optimised four informative objectives. Fourth, all scoring oracles---MPO, RRS-informed chemotype similarity, PNS-informed Tartarus docking proxy, and public-activity proximity---are computational proxies, not experimental measurements. The RRS-informed proxy encodes Morgan/Tanimoto similarity to reference chemotypes and does not propagate per-target WT/mutant binding ratios; the PNS-informed proxy averages three detected Tartarus docking columns and is not a direct PfDHFR/PfCRT/PfATP4/PfClpR polypharmacology panel; the activity oracle scores maximum Tanimoto similarity to publicly known antimalarial actives, not predicted IC\textsubscript{50}. To assess this limitation, we trained a random forest classifier on the P5 panel (\num{19836} molecules, ECFP4 features) to predict experimental activity; the oracle achieved five-fold cross-validation AUC of \num{0.9479} $\pm$ \num{0.0040}. Applied to the \num{20} P4 best-in-seed molecules, the RF assigned a mean activity probability of \num{0.676} $\pm$ \num{0.135}, with \num{18} of \num{20} predicted active (threshold \num{0.5}). The mean maximum Tanimoto to P5 actives was \num{0.242} $\pm$ \num{0.052}, with Pearson correlation to RF probability of $r = \num{0.803}$. This confirms that the Tanimoto-max proxy captures activity-relevant signal but systematically underestimates RF-based activity probability, and that \num{18} of the \num{20} P4 molecules are classified as active by an orthogonal model trained on the same panel. Prospective experimental validation is required before any biological claims can be made. In particular, the present fragment space produces a narrow RRS range (\num{0.141}--\num{0.223}) and predominantly discrete PNS values (\num{0.0} or \num{1.0}), so the study demonstrates objective integration and candidate-selection provenance more strongly than it demonstrates broad phenotypic separation. Finally, MCTS cross-seed variability is modest in the canonical scalar benchmark ($\mathrm{SD}=\num{0.0068}$), but this does not establish robustness outside the curated fragment vocabulary or the fixed budget. The per-seed outputs make that uncertainty inspectable.

\subsection{Implications for antimalarial design}

A distinctive feature of the present setup is that domain-informed proxy signals are recorded alongside candidate retention rather than appearing only in an opaque final ranking. Whereas prior Pareto-MCTS generators commonly optimise binding, drug-likeness, toxicity or dual-target objectives~\citep{Mothra2024,ParetoDrug2024,CombiMOTS2026}, this implementation adds an RRS-informed chemotype-similarity proxy and a PNS-informed Tartarus docking proxy alongside potency and accessibility. The search exposes the resistance-related and docking-derived proxy signals during generation, while SYBA is explicitly limited to post-hoc re-evaluation because its search-time fallback was constant. The current data also define the limit of that claim: the RRS-informed proxy spans only \num{0.141}--\num{0.223}, and PNS equals \num{1.0} for three of four front points. The present study consequently demonstrates auditable objective integration and Pareto retention, not broad biological discrimination or direct resistance validation.
