...
\begin{abstract}
\noindent
Scalar rewards can conceal trade-offs in de novo molecular design. We present a Pareto-guided Monte Carlo tree search (MCTS) framework that maintains a pre-activity non-dominated front across multi-parameter optimisation (MPO), synthetic Bayesian accessibility (SYBA), an RRS-informed chemotype-similarity proxy and a PNS-informed Tartarus multi-target docking proxy, using a scaffold-aware fragment policy informed by ChEMBL27 frequencies and Tanimoto compatibility. In a four-method benchmark with \num{20} independent seeds and a public-activity proximity term, random search had the highest mean reward (\num{0.6724} $\pm$ \num{0.0056}), followed by MCTS (\num{0.6649} $\pm$ \num{0.0068}), the genetic algorithm (\num{0.6453} $\pm$ \num{0.0124}) and greedy search (\num{0.4278} $\pm$ \num{0.0000}). MCTS was below random (paired $t_{19}=\num{-4.97}$, $p=\num{0.000085}$, mean $\Delta=\num{-0.0075}$, 95\% CI $[\num{-0.0107},\,\num{-0.0043}]$) but above the genetic algorithm ($t_{19}=\num{6.95}$, $p<\num{0.0001}$). Separately, the pre-activity MCTS search returned four mutually non-dominated solutions (hypervolume \num{1.2366}) spanning a potency--accessibility trade-off, including a high-potency/low-accessibility extreme discarded by fixed-weight scalar aggregation. Component ablations identified the scaffold-aware policy and Pareto front among the largest observed main effects. The results support MCTS as a transparent multi-objective explorer, not as a universally superior scalar optimiser.
\end{abstract}

\textbf{Scientific Contribution:} We introduce a Pareto-guided MCTS framework for de novo antimalarial design that maintains a non-dominated \textbf{pre-activity} front across potency, synthetic accessibility, an RRS-informed chemotype proxy and a PNS-informed multi-target docking proxy, guided by a scaffold-aware fragment policy; a public-activity proximity term is evaluated separately in the v12 scalar benchmark. Against a four-method benchmark across \num{20} seeds under a public-activity-augmented reward, MCTS is below random search on scalar reward (mean $\Delta = -\num{0.0075}$, $p = \num{0.000085}$; 95\% CI $[\num{-0.0107},\,\num{-0.0043}]$) but returns a multi-objective Pareto front spanning the potency--accessibility trade-off; the honest headline is that the framework's value lies in transparent multi-objective exploration, not scalar superiority. Component ablations identify the scaffold-aware policy and the Pareto front among the largest observed main effects in that ablation configuration.
...
The central empirical finding is intentionally transparent: within a curated fragment vocabulary, random search achieves the highest mean scalar reward under both the baseline and the public-activity-augmented reward. MCTS trails by a modest margin on that metric, while its separate multi-objective protocol returns a Pareto front of non-dominated candidates. Because the original scalar baseline runs were not optimised for the full vector objective, \Cref{sec:front_comparison} re-scores their deposited best molecules with the common multi-objective oracle and compares the resulting fronts. This separation of scalar performance from objective-space coverage, rather than any claim of scalar superiority, is the foundation of the present study.
...
These results establish a clear baseline hierarchy on scalar reward. They do not, however, capture multi-objective coverage. The paired-seed view in \Cref{fig:overview} makes the direction and uncertainty of the MCTS--random comparison explicit rather than reducing the comparison to bar heights.
...
\begin{figure}[htbp]
  ...
  \caption{Integrated evidence overview generated directly from the deposited canonical data. (A) Paired per-seed differences between MCTS and random search under the v12 public-activity-augmented scalar reward; the horizontal band is the 95\% confidence interval for the mean difference. (B) The locked pre-activity Pareto front, with RRS encoded by colour and PNS by marker shape; SA is constant. (C) Observed high-minus-low main effects from the factorial ablation; these relative effects use a different oracle configuration and are not comparable to the main benchmark's absolute rewards.}
  \label{fig:overview}
\end{figure}
...
A Pareto-guided MCTS agent equipped with a scaffold-aware fragment policy trails random search by a modest margin on the public-activity-augmented scalar reward (mean $\Delta = \num{-0.0075}$, $p = \num{0.000085}$, 95\% CI $[\num{-0.0107},\,\num{-0.0043}]$ at the screened optimal configuration) and, under the separate multi-objective protocol, returns a front of non-dominated antimalarial candidates. The chemistry-informed policy and Pareto front show among the largest observed main effects in the factorial ablation, whose oracle configuration differs from v12. Within the studied fragment space, random search remains a strong scalar baseline; the added value of MCTS is transparent exploration of potency--accessibility trade-offs while retaining RRS and PNS as explicit domain objectives that scalar aggregation can conceal. The code and currently released data needed for reproduction are available in the public repository; the reserved Zenodo deposit remains pending.
