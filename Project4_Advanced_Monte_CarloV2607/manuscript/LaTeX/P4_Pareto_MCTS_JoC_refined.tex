...
\begin{abstract}
\noindent
De novo molecular generation often collapses multiple design objectives into a single scalar reward, obscuring trade-offs between potency, synthetic accessibility and drug-likeness. We present a Pareto-guided Monte Carlo tree search (MCTS) framework that maintains a non-dominated front across multi-parameter optimisation (MPO), synthetic Bayesian accessibility (SYBA), resistance-resilience (RRS) and polypharmacology-network (PNS) scores, guided by a scaffold-aware fragment policy derived from ChEMBL27 frequencies and Tanimoto compatibility. A four-method scalar benchmark across \num{20} independent seeds (fixed search budget, reward augmented with a public-activity proximity term; see \Cref{sec:oracles}) shows that random search achieves the highest mean reward (\num{0.6724}), followed by MCTS (\num{0.6649}), a genetic algorithm (\num{0.6453}) and deterministic greedy search (\num{0.4278}); the MCTS--random gap is modest but significant (mean $\Delta = \num{-0.0075}$, paired $t_{19} = \num{-4.97}$, $p = \num{0.0001}$, Cohen's $d=\num{-1.11}$), while MCTS remains far above greedy ($\Delta = +\num{0.237}$) and significantly above the genetic algorithm ($\Delta = +\num{0.020}$, $p<\num{0.0001}$). MCTS is therefore not a scalar-reward optimiser and, within our fragment-vocabulary oracle design, is the method that returns a \textbf{pre-activity} multi-objective Pareto front of non-dominated candidates (four solutions, hypervolume \num{1.2366}) spanning the potency--accessibility trade-off, including the high-potency / low-accessibility extreme that fixed-weight scalar aggregation discards. To our knowledge this is the first Pareto-guided MCTS to fold resistance-resilience and polypharmacology-network scores into the generation objective, in contrast to generic affinity/drug-likeness metrics in prior Pareto-MCTS generators, with the honest caveat that in the present front these domain scores remain comparatively homogeneous. Component ablations identify the scaffold-aware policy ($\Delta = +\num{0.148}$) and the Pareto front itself ($\Delta = +\num{0.108}$) as the dominant drivers of performance. All code, molecule sets and score tables are available in the public GitHub repository under an open licence; the Zenodo DOI is reserved and the upload remains pending. These results position MCTS as a complementary multi-objective explorer rather than a universal single-objective optimiser.
\end{abstract}

\textbf{Scientific Contribution:} We introduce a Pareto-guided MCTS framework for de novo antimalarial design that maintains a non-dominated \textbf{pre-activity} front across potency, synthetic accessibility, resistance-resilience and polypharmacology-network scores, guided by a scaffold-aware fragment policy; a public-activity proximity term is evaluated separately in the v12 scalar benchmark. Against a four-method benchmark across \num{20} seeds under a public-activity-augmented reward, MCTS is significantly below random search on scalar reward (mean $\Delta = -\num{0.0075}$, $p = \num{0.0001}$) but uniquely recovers a multi-objective Pareto front spanning the potency--accessibility trade-off; the honest headline is that the framework's value lies in transparent multi-objective exploration, not scalar superiority. Component ablations identify the scaffold-aware policy and the Pareto front itself as the dominant drivers of performance.
...
The central empirical finding is intentionally transparent: within a curated fragment vocabulary, random search achieves the highest mean scalar reward under both the baseline and the public-activity-augmented reward. MCTS trails by a modest but significant margin on that metric and is the method that, under the multi-objective protocol, returns a Pareto front of non-dominated candidates; the scalar baselines (random, greedy, genetic) were evaluated on scalar reward only. \Cref{sec:front_comparison} re-scores those baselines on the full multi-objective oracle and compares fronts directly; this ranking, rather than a claim of scalar superiority, is the foundation of the present study.
...
When the same MCTS agent maintains a non-dominated \textbf{pre-activity} front across MPO, SYBA, RRS and PNS, the merged front obtained from \num{20} independent seeds contains four mutually non-dominated solutions with hypervolume \num{1.2366} (\Cref{fig:pareto}, \Cref{tab:pareto}).
...
Within the curated medium fragment vocabulary and a fixed search budget, random search is a strong baseline and attains the highest mean scalar reward under both the baseline and the public-activity-augmented reward, as well as the single best seed. With its screened optimal configuration, MCTS trails by a modest but significant margin on scalar reward (mean $\Delta = \num{-0.0075}$, $p = \num{0.0001}$) and, under the multi-objective protocol, maintains and returns a Pareto front of non-dominated candidates. The practical value of the framework therefore lies not in peak single-objective performance but in the transparent exploration of trade-offs that scalar aggregation conceals.
...
The agent maintains a global non-dominated \textbf{pre-activity} front over the objective vector
\[
...
Each molecule is scored by an aggregator returning MPO, SYBA, SA, docking-derived terms, resistance-resilience score (RRS), polypharmacology-network score (PNS) and the public-activity proximity (activity). Default scalar-reward weights are $w_{\text{MPO}}=\num{0.27}$, $w_{\text{docking}}=\num{0.225}$, $w_{\text{SYBA}}=\num{0.135}$, $w_{\text{SA}}=\num{0.045}$, $w_{\text{RRS}}=\num{0.135}$, $w_{\text{PNS}}=\num{0.09}$, $w_{\text{activity}}=\num{0.10}$. The activity oracle scores a molecule by its maximum Morgan-2 Tanimoto to the \num{19321} experimentally active antimalarial compounds of the independent public ChEMBL IC50/EC50 dataset (\num{22447} molecules), scaled continuously (Tanimoto $\le \num{0.20}$ gives a soft gradient; $\num{0.20}$--$\num{1.0}$ maps linearly). The four-method scalar benchmark uses the reduced reward \texttt{compute\_mpo\_reward} with weights $w_{\text{MPO}}=\num{0.36}$, $w_{\text{docking}}=\num{0.315}$, $w_{\text{SYBA}}=\num{0.135}$, $w_{\text{SA}}=\num{0.09}$, $w_{\text{activity}}=\num{0.10}$ and the RRS/PNS oracles disabled (they require external P2 mutant and network data); the full seven-component aggregator is used for scalar/oracle evaluations, whereas the canonical Pareto-MCTS front reported in \Cref{tab:pareto} was generated under the pre-activity objective set (MPO, SYBA, RRS, PNS); the public-activity term enters the scalar benchmark reward only and is excluded from the front's dominance check and hypervolume, so the front values are unchanged by the activity-augmented benchmark. MPO and docking scores are drawn from precomputed libraries where available; QED serves as an MPO fallback for novel structures. SYBA on the reported Pareto front was recomputed with the lich/conda classifier after the original run returned a constant zero fallback. The accessibility column reported in \Cref{tab:pareto} is the linear transform $\mathrm{SA}^{-1} = (10 - \mathrm{SA})/9$ of the raw Ertl score, so that the shared raw value $\mathrm{SA} = \num{3.0}$ corresponds to $\mathrm{SA}^{-1} = \num{0.778}$; the deposited CSV stores the raw value.
...
A Pareto-guided MCTS agent equipped with a scaffold-aware fragment policy trails random search by a modest but significant margin on the public-activity-augmented scalar reward (mean $\Delta = \num{-0.0075}$, $p = \num{0.0001}$ at the screened optimal configuration) and, under the multi-objective protocol, returns a front of non-dominated antimalarial candidates. The dominant performance drivers are the chemistry-informed policy and the Pareto front itself. Within the studied fragment space, random search remains a strong baseline; the added value of MCTS lies in transparent exploration of potency--accessibility trade-offs that scalar aggregation conceals. All currently released artefacts required for reproduction are available in the public repository under an open licence; the Zenodo upload remains pending.
