...
**Generated:** July 9, 2026 — **Updated July 31, 2026** (v45: **P4 QMC Tier 2 (VMC/DMC) deep-diagnostic completed on HPC** — installed `pyqmc` 0.8.1 (CPU numba) into `malaria_md`; evidence chain: (1) Slater-only VMC on H₂O/PBE/cc-pvdz = −75.09 exactly reproduces the HF-expectation of the PBE orbitals (numba Slater correct; basis kernels FD-verified to 1e-5); (2) canonical LiH docs example gives VMC −7.39 vs RHF −7.78 — a trial WORSE than the bare Slater determinant (impossible for a valid Jastrow) ⇒ **JastrowSpin combination/summation path broken in this env**; (3) `ion_cusp=False` restores valid energies on SMALL systems only (H₂O VMC −76.53, DMC −76.50 vs SCF −76.33, FCI −76.44; 5-block, nconfig 400); (4) on the 144-electron candidates DMC population-collapses (cand_0: DMC −1021 vs SCF −864 = −157 Eh below SCF, impossible; walkers killed 15→92 of 100; mean walker position 0.25 Bohr; VMC −834.69 is the expected XC-free Slater expectation, +29.5 Eh above PBE). **Conclusion: Tier 2 candidate-level energies are NOT publication-grade in this env.** Garbage CSV row (cand_0 DMC −1021, corr −4267 eV) deleted; runtime sanity guard added to `p4_qmc_pipeline.py` (blanks DMC columns when non-finite or >20 Eh below SCF — cannot reach the manuscript). Production protocol would require a working Jastrow (patched PyQMC or QMCPACK), Jastrow optimisation (`pyqmc.recipes.OPTIMIZE`), nconfig ≥ 1000, and τ→0 extrapolation. 12 diagnostic scripts marked `DIAGNOSTIC — not production` (bisect, jastrow_test, cusp_fix [HYPOTHESIS REFUTED], kernel_check, scale_test, fd_wf [INCONCLUSIVE harness], dump_hdf5, jax_cart, lih_docs, isolate_bug, jax_test, pyqmc_sanity_test). Scale test (Slater-only vs Jastrow DMC, nconfig=300, cand_0) COMPLETED on HPC — **collapse confirmed on both branches**: Slater-only DMC −897.4/−900.7 Eh (±17–18 Eh, run-to-run unstable, ~33 Eh below SCF −864.165 ⇒ unphysical) and Jastrow DMC (ion_cusp=False) −867.35/−867.59 Eh (±1.8–2.0 Eh, stable across runs but still ~3.2 Eh below SCF — no e-N cusp at 144-e scale, not publication-grade).; v44: P4 QMC Tier 1 GPU-accelerated via `gpu4pyscf` 1.8.0 (CUDA 12x) installed into `malaria_md` (RTX A4000, cupy 14.1.1/CUDA 12.9, pyscf 2.14.0); `p4_qmc_array.sbatch` gained `--gres=gpu:1`; job 12686 array completed 4/4 on GPU — gpu4pyscf RKS branch (`Using GPU-accelerated PySCF`), SCF converged (cand 0 −864.164252, 1 −1258.406162, 2 −1012.418277, 3 −613.949885 Eh) + `*_pbe.molden` saved, zero errors; candidate-0 wall ~56 s vs ~131 s CPU (~2.3× end-to-end; PySCF portion far faster, xtb CPU-bound). **Tier 2 (VMC/DMC) still a stub — PyQMC/QMCPACK not installed.**; v43: P4 QMC Tier 1 pipeline restored on HPC — installed `pyscf` 2.14.0 (pip) + `xtb` (conda-forge) into `malaria_md`; fixed `p4_qmc_array.sbatch` (array 0-4→0-3 for 4 Pareto candidates, absolute CONDA_BASE/PYTHON paths fixing batch exit-127 "conda: command not found", script-dir path fixing exit-2) and `p4_qmc_pipeline.py` (`mol.ecp="def2-ecp"`→None — invalid PySCF ECP name, candidates are Z≤16; `pyscf.molden`→`pyscf.tools.molden` import; prints/docstring de-ECP'd; array help 0-4→0-3). Job 12682 array (0-3) ran clean: candidates 0/1/3 completed PBE/def2-SVP SCF (−864.165/−1258.406/−613.950 Eh) + `*_pbe.molden` trial wavefunctions (all 4 candidates; task 2 finished 16:17:47). **Tier 2 (VMC/DMC) remains a stub — requires PyQMC/QMCPACK (not installed); this restores Tier 1 only.** ChEMBL IC₅₀ validation (Action 1) confirmed already-completed (July 25) — no rerun needed; v42: `Project3_Quantum_Inspired_RepresentationsV2607_V2/` (HPC-only) is **Dr Tchapet Njafa's working version of the P3 project — intentionally kept, do not delete** (decision July 31, 2026) — ignore in grep audits; v41: `Project3_Quantum_Inspired_RepresentationsV2607_V2/` (HPC-only) is an obsolete duplicate of the canonical P3 project, **intentionally kept** (decision July 31, 2026) — ignore in grep audits; v40: top-level stale audit/strategy docs deleted locally + HPC — `GEMINI.md` (empty), `PROJECT3_BENCHMARK_AUDIT.md` (superseded), root `P3_Stategic_85PA.md` + `P3_ADVERSARIAL_AUDIT_MITIGATION.md` (duplicates; P3-dir copies canonical), root `Cover_Letter_P3.*` (duplicates; P3 manuscript copy canonical), `Nouvel audit adversériel.md` (superseded by consolidated Jul 25 audit), `BMAD_Q1_Analysis_v2.md` (pre-canonical); v39: obsolete BMAD docs deleted locally + HPC — `BMAD_STEP1_REPORT.md` (completed step-1 report), `BMAD_LOCAL_IMPL_PLAN.md` (stale local plan), `update_bmad.py` (helper script), `Project3_Quantum_Inspired_RepresentationsV2607_V2/outputs/critical-reviews/BMAD-RECONCILIATION.md` (HPC-only fix-queue superseded by v38) — **this report is the single canonical source**; v38: `BMAD_Q1_DATA_ANALYSIS_REPORT_V1.md` deleted locally and on HPC; v37: HPC-audit corrections — 5,000-molecule hybrid benchmark COMPLETED on HPC (jobs 12651→12660, state-vector QK optimization, Hybrid RF AUC 0.8423 ± 0.0076); TDA/TNE validity transpose fixed (§3.1: TDA 19,849/0, TNE 19,836/13); QKS §3.3 table regenerated from canonical v11 summary (TA 0.543, acc 0.718/0.694); unverified speedup claim removed; job-ID reconciliation (12618→12651/12660); SwissModel API token redacted)
...
## P4 QMC Tier 2 deep-diagnostic — VMC/DMC (2026-07-31)

*(This July 31 diagnostic supersedes the July 29 "QMC removed" manuscript-cleanup note that follows — the manuscript stays clean; this section documents why.)*

After Tier 1 (PySCF/xtb/gpu4pyscf SCF + molden, jobs 12682→12686) was restored, the Tier 2 (VMC/DMC) gap was closed by installing **`pyqmc` 0.8.1** (CPU numba backend) into `malaria_md` and running a systematic diagnostic to determine whether production-quality QMC energies could be produced for the 4 Pareto candidates (144 electrons). **Result: the JastrowSpin combination/summation path of PyQMC 0.8.1 is broken in this environment — Tier 2 candidate-level energies are NOT publication-grade.**

### Evidence chain (all on HPC, July 31, 2026)
...
| 3 | **Canonical LiH docs example (RHF/cc-pvdz)** | VMC −7.39 vs RHF −7.78 | Trial WORSE than the bare Slater determinant — impossible for a valid Jastrow ⇒ **JastrowSpin value/derivative combination path broken** |
| 4 | **H₂O with `ion_cusp=False`** | VMC −76.53, **DMC −76.50** vs SCF −76.33 (FCI −76.44) | `ion_cusp=False` restores valid energies **on small systems only** (5-block, nconfig 400) |
...
### Production path (for a future run)

A publication-grade QMC section requires: a **working Jastrow** (patched PyQMC or QMCPACK), **Jastrow optimisation** (`pyqmc.recipes.OPTIMIZE`), **nconfig ≥ 1000**, and a **τ→0 extrapolation**. Until then, P4 manuscript must not claim QMC/DMC validation (already removed per the 2026-07-29 cleanup below).
