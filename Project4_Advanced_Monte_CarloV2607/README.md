...
- **Pareto MCTS**: Multi-objective candidate discovery with an audited post-hoc front over the recorded MPO, SYBA, RRS and PNS schema (SA is constant in the reported front). In the historical canonical run SYBA was a constant zero fallback and was recomputed before the final front was re-derived; canonical HV = 1.2366.
- **P1/P2-informed computational oracles**: MPO (≥0.75), docking (Tartarus V2 targets), SYBA (>0 when the classifier is available), SA (<3.5), an **RRS-informed Morgan/Tanimoto proxy** to P2 reference chemotypes, and a **PNS-informed normalized Tartarus multi-target proxy** (`score_1syh`, `score_6y2f`, `score_4lde`), plus GPU-accelerated CuPy batch Tanimoto lookup. Historical Pareto SYBA provenance is explicitly post hoc.
- **4-Method Benchmark**: MCTS+ScafVAE vs. Random vs. GA vs. Greedy across 20 independent seeds with paired t-testing and real best-in-seed molecule sets. The **v12-activity benchmark** (`results/benchmark_molecules_opt_v12/p4_benchmark_merged.csv`, job 12865) is canonical for scalar reward: Random 0.6724 > MCTS 0.6649 > GA 0.6453 > Greedy 0.4278. MCTS--Random Δ=−0.0075, paired $t_{19}=−4.97$, $p=0.000085$; the historical pre-activity Pareto front remains a separate locked artifact (post-hoc SYBA re-derivation; HV 1.2366, 4 non-dominated solutions). The v11 optimal-config benchmark is retained as historical sensitivity context.
...
| **N5** | **Resistance-aware lead selection:** the historical Pareto run records P2-informed RRS/PNS proxies during candidate search; SYBA was constant at its fallback and is explicitly re-derived post hoc rather than treated as an informative search-time objective. |

**Resistance/polypharmacology integration:** the RRS-informed chemotype-similarity proxy and PNS-informed Tartarus docking proxy are recorded during historical candidate discovery and used in the audited post-hoc front (the latter averages detected columns `score_1syh`, `score_6y2f`, `score_4lde`). They are P2-informed, not direct P2 WT/mutant or four-target validation. The reported front is re-derived on MPO, recomputed SYBA, and these RRS/PNS-informed proxies, making P4 the generation-side carrier of the programme's domain thread while preserving provenance: P2-informed resistance/multi-target signals → candidate discovery and retention → auditable front re-evaluation. In the canonical front, three of four candidates have PNS=1 and the RRS-informed proxy spans 0.141–0.223; the narrow range and discrete PNS values are reported as limitations, not hidden.
