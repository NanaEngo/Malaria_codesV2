...
With vehicle controls retained, the phenotype-only baseline achieved mean column-wise log loss \num{0.02378}, macro-AUROC \num{0.6435}, macro-AUPRC \num{0.1428}, mean Brier score \num{0.00374}, and expected calibration error \num{0.00376} across \num{25} fold--seed evaluations. Removing vehicle controls produced nearly identical values (log loss \num{0.02389}, macro-AUROC \num{0.6417}, macro-AUPRC \num{0.1412}); the change in macro-AUROC was only $-\num{0.0018}$. This sensitivity analysis supports the robustness of the phenotype-only reference. Its log-loss, AUPRC, and AUROC values are not numerically comparable to the molecular ROC-AUC results because the task, labels, feature space, aggregation unit, and primary metric differ. Consistent with the distinction between phenotype, chemical, and network evidence in computational MoA analysis \citep{Trapotsi2022MoA}, these labels indicate MoA-associated prediction rather than causal target engagement.
...
\begin{tabular}{lccc}
\toprule
Model & Random & Scaffold & $\Delta$ vs.\ ECFP4 \\
\midrule
ECFP4--RF (baseline) & \num{0.9433} $\pm$ \num{0.0003} & \textbf{\num{0.8300}} $\pm$ \num{0.0023} & --- \\
GIN & \num{0.9098} $\pm$ \num{0.0022} & \num{0.8047} $\pm$ \num{0.0141} & $-\num{0.025}$ ($p=\num{0.018}$*) \\
...
This comparison used a fixed training protocol with AdamW, validation-based early stopping, selection of the validation-optimal epoch, and \num{25} fold--seed replicates. The result is consistent with reports across ADMET/Tox21 endpoints \citep{guo_ding_2026} and pretrained-embedding benchmarks \citep{benchmark_pretrained_2025}, in which local substructure encodings captured by ECFP4 and exploited by tree ensembles remain highly data-efficient.
