...
All code, frozen splits (random and scaffold, five seeds each), per-fold results, salience data, figure scripts, and the canonical \num{19836}-molecule panel are available under the MIT licence in the public GitHub repository (\url{https://github.com/NanaEngo/Malaria_codesV2}). Trained checkpoints and frozen feature tables are included; reproducibility is supported by the frozen split files and pinned environment.

\noindent\textbf{Secondary scaffold-only replication.} An independent re-execution limited to the leak-fixed ChemBERTa scaffold protocol (five folds $\times$ five seeds) produced all \num{25} fold--seed records and a mean ROC AUC of $\num{0.7908} \pm \num{0.0298}$, closely bracketing the primary scaffold estimate ($\num{0.7867} \pm \num{0.0338}$). The primary random-split estimate was $\num{0.9121} \pm \num{0.0047}$; no independent random-split rerun was performed.
