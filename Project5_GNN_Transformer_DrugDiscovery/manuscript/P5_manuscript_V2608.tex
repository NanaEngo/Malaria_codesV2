...
\noindent\textbf{Replication provenance.} As an additional audit, SLURM job \texttt{12889} independently re-executed the leak-fixed ChemBERTa scaffold protocol (five folds $\times$ five seeds) with metric capture. The scaffold output was complete (25/25 fold--seed records; mean ROC AUC $\num{0.7908} \pm \num{0.0298}$) and is retained as a separate replication artifact, not substituted for the canonical scaffold estimate ($\num{0.7867} \pm \num{0.0338}$). The same job was deliberately cancelled after the scaffold step to avoid a redundant random-split rerun; the canonical random result (job \texttt{12815}, $\num{0.9121} \pm \num{0.0047}$) is unchanged. The cancellation affected no reported primary result.
