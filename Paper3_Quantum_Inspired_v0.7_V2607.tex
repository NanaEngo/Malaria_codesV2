...
QKS (quantum kernel)$^\dagger$ & 0.936 & 0.842 & 0.880 & +0.068 \\
Hybrid$^\ddagger$ (TFP+TNE+QK) & 0.691 & 0.620 & 0.610 & -0.177 \\
TFP + ECFP4 & 0.865 & 0.780 & 0.787 & -0.003 \\
\bottomrule
\end{tabularx}
\\vspace{2mm}
\footnotesize $^\dagger$ QKS evaluated separately (10,000 mol benchmark, $n = 5$-fold CV on representative subsample).\\
...
\begin{table}[htbp]
\centering
\caption{Kernel method comparison for activity prediction on \num{10000} molecules (July 2026 benchmark). The RBF baseline is anomalously low.}
\label{tab:qkernel}
...
\midrule
Quantum (StronglyEntangling) & 0.936 & 0.842 & 0.880 & 0.684 & \num{0.0003} \\
RBF (SVM) & 0.105 & 0.510 & 0.400 & 0.334 & {---} \\
Linear & 0.720 & 0.710 & 0.771 & {---} & \num{0.0005} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Full activity prediction benchmark. AUC-ROC, accuracy, and F1 for all representation methods. $\Delta$AUC is relative to ECFP4 baseline. Bonferroni-corrected $p$-values from paired $t$-tests across 5 CV folds.}
\label{tab:hybrid}
\begin{tabularx}{\textwidth}{l S[table-format=1.3] S[table-format=1.3] S[table-format=1.3] S[table-format=+1.3] l}
\toprule
...
Hybrid $-$ TFP & 0.685 & 0.610 & 0.605 & -0.183 & \num{0.002} \\
Hybrid $-$ TNE & 0.680 & 0.605 & 0.600 & -0.188 & \num{0.002} \\
Hybrid $-$ QKS & 0.605 & 0.590 & 0.403 & -0.263 & {< \num{0.001}} \\
\bottomrule
\end{tabularx}
\\vspace{2mm}
\footnotesize $^\dagger$ QKS evaluated separately (10,000 mol benchmark, $n = 5$-fold CV on representative subsample).\\
...
The 20 high-confidence polypharmacological leads identified in Paper~2 include compounds classified by their Resistance Resilience Score (RRS) into Class~A (resistance-resilient, RRS $\geq 120$), Class~B (robust, RRS $\geq 80$), Class~C (intermediate, RRS $\geq 65$), and Class~D (resistance-vulnerable, RRS $< 65$) \citep{temgoua2026antimalarial}. We computed TFP features for the 14 compounds with complete RRS data and performed a direct cross-paper analysis.

The results establish a strong topological signature of resistance resilience: H$_1$ total persistence (cumulative ring topology) exhibits a Spearman correlation of $\rho = 0.916$ ($p < 0.0001$, $n=14$) with the RRS mean score. Class~A compounds (resistance-resilient) exhibit systematically higher H$_1$ total persistence (mean $3.74 \pm 0.34$~\AA) and H$_1$ feature count (mean $5.33 \pm 0.58$) compared to Class~C compounds (mean $2.64 \pm 0.92$~\AA, $4.29 \pm 1.89$ features) and the single Class~D compound ($1.73$~\AA, 4 features). H$_1$ feature count also correlates with RRS ($\rho = 0.801$, $p = 0.0006$). This finding implies that resistance-resilient leads systematically possess richer ring topologies --- more and longer-lived H$_1$ features --- than resistance-vulnerable compounds, providing a geometrically interpretable basis for resistance tolerance. The violin plots of H$_1$ features across RRS classes are shown in \cref{fig:h1_rrs} (Supplementary).

This cross-paper result has two direct implications. First, it validates the independent scientific utility of TFP beyond activity prediction: even though TFP does not outperform ECFP4 for primary virtual screening (\cref{tab:benchmark}), it reveals a structural determinant of resistance resilience that classical fingerprints obscure. Second, it provides a prospective hypothesis for future lead optimization: compounds with H$_1$ total persistence above $3.5$~\AA\ may be prioritized as resistance-resilient candidates, without requiring additional MD simulation.
