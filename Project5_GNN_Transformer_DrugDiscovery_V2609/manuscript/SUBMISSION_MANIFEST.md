...
**Generated:** superseded; regenerate after the final P5V2 source/package freeze.
...
- Calibration and NN-Tanimoto OOD analyses are `COMPUTED_SECONDARY_POSTHOC` / `COMPUTED_SECONDARY_NEIGHBOR_OOD`; Butina and extended campaigns are `COMPUTED_SECONDARY`. Calibration slope/intercept, recalibrated models, and prospective validation remain `NOT_COMPUTED` and outside the current submission scope.

## Zenodo package status (superseded pending final P5V2 regeneration)
...
- The previous 31-file staging record is superseded. The final package must be rebuilt from the frozen P5V2 manuscript, SI, cover letter, bibliography, claim-supporting artifacts, and updated manifest; status remains `READY_FOR_AUTHOR_REVIEW`, not uploaded.
