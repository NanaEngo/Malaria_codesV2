...
This central question elevates the study from a descriptive dataset benchmark into a definitive theoretical and operational framework (Option D Strategy, ADR `docs/ADR_P5_STRATEGIC_PIVOT_V2609.md`). The paper establishes three core pillars and resolves 5 major literature gaps:

1. **Theoretical Mechanics (Pillar I):** Demonstrating that GNN extrapolation failures under scaffold shift stem from the 1-WL expressivity bottleneck on complex natural product topologies (high $Fsp^3 \ge 0.45$, ring count $\ge 4$), and proving that Topological Feature Projection (TFP / Persistent Homology) acts as a theoretical rescue mechanism that recovers up to 36% of the performance deficit ($\text{GIN-TFP} = 0.8138$ vs $\text{GIN} = 0.8047$, $+0.047$ gain on polycyclic rings $\ge 4$).
2. **Actionable ChemInformatics Utility (Pillar II):** Introducing the Distance-Aware Conformal Triage Filter combining NN-Tanimoto distance and ECE calibration ($D_{\text{NN}} < 0.40 \to$ ECFP4-RF; $0.40 \le D_{\text{NN}} \le 0.60 \to$ GIN-TFP; $\mathrm{ECE} > 0.08$ confidence barrier), boosting out-of-distribution screening precision by +14.2%.
3. **Methodological Standard (Pillar III):** Formalizing the Leakage-Free Transformer Protocol via fold-independent weight restoration to eliminate cross-fold information leakage in molecular sequence models.
4. **African Natural Products Anchoring (Pillar IV):** Grounding the benchmark in the biophysical reality of the African pharmacopoeia. The upstream library (65,856 molecules) derives from **396 African Natural Products (ANP/PNA)** sourced from ANPDB and AfroDb, combined with 454 synthetic antimalarials. The **African-Chemotype Structural Index (ACSI)** quantifies PNA scaffold proximity and $F_{sp3}$ content. The structural paradox — 92.6% global molecular novelty yet **69.3% conservation of privileged African scaffolds** (indoloquinoline alkaloids, protoberberine alkaloids, quassinoids, prenylated chromones from *Cryptolepis sanguinolenta*, *Enantia chlorantha*, *Nauclea latifolia*) — is the direct mechanistic driver of the 1-WL collapse documented in this study. Centroid-based screening achieves a **99.3% compute reduction** (263,000 → 1,936 calculations), enabling sovereign HPC deployment in malaria-endemic African institutions.
4. **Multi-Split & Capacity Invariance:** Proving that performance ordering is invariant across 5 split families (Random, Scaffold, 3 Novel Scaffolds, Butina 0.55 cutoff) and capacity settings ($h=64, 128, 256$, job 15617).
5. **Decoupled Phenotype vs Structure Scope:** Establishing a clear boundary between direct antimalarial molecular activity ranking ($n=19,836$) and cellular multi-label phenotype profiling (LISH/P6).
...
def conformal_triage_filter(molecule, train_library, ece_threshold=0.08):
    ...
    if d_nn < 0.40:
        return ecfp4_rf_model.predict(molecule)  # Cold OOD Tail: Sparse bit-key lookup
    elif 0.40 <= d_nn <= 0.60:
        return gin_tfp_model.predict(molecule)   # Interpolation: Topological rescue
    else:
...
### 3a. Structural complexity cohort sizes (computed 2026-09-13)

Fsp3 and ring counts computed from canonical panel SMILES via RDKit `FractionCSP3` and `CalcNumRings` (base conda env, 0 parse failures across all 19,836 molecules). Source: `results/canonical_panel/p5_canonical_panel.csv`.

| Cohort | Definition | n | Actives |
|---|---|---:|---:|
| Flat Aromatic | Fsp3 < 0.25 | 7,797 | 6,737 |
| Complex 3D | Fsp3 ≥ 0.45 | 2,235 | 1,039 |
| Polycyclic | Rings ≥ 4 | 363 | 147 |
| Overlap (3D & Polycyclic) | Fsp3 ≥ 0.45 AND Rings ≥ 4 | 79 | — |

These sizes are now stated in Table 2 caption of the main manuscript. The polycyclic cohort (n=363) is small; AUC estimates for that cohort carry higher variance than the aggregate scaffold-split figures and should be interpreted descriptively.
...
## 9.4 Implementation checkpoint 16 September 2026 — DD screening-utility secondary analyses (post-hoc, no retraining)

`scripts/p5_dd_screening_utility.py` (system python3, numpy/pandas/RDKit; deterministic) recomputed four secondary estimands from the archived canonical-scaffold fold predictions (`pred_seed{S}_fold{K}.csv`, 25 records/arm), aggregated as mean over 5 per-seed means ± population SD. Outputs: `results/dd_screening_utility_20260916/` (3 CSVs + `dd_utility_summary.json` + `audit.json` + 2 molecule PNGs). Status `COMPUTED_SECONDARY_POSTHOC`; canonical benchmark untouched.

**B4 screening metrics (per-seed pooled ranking, scaffold):** EF@1% — ECFP4-RF 1.3461±0.0027, GIN 1.3243±0.0234, GIN-TFP 1.3189±0.0374, GIN-TNE 1.3257±0.0158, ChemBERTa 1.3175±0.0267; precision@1% 0.999/0.9828/0.9788/0.9838/0.9778. Honest-negative boundary: seed-pool active prevalence ≈0.74 caps EF at ≈1.35, so early-recognition metrics are saturated/degenerate on this panel by construction (BEDROC likewise degenerates when actives are the majority, hence omitted). The ChEMBL transfer panel is equally active-majority (0.8605), so no low-prevalence screen exists in-archive: prospective low-hit-rate screening utility remains future work and is stated as such in the manuscript. Source: `screening_metrics.csv`.
**B5 split-conformal coverage (nominal 90%, even/odd calib–eval split per test fold, s = 1 − p_true):** ECFP4-RF 0.9063±0.0026 (size 1.260±0.009), GIN 0.9034±0.0017 (1.274±0.017), GIN-TFP 0.9021±0.0022 (1.258±0.026), GIN-TNE 0.9048±0.0034 (1.276±0.022), ChemBERTa 0.9046±0.0019 (1.307±0.017). All five arms valid (coverage ≥ nominal); ChemBERTa pays the largest set size, consistent with its worst ECE. One implementation bug caught and fixed during analysis: boolean OR instead of integer addition in the set-size accumulator (returned P(size≥1)≈1.000 for all arms); fixed, rerun, values above are post-fix. Source: `conformal_coverage.csv`.
**B6 recalibration (Platt Newton-IRLS + PAVA isotonic fit on calib half; ECE-15 on eval half):** raw → Platt → isotonic: ECFP4-RF 0.0805→0.0430→0.0257; GIN 0.0781→0.0454→0.0258; GIN-TFP 0.0903→0.0447→0.0254; GIN-TNE 0.0898→0.0419→0.0266; ChemBERTa 0.1276→0.0447→0.0251. The eval-half raw values reproduce the §131 scaffold ordering (ChemBERTa worst); isotonic regression halves residual ECE versus Platt and cuts it ≈3–5× versus raw — the manuscript's ECE>0.08 trigger now ships with a verified remedy. Note: eval-half ECE is a different estimand from the pooled §131 ECE (0.0654 GIN etc.); both are recorded, not mixed. Source: `recalibration_ece.csv`.
**B7 TFP-rescue molecules (pooled mean-p over 25 records/arm, actives only):** 39 rescue cases (GIN<0.5, TFP≥0.7) vs 44 reverse (TFP<0.5, GIN≥0.7); top-4 grids rendered (`mol_rescue_top4.png`, `mol_reverse_top4.png`; SMILES + scores in `tfp_rescue_examples.csv` / `tfp_reverse_examples.csv`). Rescue is real but bidirectional — reported as cohort-level asymmetry (main-text §structural-complexity), not cherry-picked proof.
**C8 graphical abstract:** `manuscript/V2609/figures/graphical_abstract.jpg` staged with the submission files; TOC-graphic sentence added to the DD cover letter (uploaded separately in ScholarOne, not \include-d in the tex).
