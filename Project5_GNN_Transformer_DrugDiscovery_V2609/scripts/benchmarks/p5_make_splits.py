...
def make_scaffold_folds(smiles_list: list[str], y: np.ndarray, seed: int) -> list[dict]:
    """Scaffold-aware 5-fold: test scaffolds disjoint from train/val scaffolds.

    Whole scaffold groups are assigned to folds, so the held-out test fold shares no
    scaffold with the rest of the data. Validation is then carved at the molecule level
    from the non-test portion (see below), so val and train DO share scaffolds; early
    stopping therefore measures optimization on the training chemical distribution, and
    only the test fold estimates scaffold shift.
    """
