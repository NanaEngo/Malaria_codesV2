...
Repeats the P5 protocol on a public ChEMBL Plasmodium falciparum panel
(target CHEMBL364, IC50/EC50 with numeric pChEMBL) to show that the canonical
P5 verdict (ECFP4-RF >= GNN on this library) is not panel-specific.
...
Dataset: ChEMBL CHEMBL364 panel (smiles, activity, pchembl), built by
scripts/p5_public_malaria_chembl.py; actives at pChEMBL >= 6.0, inactives < 5.0,
the 5.0-6.0 band dropped as ambiguous, and P5-panel SMILES overlaps removed
(results/p5_public_chembl_malaria_disjoint.csv). Supply via --csv.
...
def load_dataset(csv_path: str | None) -> pd.DataFrame:
    ...
    if csv_path and Path(csv_path).exists():
        ...
        # binarize if continuous (the disjoint ChEMBL panel is already binary 0/1)
...
def main() -> int:
    ...
    report = {"dataset": "ChEMBL CHEMBL364 (P. falciparum) IC50/EC50 panel", "n": len(df),
