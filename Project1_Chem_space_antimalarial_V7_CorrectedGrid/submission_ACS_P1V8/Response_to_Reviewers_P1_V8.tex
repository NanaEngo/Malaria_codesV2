...
% (to be provided upon assignment of the prepared deposit): repo public, MIT license,
% release tag v1.0.0, staged package zenodo_package_P1/ (sha256-verified). Insert the
% DOI in Data Availability (main) and SM reproducibility when assigned.
...
\textbf{Response:} The repository is now publicly accessible at
\url{https://github.com/NanaEngo/Malaria_codesV2} under the MIT license (\texttt{LICENSE} at
the repository root), with a versioned archival release \texttt{v1.0.0}
(\url{https://github.com/NanaEngo/Malaria_codesV2/releases/tag/v1.0.0}) containing the complete
revision codebase, the canonical result directories (including the two-arm DEKOIS re-run of
R2.4), and a checksummed staging package for archival (\texttt{zenodo\_package\_P1/},
sha256-verified manifest). We propose Zenodo for the archival DOI: the deposit package is
prepared and checksummed, and the DOI will be provided upon assignment and inserted into the
Data Availability statement (main) and the SI reproducibility section
...
Additionally executed for R2.6: the repository is now public (MIT license, release tag
\texttt{v1.0.0}, GitHub release), with the staged Zenodo package ready; the archival DOI will
be provided upon assignment of the prepared Zenodo deposit.
