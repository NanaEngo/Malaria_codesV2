...
% integrated into the SM table and main text). R2.6 is executed except the Zenodo DOI
% (to be provided upon assignment of the prepared deposit): versioned archival deposit
% zenodo_package_P1/ assembled (sha256-verified, MIT license, Zenodo-only policy —
% no repository URLs in the manuscript-facing documents). Insert the DOI in Data
% Availability (main) and SM reproducibility when assigned.
...
minor points. We have revised the manuscript to address every point. Below we
...
protocols (\num{40} runs, exhaustiveness \num{32}), and computed per-drug RRS profiles to test
whether the framework recovers clinically documented resistance signatures. The result is
negative (\Cref{SM-tab:sm_retrospective_antimalarials}; \Cref{M-sec:disc_validation}): pyrimethamine
...
check, a sensitivity analysis shows that the PNS--RRS association survives and
...
top-ranked pocket (score \num{162.7}, probability \num{0.999}) lies \qty{5.7}{\angstrom} from the grid center (\Cref{SM-sec:sm_val_crt}), i.e., the box covers the central cavity, not a peripheral region.
...
the V2 grid (exhaustiveness \num{32}): \num{68} runs, all finite. \Cref{SM-tab:vina} (PfCRT column), \Cref{M-tab:rrs_main}
...
substantially more favorable (mean \qty{-8.42}{\kilo\calorie\per\mole} vs \qty{-6.19}{\kilo\calorie\per\mole}), and the corrected
PfCRT RRS values all fall between \qtyrange{99.1}{100.6}{\percent}. Class counts changed from \numlist{6;5;5;1} to
\numlist{7;4;6;0} (A*/B/C/D), and the corrected RRS range is \qtyrange{73.2}{115.6}{\percent}.
\end{enumerate}
...
(RMSD $\approx \qty{30}{\angstrom}$) and the PfDHFR grid position indicate that the null enrichment partly
...
We ran the requested control as a two-arm experiment on the identical DEKOIS
panel (the same \num{40} actives and \num{1200} property-matched decoys, same grid): (i) the deposited 7F3Y
receptor with MTX retained, which matches the original baseline
(ROC-AUC \num{0.502} [\qty{95}{\percent} CI: \numrange{0.423}{0.586}], EF@1\% \num{0.00}, EF@5\% \num{0.50}), and (ii) an MTX-stripped
receptor rebuilt in the same coordinate frame (ROC-AUC \num{0.563} [\qty{95}{\percent} CI: \numrange{0.477}{0.646}],
EF@1\% \num{0.00}, EF@5\% \num{1.00}), both at exhaustiveness \num{32} (the revised-protocol standard, matched
in both arms). The result is negative for the
blockade hypothesis: the confidence intervals overlap and EF@1\% remains zero in both arms, so
removing MTX yields only a marginal, non-significant improvement ($\Delta$AUC $= \num{+0.060}$) and
does not restore enrichment. The near-chance DEKOIS result is therefore not explained by steric blockade from the retained cofactor ligand, and the two-arm data are added to \Cref{SM-tab:sm_enrichment_validation}
...
The neutral-within-null reading is corroborated by additional analyses performed for this study. (i) An extended replication panel (39 ligands
$\times$ 8 PfDHFR/PfCRT states, 312/312 finite records, prepared independently
for this study) gives a bootstrap RRS distribution of 100.45
[99.59, 101.32], so the corrected PfCRT channel's null spread (99.4--100.5\%)
falls inside the pipeline noise of this independent preparation. (ii) In a score-perturbation sensitivity analysis, RRS class assignments are robust to
bounded $\pm1$ kcal/mol perturbations of individual
...
5 kcal/mol) sensitivity runs leave the reported class distributions stable. Separately, an MD-based MM-GBSA $\Delta\Delta G$ analysis
performed for this study found 0 of 12 PfDHFR/PfCRT mutants significantly weaker
...
\textbf{Response:} All data, code, machine-readable tables, raw docking outputs, and
records have been assembled into a dedicated, versioned archival deposit
(\texttt{zenodo\_package\_P1/}; 384 files with a sha256-verified checksum manifest) that
will be published on Zenodo under an CC BY 4.0 license, with a versioned release record and the
accompanying checksum manifest, so that the exact version used in this study remains
permanently citable and verifiable. We propose Zenodo as the archival repository: the deposit
is prepared and checksummed, and the archival DOI will be provided upon assignment and
inserted into the Data Availability statement (main) and the SI reproducibility section
(\Cref{SM-sec:sm_repro}). In parallel, the WT/mutant score table required to
...
every RRS denominator), so the RRS denominators no longer reside only in external resources.
...
revised canonical-grid protocol and therefore report seed dispersion for that protocol. After the
...
\qty{0.3}{\kilo\calorie\per\mole}, including four that sit exactly at the median (PP-05 PfATP4, PP-10
PfClpP, PP-12 PfCRT, PP-13 PfDHFR), so the within-target favorability counts carry
this boundary sensitivity. The corrected-protocol single-run scores used for those assignments
are the same values reported in the corrected \Cref{SM-tab:vina}, and the re-dock used a single seed
with exhaustiveness \num{32} as in the original protocol. The seed dispersion shown in \Cref{SM-tab:sm_multiseed_validation}
(\qty{<0.06}{\kilo\calorie\per\mole}) was measured under the revised canonical-grid protocol and therefore
provides a scale reference for seed sensitivity rather than a direct bound on the corrected
single-seed V2-grid runs; the boundary analysis above (\num{20} pairs within \qty{0.3}{\kilo\calorie\per\mole} of a
...
stands alone; the full methodology is described in the Methods and Supporting Information of this manuscript.
\end{enumerate}

\section*{Status of items formerly marked}
...
antimalarials (R1.2; 40 runs, negative result in \Cref{SM-tab:sm_retrospective_antimalarials} and
...
MTX-stripped DEKOIS comparison (R2.4; negative result, two rows added to
\Cref{SM-tab:sm_enrichment_validation} and discussed in \Cref{SM-sec:sm_val_dekois}).
Additionally executed for R2.6: the complete versioned archival deposit (\url{https://doi.org/10.5281/zenodo.22686176};
sha256-verified manifest, CC BY 4.0 license) has been assembled for Zenodo publication; the archival
DOI will be provided upon assignment.
