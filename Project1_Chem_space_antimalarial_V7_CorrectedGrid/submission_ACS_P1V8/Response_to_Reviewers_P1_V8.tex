...
(\texttt{zenodo\_package\_P1/}; 384 files with a sha256-verified checksum manifest) and
published on Zenodo under an MIT license at \url{https://doi.org/10.5281/zenodo.22696778},
with a versioned release record and the accompanying checksum manifest, so that the exact 
version used in this study remains permanently citable and verifiable. In parallel, the 
WT/mutant score table required to recompute \Cref{M-tab:rrs_main} is now included directly 
in the SI (\Cref{SM-tab:sm_wt_mutant_scores}: the
