...
- **Size:** 592 KB
- **Status:** ✅ Ready
- **Sections:** S1-S13 (13 numbered sections, incl. S12.1-S12.7 validation block)
- **Tables:** 14 (auto-numbered S1-S14; includes WT/mutant scores Table S6, retrospective approved-antimalarials Table S7, and multi-seed Table S13)
- **Compilation:** 0 errors, 0 undefined references
- **Latest Update:** 2026-09-09 (revision V8: R1.2/R2.x computed results incl. R2.4 two-arm DEKOIS re-run rows in the enrichment table, cross-referencing refactor)

#### 3. Response to Reviewers (PDF)
- **File:** `Response_to_Reviewers_P1_V8.pdf`
- **Pages:** 7
- **Size:** ~270 KB
- **Status:** ✅ Ready
- **Compilation:** 0 errors, 0 undefined references
- **Notes:** Cross-references via `xr` + `cleveref` (`\Cref{M-...}` / `\Cref{SM-...}`); reviewer quotes kept verbatim; no [TO COMPLETE] items remain except author-action R2.6

#### 4. Cover Letter (PDF)
- **File:** `Cover_Letter_P1_V8.pdf`
- **Pages:** 1
- **Size:** 155 KB
- **Status:** ✅ Ready
...
- **Label-driven cross-referencing (2026-09-09):** all SM sections are now numbered
  (`\SectionNumbersOn` + `\thesection = S\arabic{section}`) with `\label{sec:sm_*}`; the
  main text, SM, and response letter reference each other exclusively via `\Cref`
  (`xr` + `cleveref`; prefixes `M-` and `SM-`). No `\section*` and no hard-coded section/
  table numbers remain in the main or SM. The S12 validation block is naturally numbered
  S12.1–S12.7; auto-numbered tables are S1–S13.
...
- [x] Main PDF compiled (28 pages, 0 errors)
- [x] SM PDF compiled (23 pages, 0 errors)
- [x] Response to reviewers PDF compiled (7 pages, 0 errors)
- [x] Cover letter PDF compiled (1 page, 0 errors)
...
- [x] Cover letter prepared (separate file)
- [ ] Final author review completed (see `P1_V8_REVISION_STATUS.md` — READY FOR AUTHOR REVIEW)
- [ ] TOC graphic verified (300+ DPI)
- [ ] **R2.6 release:** repo public + licence + tag + Zenodo DOI (see `P1_R26_RELEASE_CHECKLIST.md`; inventory verified against disk 2026-09-09)
