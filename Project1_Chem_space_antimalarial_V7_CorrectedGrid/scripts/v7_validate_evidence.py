#!/usr/bin/env python3
"""P1 V7 evidence-integrity audit.
...
def write_candidate_manifest(v5_map: dict[str, dict[str, str]], p2_rows: list[dict[str, str]], errors: list[str], out: Path) -> dict:
    ...
    if len(rows) != 17:
        errors.append(f"V7_CANDIDATE_MANIFEST_ROWS:{len(rows)} != 17")
...
def main() -> int:
    ...
    ap.add_argument("--output", type=Path, default=ROOT / "results/validation/v7_integrity_audit.json")
    ap.add_argument("--dossier", type=Path, default=ROOT / "results/validation/v7_review_dossier.md")
    ap.add_argument("--candidate-manifest", type=Path, default=ROOT / "results/v7_candidate_manifest.csv")
    args = ap.parse_args()
    ...
    result = {
        "schema": "p1-v7-evidence-integrity-audit/v2",
