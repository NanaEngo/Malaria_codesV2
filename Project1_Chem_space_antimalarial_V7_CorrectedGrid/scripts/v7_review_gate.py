#!/usr/bin/env python3
"""V7 hash-bound independent-review promotion gate.
...
``PRE_SUBMISSION_DEVELOPMENT``, use the phase-aware V7 generation workflow;
...
def check_gate(path: Path = REGISTER) -> tuple[bool, str]:
    if not path.exists():
        return False, f"FAIL-CLOSED: V7 review register missing: {path}"
    try:
        reg = json.loads(path.read_text())
    except (OSError, json.JSONDecodeError) as exc:
        return False, f"FAIL-CLOSED: unreadable V7 review register: {exc}"
    ...
    if reg.get("accepted_for_full_run") is not True:
        return False, "FAIL-CLOSED: independent review has not accepted the V7 evidence"
    ...
    return True, "GATE PASSED: hash-bound signed independent V7 review payload is present for all four targets"
