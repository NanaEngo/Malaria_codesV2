#!/usr/bin/env python3
"""Generate V7 integrated candidate data and exploratory figures.

The join is performed through the locked V7 canonical-SMILES manifest. No
...
from v7_review_gate import check_gate as check_v7_review_gate  # noqa: E402
...
OUT = V7 / "results" / "derived"
FIG = V7 / "results" / "figures"
MANIFEST = V7 / "results" / "v7_candidate_manifest.csv"
...
V7_REGISTER = V7 / "results" / "v7_review_register.json"
...
def load(internal_development: bool = False) -> tuple[pd.DataFrame, dict]:
    ...
    if not required.issubset(manifest.columns):
        raise ValueError("V7 manifest lacks required columns")
    if set(manifest["cohort_id"].dropna().unique()) != {"P2_SET_C_POLYPHARM_17"}:
        raise ValueError("V7 manifest contains an unexpected cohort")
    if len(manifest) != 17 or manifest["candidate_id"].nunique() != 17:
        raise ValueError("Expected 17 unique candidates in V7 manifest")
    if (manifest["canonical_smiles"] != manifest["v5_source_smiles"]).any():
        raise ValueError("V7 manifest canonical and V5 source SMILES differ")
    ...
    if manifest["p2_source_sha256"].nunique() != 1:
        raise ValueError("V7 manifest has inconsistent P2 source hashes")
    ...
    for name, df, required_metric in [
        ...
        if str(df["candidate_sha256"].iloc[0]) != expected_source_hash:
            raise ValueError(f"{name} hash does not match the locked V7 manifest hash")
    ...
    # Keep only declared V7 analysis fields and explicit raw target-wise scores.
    ...
    if len(merged) != 17 or merged["candidate_id"].nunique() != 17:
        raise ValueError("Merged V7 data is not 17 unique candidates")
    register = json.loads(V7_REGISTER.read_text(encoding="utf-8"))
    v5register = json.loads(V5_REGISTER.read_text(encoding="utf-8"))
    v7_pending = register.get("status") == "PENDING_INDEPENDENT_REVIEW" and register.get("accepted_for_full_run") is False
    ...
    if internal_development:
        ...
        if not (v7_pending and v5_pending):
            raise ValueError("internal development requires both V5 and V7 registers to remain truthfully pending")
    elif pre_submission:
        ...
        v7_gate_ok, v7_gate_message = check_v7_review_gate(V7_REGISTER)
        if not v7_gate_ok:
            raise ValueError(f"V7 submission gate is not cryptographically accepted: {v7_gate_message}")
    ...
    return merged, {"v7": register, "v5": v5register}


def review_gate_provenance(gates: dict, exploratory: bool) -> dict:
    """Return non-promoting provenance for exploratory V7 outputs."""
    return {
        "v5_status": "PENDING_INDEPENDENT_REVIEW" if exploratory else gates["v5"]["status"],
        "v7_status": "PENDING_INDEPENDENT_REVIEW" if exploratory else gates["v7"]["status"],
        ...
        "v7_accepted_for_full_run": False if exploratory else gates["v7"]["accepted_for_full_run"],
    }
...
def fig_rrs_heatmap(df: pd.DataFrame) -> list[str]:
    ...
    return save_fig(fig, "p1_v7_rrs_mutation_profiles")
...
def fig_metric_scatter(df: pd.DataFrame) -> list[str]:
    ...
    return save_fig(fig, "p1_v7_exploratory_metric_relationships")
...
def fig_profile(df: pd.DataFrame) -> list[str]:
    ...
    return save_fig(fig, "p1_v7_targetwise_profile_summary")
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="write exploratory V7 data/figures to results/exploratory while review is pending",
    )
    args = ap.parse_args()
    pre_submission = is_pre_submission()
    exploratory = bool(args.internal_development or pre_submission)
    if exploratory:
        OUT = V7 / "results" / "exploratory" / "derived"
        FIG = V7 / "results" / "exploratory" / "figures"
    else:
    ...
    out_csv = OUT / "v7_integrated_candidate_metrics.csv"
    ...
    provenance = {
        "schema": "p1-v7-integrated-data-figures/v2",
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "status": ("EXPLORATORY_UNVERIFIED_VOID_FOR_SUBMISSION" if exploratory
                    else "INDEPENDENT_REVIEW_ACCEPTED_SOURCE_PANEL"),
        "join_rule": "V7 candidate manifest canonical_smiles -> P2 source tables smiles; never row-order join",
        "scientific_boundary": [
        ...
        "sources": {str(p.relative_to(ROOT)): sha256(p) for p in [MANIFEST, VINA, V5 / "results" / "v5_four_target_vina_review_table.json", RRS, ACSI, PNS, V7_REGISTER, V5_REGISTER]},
        "generator": {
            "version": "1.0.0",
    ...
    prov = OUT / "v7_integrated_data_figures_provenance.json"
