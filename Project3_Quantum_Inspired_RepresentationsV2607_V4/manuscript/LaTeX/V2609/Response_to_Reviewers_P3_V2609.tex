...
This pools multiple Plasmodium assays from ChEMBL 34 and represents a label-source 
transfer test rather than rigorous endpoint-specific validation. We acknowledge this limitation 
explicitly in the manuscript and Supporting Information: the pooled analysis tests transferability 
to experimental labels broadly but does not provide assay-specific, homogeneous endpoint 
benchmarking (\Cref{M-sec:methods_dataset}; Supporting Information, descriptor transfer section).
\end{enumerate}

\textbf{Scope acknowledgment and endpoint-specific feasibility audit:} The current revision does not 
implement the requested endpoint-specific ChEMBL benchmark (one assay ID, one target, one measurement 
type per endpoint). To evaluate feasibility, we conducted a systematic audit of ChEMBL \textit{Plasmodium 
falciparum} activities using the following protocol criteria: (i)~standard measurement types IC$_{50}$, 
EC$_{50}$, or $K_i$ with pchembl\_value in nM; (ii)~minimum n=500 compounds per endpoint; (iii)~minimum 
100 compounds per class (active/inactive) using threshold pchembl\_value 6.5; (iv)~organism \textit{P. 
falciparum}. The audit queried 82,112 total activities from ChEMBL, filtered to 67,885 with IC$_{50}$/EC$_{50}$/$K_i$ 
measurements, across 5,483 unique assay IDs.

\textbf{Result:} Only \textbf{3 viable endpoints} were identified meeting the protocol criteria:
\begin{itemize}[leftmargin=1.6em]
\item CHEMBL1040692: Novartis W2 (drug-resistant) screen, EC$_{50}$, n=4,570 (organism-level)
\item CHEMBL1040691: Novartis 3D7 (drug-susceptible) screen, EC$_{50}$, n=4,294 (organism-level)
\item CHEMBL4649964: Calibr delayed death assay, IC$_{50}$, n=867 (organism-level)
\end{itemize}

All three are whole-organism phenotypic screens (not target-specific biochemical assays), and two 
derive from the same Novartis study (W2/3D7 strain pair), limiting independent validation. With only 
3 endpoints below our threshold of $\geq$5 independent assays, and insufficient target-level diversity, 
endpoint-specific analysis would not provide robust validation of descriptor generalizability across 
diverse antimalarial targets. The pooled ChEMBL panel (n=22,447) is therefore retained as exploratory 
label-source-shift analysis only and makes no claim to address endpoint-specific validation. The audit 
demonstrates that this scope limitation reflects \textit{data availability constraints} in the ChEMBL 
antimalarial corpus, not technical implementation difficulty.
...
$\Delta G \leq -7.0$ kcal mol$^{-1}$. The data were obtained from the Tartarus external docking 
repository (Tartarus, 2024), which provided 19,913 docked leads; 17,011 had complete four-target 
profiles. Docking parameters (receptor preparation, grid specifications, exhaustiveness) are 
documented in the Tartarus repository cited in the Supporting Information.''
...
\textbf{Data provenance note:} The docking dataset is an external validation resource used 
to test descriptor information retention. It originates from the Tartarus repository, not 
from P3-internal calculations. This external sourcing is now explicitly documented in the 
