...
% Cross-referencing - xr (not xr-hyper) loaded after hyperref, matching the main
% manuscript. xr-hyper breaks \cref on externally imported labels.
\usepackage{xr}
\usepackage{cleveref}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{persistence_diagrams_v2.pdf}
% \caption{Representative topological fingerprints of an African natural product seed and a generated non-seed candidate. (A~--~B)~2D structures of the 
% representative molecules. (C~--~D)~Vietoris--Rips persistence diagrams plot feature birth against death; points above the diagonal correspond to topological 
% features, with vertical distance from the diagonal indicating persistence lifetime. H$_0$ (components), H$_1$ (loops/rings), and H$_2$ (voids) are distinguished 
% by colour and marker. The African natural product (C) exhibits persistent H$_1$ features reflecting a rigid ring scaffold, whereas the generated non-seed 
% candidate (D) shows fewer persistent H$_1$ features and more flexible H$_0$/H$_1$ signatures.}
 \caption{Representative topological fingerprints of an African natural product seed
  and a generated non-seed candidate.
  (\textbf{A},~\textbf{D})~2D structures of the two representative molecules.
  (\textbf{B},~\textbf{E})~Vietoris--Rips persistence diagrams: each point plots
  feature birth (filtration radius at which the feature appears) against death
  (radius at which it merges or fills); vertical distance from the diagonal equals
  the feature's persistence lifetime.
  H$_0$ (connected components), H$_1$ (loops/rings), and H$_2$ (voids) are
  distinguished by colour and marker; point area is proportional to persistence.
  Vertical line segments connect each H$_1$ point to the diagonal to make
  persistence lifetimes directly readable.
  The African natural product (\textbf{B}) carries five H$_1$ features with
  persistence $> 0.5$ (mean \num{0.82}), reflecting a fused polycyclic ring scaffold;
  the generated non-seed candidate (\textbf{E}) carries only one such feature
  (mean \num{0.34}), with the remaining H$_1$ points clustered near the diagonal,
  consistent with a flexible chain bearing a single ring.
  (\textbf{C},~\textbf{F})~Persistence barcodes show the same features as
  horizontal bars (birth to death), sorted by decreasing persistence within each
  homology dimension; bar opacity encodes persistence.
  H$_0$ bars are capped at the eight most persistent features for legibility
  (43 total for the seed, 29 for the non-seed).}
...
\label{SM-sec:si_tartarus_docking}

To assess information retention after TNE compression, we analysed Tartarus docking scores \citep{tartarus_2024}: the full docking dataset comprised \num{19913} primary leads docked against four targets (PfDHFR/1SYH, PfCRT/4LDE, PfATP4/6Y2F, PfClpP/2F6I) using AutoDock Vina with target-specific grid configurations; of these, \num{17011} molecules had complete docking profiles across all four targets and were used for polypharmacology analyses. These data served as an external chemoinformatic validation oracle to test descriptor information retention; detailed docking parameters (receptor preparation, grid specifications, exhaustiveness) are provided in the Tartarus repository \citep{tartarus_2024}.

\textbf{Note on structural models:} The Tartarus docking structures (1SYH, 6Y2F, 4LDE, 2F6I) serve solely as an external chemoinformatic validation oracle—a descriptor stress test to assess information retention—and are distinct from the refined structural models (PfDHFR/7F3Y, PfCRT/6UKJ, PfATP4/9N10) used for molecular dynamics and resistance-resilience assessments in companion studies \citep{temgoua2026tb,saognn2026}. The docking analysis here does not inform, and is not informed by, the MD-based binding free energy calculations reported elsewhere.

Random Forest regressors were trained on TNE-only vectors (\num{192}-dim) to predict each target's docking score, with ECFP4 as baseline. Detailed results are reported for three targets; PfClpP followed similar patterns.
...
All descriptor methods were evaluated on an external ChEMBL antimalarial dataset ($n = \num{22447}$) using 5-fold stratified cross-validation with Random Forest (\num{200} trees, \texttt{random\_state=42}). The external panel was assembled from ChEMBL~34 bioactivity records for \textit{Plasmodium falciparum} assays (IC$_{50} \leq \SI{10}{\micro\molar}$ = active, $> \SI{10}{\micro\molar}$ = inactive). 

\textbf{Limitations:} This analysis pools heterogeneous assays using a single IC$_{50}$ threshold. It does not report assay-specific benchmarking, does not identify individual assay IDs, targets, or protocols, and does not address the methodological requirement for homogeneous endpoint-level analysis raised by Reviewer~1. It is presented as an exploratory label-source-shift analysis only. The panel is external to the primary eos80ch label source; molecule-level disjointness was not established and is not claimed. Feature extraction followed the identical pipeline used for the internal benchmark. For the intention-to-evaluate analysis, TNE embedding failures ($n = \num{351}$, \qty{1.6}{\percent}) were assigned zero vectors. The complete-case analysis ($n = \num{22096}$) gave ECFP4 \num{0.960}, TFP \num{0.861}, TNE \num{0.644}, and TFP+TNE \num{0.798}, versus ITT values of \num{0.960}, \num{0.864}, \num{0.645}, and \num{0.800}. The qualitative ranking was unchanged.
