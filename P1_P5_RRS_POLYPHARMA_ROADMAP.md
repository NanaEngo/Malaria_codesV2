...
|---|---|---|---|
| P1 V4 baseline / V5 in progress | Strong computational chemical-space study; V4 wording is locally refinable while V5 adds a blocked, evidence-bounded reconstruction path | circular/retrodictive validation overinterpretation; centroid-to-member transfer; provenance wording; V5 target identity/pocket gates; resistance bridge | **80–85%** for V4 at a well-matched computational-cheminformatics venue after clean recompile + audit; V5 has no submission estimate until its gates pass |
...
### Daily execution log — 8 August 2026

| Track | Action today | Status / stop condition |
|---|---|---|
| P5 | **Benchmark completed in full**: GIN–TFP random `0.9084 ± 0.0060`, GIN–TNE random `0.8918 ± 0.0060` (jobs 12841/12842, 25 fold×seed each). Paired-t (df=4) + BH-FDR verified on both splits; every arm significantly below ECFP4 under random AND scaffold. Manuscript `tab:h1`, abstract, §H1, Methods, cover letter, DAR v3-h, README, and strategic plan v2 updated; benchmark and learning-curve figures regenerated; main + cover recompiled clean (0 error, 0 undefined ref). Pushed `71df114c1`, `8de2260c9`, `d38d65b9e` | **P5 submission package ready pending JoC final checks + Zenodo upload** |
| P1 V5 | Re-checked gates: grid-revision preflight **FAIL-CLOSED** (4GM2 is PfClpR, not PfClpP); structural-pocket preflight **REVIEW_REQUIRED_NOT_AUTHORIZED** (independent signed review still pending); reference-ligand/grid preflight reruns present | DiffDock/Vina remains blocked until P1.0 gates pass; no new V5 docking runs today |
| P2 | Re-checked preflight: still **BLOCKED** (`SOURCE_IDENTITY_BLOCKED` for 214–PfCRT, requires independent review; force-field manifests missing for all four systems) | **0 `grompp`/`mdrun` processes, 0 new trajectories**; generate + independently audit force-field manifests before retry |
| P2 (manifests) | **Truthful force-field manifests generated for all 4 parent systems** (08/08): `MD_systems/{201_PfDHFR,214_PfCRT,438_PfATP4,164_PfClpP}/forcefield_manifest.json` with real SHA-256 hashes (topol.top, ions.gro, npt.gro/npt.cpt where present, ligand .itp parameter files). **Declared force fields = reality (CHARMM36-jul2022 protein, GAFF2/AM1-BCC via ACPYPE ligand, TIP3P, 310.15 K), NOT the publication-grade policy values (CHARMM36m/CGenFF).** Validator therefore remains FAIL-CLOSED by design; 438_PfATP4 also lacks npt.gro/npt.cpt. **Decision required (author-level):** either (a) accept the fully-documented CHARMM36-jul2022/GAFF2 alternative by updating the manifest policy, or (b) re-run MD in CHARMM36m+CGenFF. Manifests are not git-versioned (`MD_systems/` gitignored); they live with the system trees. | Gate intentionally fail-closed; no GROMACS launched |
...
| P3–P4 | Preserve frozen benchmark claims; P4 checker discrepancy (greedy 0.6147 vs canonical 0.7211) documented as stale checker, no rerun | No new headline result |
