...
**Daily status (8 August 2026, evening refresh):** V4 remains the frozen evidence baseline and V5 is the canonical successor under controlled migration. The inherited 4GM2/PfClpP identity remains invalid (4GM2 is PfClpR), and fixed-center DiffDock containment remains failed closed at 79.17%; those artifacts are provenance-only. Corrected target-anchored Vina evidence is now complete for all four targets (17/17 per target), but V5 is still not submission-ready because the independent structural-review register remains pending. No consensus, RRS, or PNS result is accepted before the mandatory signed review.
...
**Computational rescue update (09/08/2026):** the initial 2F6I array yielded 449/484 raw passes and 35 explicit failures. A separate, non-overwriting retry array is authorized for failed IDs only. It changes only the ligand conformer seed and Vina exhaustiveness (`32`, seed `20260809`), while retaining the exact 2F6I triad-centered box and biological gate. Multi-fragment or persistently out-of-pocket molecules remain failed; no threshold or box relaxation is allowed. The retry is a software robustness/sensitivity layer, not a new biological validation. The old dependency-blocked job 12966 is not used. A separate uniform 484-centroid run under the same rescue protocol is the preferred candidate replacement; it remains raw/pending until aggregation and independent review.
...
**Status (09/08/2026 refresh) — PILOT EN COURS :** panel mutant uniforme préparé (8 récepteurs protéine-seule, frames rmsd=0.0) via `scripts/p1_v5_rrs_prepare_receptors.py` ; dockings Vina soumis (jobs **12894** PfDHFR / **12895** PfCRT — resoumission après fix anti-clobber : CSV par cible `v5_mutant_vina_scores_<TARGET>.csv` + fusion `scripts/p1_v5_rrs_merge_scores.py`) ; RRS par-cible via `scripts/p1_v5_rrs_pilot.py` (protocole P2, baseline |ΔG_WT|≥5.0) ; **découverte : 6UKJ = isoforme 7G8 (fond K76T) → baseline WT PfCRT = révertant T76K** ; dossier de revue prêt-à-signer `results/INDEPENDENT_REVIEW_DOSSIER_v2.md` (mis à jour : jobs 12894/12895, commandes de signature openssl/gpg §7, **validation de procédure sur copie réussie 08/08**) ; **auto-vérification machine `scripts/p1_v5_review_selfcheck.py` → `results/review_selfcheck_report.json` (6/6 checks : table 17×4, frames rmsd=0, provenance, identité, register PENDING)**. **DÉCISION D'AUTEUR 08/08 (soir) : la revue indépendante est OBLIGATOIRE — bypass interne REJETÉ** (« il faut faire absolument la revue indépendante »). Register `authorization_mode = INDEPENDENT_REVIEW_REQUIRED`, `internal_work_authorized=false`, gate strict fail-closed **restauré** (fonction partagée `check_gate` : `accepted_for_full_run=true` + 4 cibles `ACCEPTED`). Le consensus 4 cibles calculé sous le bypass rejeté est **VOID** (provenance status VOID, CSV supprimé). Gate-opener corrigé (`decision=ACCEPTED` sur chaque cible PASS). Verdict pilote = `DOCKING_RRS_PILOT_COMPUTED_PENDING_REVIEW` ; **aucun consensus/RRS/PNS/manuscrit V5 avant signature humaine du register** ; priorité HPC : jobs mutants 12894/12895 (demande CPU 16→4 pour backfill immédiat sur les 8 CPU libres).
