...
Across $N = \num{17011}$ molecules with multi-target docking data, H$_0$ count ($\rho = -\num{0.248}$, $p < 10^{-137}$), H$_0$ entropy ($\rho = -\num{0.243}$), and H$_1$ entropy ($\rho = -\num{0.190}$) negatively correlate with the number of targets bound ($\geq 2$ targets at $\Delta G \leq -\SI{7.0}{\kcalmol}$). Lower topological complexity thus associates with broader polypharmacological activity, consistent with reduced steric clash across multiple binding sites.
...
Four limitations constrain interpretation. First, TFP does not outperform ECFP4 in activity prediction; it adds value primarily at the high-complexity tail (molecules with $\geq 3$ fused rings or $\geq 2$ stereocentres). Second, activity labels are computational predictions from Ersilia ML models, so all claims regarding activity and polypharmacology are predictions pending experimental validation. Third, the H$_1$--RRS association ($\rho = \num{0.2399}$, $p = \num{6.76e-8}$) disappears after controlling for molecular weight, ring count, fraction sp$^3$, and H$_0$ count ($\rho_{\text{partial}} = \num{0.0329}$, $p = \num{0.4668}$), indicating size-mediated confounding. Fourth, the quantum kernel was evaluated on a classical simulator; NISQ hardware would degrade fidelity \citep{preskill2018nisq}.

\subsection{Practical recommendations}

Based on these results, we recommend: (i)~ECFP4 for virtual screening of African natural product libraries (AUC \num{0.948}, robust to distribution shift); (ii)~TFP for assessing scaffold preservation in generative campaigns (interpretable H$_1$/H$_0$ decomposition); (iii)~TNE for fixed-length embeddings when computational budget constrains dimensionality (validate on external panels); and (iv)~QKS for retrospective mechanistic analysis on small lead-optimisation sets ($\leq \num{10000}$ compounds), where its $O(N^2)$ scaling is acceptable. Future work should integrate TDA features with graph neural networks \citep{topologynet_2018,rollins2024molprop}, validate tensor-network descriptors experimentally, and deploy quantum kernels on NISQ devices with error mitigation.
