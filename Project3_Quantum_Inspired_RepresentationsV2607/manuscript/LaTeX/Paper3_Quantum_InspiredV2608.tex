...
\title{\textbf{Persistent homology and tensor network descriptors for antimalarial virtual screening: resolving the scaffold paradox in African natural product chemical space}}
...
\begin{abstract}
Molecular representation determines what structural information is accessible to machine learning models in drug discovery, yet standard fingerprints poorly capture the topological complexity of natural product scaffolds. We introduce three complementary molecular descriptors---a Topological Fingerprint (TFP) derived from persistent homology, a Tensor Network Embedding (TNE) via Tucker decomposition, and simulated Quantum Kernel Scores (QKS)---and benchmark them against classical baselines on a library of \num{19849} African antimalarial candidates. TFP resolves a scaffold paradox: the structurally expanded molecules are \qty{92.6}{\percent} Tanimoto-distinct from seed natural products yet retain \qty{69.3}{\percent} of their Bemis--Murcko scaffolds, because H$_1$ ring topology is preserved while H$_0$ connectivity diverges. TNE compresses molecular tensors to \num{192}-element descriptors at \num{6.1}$\times$ real-atom compression (reconstruction error \num{0.113}), retaining binding-relevant information competitive with 2048-bit ECFP4 for PfDHFR ($R^2 = \num{0.473}$). On the canonical activity-prediction benchmark ($n = \num{19836}$, 5-fold CV), ECFP4 remains the strongest single descriptor (AUC \num{0.948}); TFP (\num{0.876}) and TNE (\num{0.722}) capture complementary but lower signal. The hybrid descriptor (TFP+TNE+QK) achieves AUC \num{0.888}, with QKS as the principal positive contributor (ablation $\Delta = -\num{0.040}$). The quantum kernel shows no statistically significant difference from a gamma-tuned RBF baseline ($p = \num{0.060}$ at $n = \num{19849}$), but external validation on \num{22447} ChEMBL compounds reveals a small transfer gap ($p = \num{0.005}$). We position these representations as complementary diagnostic tools that reveal topological dimensions of molecular structure, not as replacements for classical fingerprints.
\end{abstract}
...
Malaria remains a leading cause of mortality in sub-Saharan Africa, with \num{247} million cases and \num{619000} deaths reported in 2023 \citep{who2024malariareport}. The spread of artemisinin-resistant \emph{Plasmodium falciparum} strains from Southeast Asia to Africa \citep{ariyey2014,ashley2018} underscores the urgent need for new chemical scaffolds effective against resistant parasites. African natural products represent a historically validated but computationally underexplored reservoir of such scaffolds: quinine and artemisinin themselves derive from natural sources, yet African natural products remain underrepresented in public databases \citep{anpdb_2026,value_addition_african_np_2025}. Our companion study \citep{temgoua2026antimalarial} used \num{396} African natural-product and \num{454} synthetic-drug seeds to generate \num{65856} antimalarial candidates via CHEESE similarity search \citep{lzicar_cheese_2024} and STONED-SELFIES expansion. Efficient computational screening of this resistance-relevant chemical space depends critically on the molecular representations available to machine learning models.

Extended-Connectivity Fingerprints (ECFP) have dominated computational drug discovery for two decades by encoding local atom environments into fixed-length bit vectors \citep{ecfp_2010}. These representations perform well for synthetic drug-like molecules whose scaffolds are well represented in training data. However, natural product-derived compounds occupy distinct regions of chemical space: they exhibit higher sp$^3$ carbon fractions, more chiral centres, more complex ring systems, and greater three-dimensional character than synthetic libraries \citep{coconut_2021}. Classical fingerprints may not fully capture the topological features---bridged ring systems, molecular cavities, and higher-order atomic interactions---that distinguish these molecules.

Three frameworks from physics and topology offer complementary molecular representations. Topological Data Analysis (TDA) uses persistent homology to identify shape features that persist across spatial scales, producing persistence diagrams encoding connected components (H$_0$), holes (H$_1$), and voids (H$_2$) \citep{tda_review_2025}; persistent homology has been validated for protein--ligand binding affinity \citep{tda_binding_affinity_2026} and molecular property prediction \citep{tda_pretraining_2026}. Tensor networks compress high-dimensional data by exploiting low-rank structure \citep{tensornet_2023,tn_efficient_2026}. Quantum kernel methods map data into high-dimensional feature spaces through parameterised quantum circuits \citep{qml_review_2026,pqc_chemistry_2026}. These three frameworks address orthogonal limitations of classical fingerprints: TDA captures global topology invisible to local substructure hashes, tensor networks provide physically interpretable compression, and quantum kernels probe non-linear feature interactions in Hilbert space.

Here we introduce three molecular descriptors---the Topological Fingerprint (TFP), Tensor Network Embedding (TNE), and Quantum Kernel Score (QKS)---and benchmark them on a \num{19849}-molecule library of African antimalarial candidates. Our specific aims are: (i)~to determine whether persistent homology can resolve the scaffold paradox arising from structural expansion of natural product seeds; (ii)~to evaluate whether tensor network compression retains binding-relevant information; (iii)~to assess whether quantum kernel methods offer practical advantages over properly tuned classical baselines; and (iv)~to construct a hybrid descriptor that combines complementary information from all three representations. We compare against ECFP4, MACCS, and other classical baselines under rigorous cross-validation, and validate transferability on an external ChEMBL antimalarial panel of \num{22447} compounds.
...
The broader generated source library consists of \num{65856} molecules from \num{396} African natural products and \num{454} synthetic drugs \citep{temgoua2026antimalarial}. The representation benchmarks use the canonical \num{19849}-molecule panel, with \num{19836} molecules retained for analyses requiring complete TNE embeddings. Binary activity labels were obtained from Ersilia model eos80ch using its standard classification threshold of \num{0.5}; the canonical panel is \qty{74.2}{\percent} active / \qty{25.8}{\percent} inactive. For external validation, we assembled \num{22447} antimalarial compounds from ChEMBL~34 (IC$_{50} \leq \SI{10}{\micro\molar}$ = active, $> \SI{10}{\micro\molar}$ = inactive) with standard RDKit canonicalisation.
...
\end{equation}
where $\tilde{t}_i$ is min--max normalised TFP, $\tilde{e}_i$ is normalised TNE, and $q_i \in \mathbb{R}^{30}$ is the quantum kernel PCA component vector ($n_{\text{kpca}}=30$, the Phase~2 optimum). Random Forest is invariant to per-component monotone scaling, so no explicit fusion weights are needed. Hyperparameter optimisation followed a two-phase protocol: grid search on $n = \num{200}$ molecules (60 combinations), then full-panel validation at $n = \num{19836}$ with the winning configuration (bond dimension~6, \num{1} repeat, \num{30} kernel PCA components).
...
\subsection{Tensor network compression retains binding-relevant information}
...
Across $N = \num{17011}$ molecules with multi-target docking data, H$_0$ count ($\rho = -\num{0.248}$, $p < 10^{-137}$), H$_0$ entropy ($\rho = -\num{0.243}$), and H$_1$ entropy ($\rho = -\num{0.190}$) negatively correlate with the number of targets bound ($\geq 2$ targets at $\Delta G \leq -\SI{7.0}{\kcalmol}$). Lower topological complexity in ring structures and structural components thus associates with broader polypharmacological activity, consistent with reduced steric clash penalties across multiple binding sites.
...
The scaffold paradox---\qty{92.6}{\percent} Tanimoto distinction with \qty{69.3}{\percent} scaffold recovery---arises because STONED-SELFIES mutations permute side-chains and functional groups (diverging H$_0$ connectivity) while preserving cyclic macro-structures (conserving H$_1$ ring topology). This decomposition has practical implications for generative-model evaluation: scaffold-only Tanimoto (\num{0.379}) is \num{1.84}$\times$ higher than whole-molecule Tanimoto (\num{0.206}), confirming that H$_1$ persistence diagrams capture scaffold identity independently of atom-level similarity. TFP thus provides an interpretable diagnostic tool for assessing whether molecular generation protocols preserve or destroy the ring systems essential for bioactivity---a capability absent from standard fingerprint-based diversity metrics.

\subsection{Tensor network compression: when low-rank structure helps}

Tucker decomposition exploits the fact that molecular tensors are approximately low-rank: the \num{6.1}$\times$ compression retains sufficient information for PfDHFR docking-score prediction ($R^2 = \num{0.473}$, competitive with ECFP4's \num{0.461}) while providing a fixed-length, physically interpretable embedding. The compression is not uniform across chemical space: molecules with common ring systems compress better (reconstruction error \num{0.098}) than structurally atypical molecules (\num{0.137}), and the atom-coordinate factor matrix correlates with molecular volume ($\rho = \num{0.71}$). This selective compression---preserving scaffold-relevant information while discarding noise---explains why TNE clusters achieve higher chemical coherence (Silhouette \num{0.35}) than ECFP4 (\num{0.18}) or VAE latent vectors (\num{0.229}), a property relevant for virtual screening where enrichment metrics depend on hit-cluster homogeneity.

\subsection{Kernel calibration, not quantum-ness, governs performance}

The central finding regarding quantum kernels is that, with a properly tuned RBF baseline and the canonical 6-qubit circuit, the quantum kernel shows no statistically significant difference from the RBF kernel at any sample size (internal: $p = \num{0.060}$ at $n = \num{19849}$; external: $p = \num{0.005}$ at $n = \num{22447}$). The external validation reveals a small but real transfer gap that the internal comparison was underpowered to detect. This result aligns with recent matched-baseline audits: QBioFusion-QSAR \citep{qbiofusion_2026} found that quantum multiple-kernel learning improved small-data classification only for individual molecules, with no mean MCC gain, and parameter-matched quantum--classical comparisons \citep{iso_qgnn_2026} attribute topology-aligned architecture gains to inductive bias rather than quantum-ness.

The practical implication is that kernel choice and calibration matter more than the kernel's origin. For practitioners, this means: (i)~always tune the RBF gamma by inner cross-validation before claiming quantum advantage; (ii)~the quantum kernel's $O(N^2)$ scaling restricts it to lead-optimisation sets of $\leq \num{10000}$ compounds, where its ability to capture non-linear Hilbert-space structure may add value for heterogeneous or out-of-distribution compounds; and (iii)~for primary virtual screening, ECFP4 (AUC \num{0.948}) remains the recommended baseline, with TFP and TNE serving as complementary descriptors for specific chemical-space regions.

\subsection{TNE degradation on external data: a cautionary tale}

The \num{0.077} AUC degradation of TNE from internal (\num{0.722}) to external (\num{0.645}) validation is the largest among all descriptors and reflects Tucker decomposition's sensitivity to distribution shift. The \num{351} embedding failures (\num{1.6}\%) were concentrated in structurally complex natural products with unusual bond topologies---exactly the chemical-space region where TNE compression is most needed but least reliable. This finding has methodological implications: tensor-network descriptors should be validated on external panels before deployment, and embedding failures should be reported transparently rather than silently excluded. TFP, by contrast, transferred well ($\Delta = -\num{0.011}$), suggesting that persistent homology summary statistics are more robust to distribution shift than tensor core elements.

\subsection{Limitations and future directions}

Several limitations constrain interpretation. First, TFP does not outperform ECFP4 in activity prediction; it adds value primarily at the high-complexity tail (molecules with $\geq 3$ fused rings or $\geq 2$ stereocentres). Second, activity labels are computational predictions from Ersilia ML models rather than experimental measurements, so all claims regarding activity and polypharmacology should be understood as predictions pending experimental validation. Third, the H$_1$--RRS association observed in the expanded cohort ($\rho = \num{0.2399}$, $p = \num{6.76e-8}$) disappears after controlling for molecular weight, ring count, fraction sp$^3$, and H$_0$ count ($\rho_{\text{partial}} = \num{0.0329}$, $p = \num{0.4668}$), indicating size-mediated confounding rather than an independent topological predictor. Fourth, the quantum kernel was evaluated on a classical simulator; on NISQ hardware, gate errors and coherence times would degrade fidelity \citep{preskill2018nisq}. Future work should integrate TDA features with graph neural networks \citep{topologynet_2018,rollins2024molprop}, validate tensor-network descriptors experimentally, and deploy quantum kernels on NISQ devices with error mitigation.

\subsection{Practical recommendations}

Based on our benchmark, we recommend: (i)~ECFP4 as the primary descriptor for virtual screening of African natural product libraries (AUC \num{0.948}, robust to distribution shift); (ii)~TFP as a complementary descriptor for assessing scaffold preservation in generative campaigns (interpretable H$_1$/H$_0$ decomposition); (iii)~TNE for fixed-length, physically interpretable embeddings when computational budget constrains feature dimensionality (caveat: validate on external panels); and (iv)~QKS reserved for retrospective mechanistic analysis on small lead-optimisation sets ($\leq \num{10000}$ compounds), where its $O(N^2)$ scaling is acceptable and its Hilbert-space structure may reveal non-linear feature interactions absent from classical kernels.
...
We introduced three quantum-inspired molecular representations---persistent homology TFP, Tucker-compressed TNE, and simulated QKS---and benchmarked them on \num{19836} African antimalarial candidates. TFP resolves the scaffold paradox by decomposing molecular topology into H$_1$ ring preservation and H$_0$ connectivity divergence. TNE achieves \num{6.1}$\times$ real-atom compression while retaining binding-relevant information competitive with ECFP4 for specific targets. The quantum kernel shows no significant advantage over a properly tuned RBF baseline, with external validation revealing a small transfer gap. These representations are best positioned as complementary diagnostic tools that reveal topological dimensions of molecular structure, with ECFP4 remaining the recommended primary descriptor for virtual screening. The persistent homology decomposition provides an interpretable framework for evaluating generative-model outputs in natural product drug discovery.
