...
\begin{table}[htbp]
\centering
\caption{Top-5 quantum circuit configurations. Rows 1–3: re-benchmarked on $n = 1000$ molecules (5-fold CV, per-fold data in raw CSVs). Rows 4–5: from the initial grid search on $n = 200$ molecules. The default configuration ($d = 8$, $n_{\text{rep}} = 2$, $n_{\text{kpca}} = 20$) is highlighted.}
\label{tab:qp_optimisation}
\begin{tabularx}{\textwidth}{S[table-format=1.0] S[table-format=1.0] S[table-format=2.0] S[table-format=1.4] S[table-format=1.4] l}
\toprule
{$d$} & {$n_{\text{rep}}$} & {$n_{\text{kpca}}$} & {AUC} & {Std} & {$n_{\text{mol}}$} \\
\midrule
6 & 1 & 30 & 0.8283 & 0.0371 & 1000 \\
6 & 6 & 20 & 0.8047 & 0.0354 & 1000 \\
6 & 6 & 30 & 0.8121 & 0.0396 & 1000 \\
4 & 1 & 30 & 0.8412 & 0.0521 & 200 \\
8 & 2 & 20 & 0.8420 & 0.0510 & 200 \\
\bottomrule
\end{tabularx}
\vspace{1mm}
\footnotesize Rows 1–3 are from the authoritative n=1000 re-benchmarking (per-fold data in phase2 raw CSVs). Rows 4–5 are from the initial n=200 grid search. The default configuration (row 5) is used throughout the main manuscript.
\end{table}
