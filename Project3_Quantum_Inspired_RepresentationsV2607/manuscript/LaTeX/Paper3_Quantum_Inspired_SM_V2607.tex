...
\cref{SM-tab:sota} reports the performance of five TDA descriptor strategies paired with Random Forest (RF) and Support Vector Machine (SVM) classifiers on $n = 5000$ molecules (stratified subsample from the \num{19849}-molecule library; 5-fold cross-validation, class-weighted classifiers). The production labels CSV contained $\num{19849}$ molecules with eos80ch activity labels merged with TDA fingerprints; $n = 5000$ was used due to SVM kernel matrix $O(N^2)$ scaling.

\begin{table}[htbp]
\centering
\caption{SOTA topological benchmark: five TDA descriptor strategies with RF and SVM classifiers ($n = 5000$ molecules, 5-fold stratified CV, class-weighted). Best result: PersStats + RF (AUC $0.842$).}
\label{SM-tab:sota}
...
\midrule
PersStats & RF & 0.8419 & 0.7694 & 0.7715 & 22 \\
TFP-Enriched & RF & 0.8381 & 0.7614 & 0.7614 & 32 \\
PersImage & RF & 0.8370 & 0.7644 & 0.7640 & 25 \\
TFP-12 & RF & 0.8303 & 0.7496 & 0.7505 & 12 \\
...
Key findings: (i)~PersStats + RF achieves AUC $0.842$, approaching the ECFP4 baseline (AUC $0.868$) with only 22 topological features; (ii)~RF consistently outperforms SVM across all strategies (mean $\Delta\text{AUC} = +0.038$); (iii)~TFP-Enriched (32 features) provides marginal improvement over TFP-12 (12 features, $\Delta\text{AUC} = +0.008$); (iv)~BettiCurve underperforms (AUC $0.772$), indicating Betti number sequences alone lack the discriminative power of persistence statistics.
