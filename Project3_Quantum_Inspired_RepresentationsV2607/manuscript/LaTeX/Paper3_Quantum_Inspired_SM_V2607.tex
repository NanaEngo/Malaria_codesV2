...
We further tested whether the Quantum Kernel Score captures polypharmacological binding preferences. Compounds were labelled as polypharmacological if they bound $\geq 2$ targets at $\Delta G \leq -\SI{7.0}{\kcalmol}$ (\qty{12.0}{\percent} prevalence in the merged $N=\num{17011}$ set). A 5-fold CV classification on the full set ($N = 19{,}900$, class-1 prevalence \qty{10.3}{\percent}) reports QKS AUC \num{0.747} $\pm$ \num{0.013} vs.\ RBF AUC \num{0.737} $\pm$ \num{0.010}: the quantum kernel provides task-specific sensitivity to polypharmacology on par with the tuned classical baseline. An earlier $n=\num{1000}$, 10-fold benchmark was terminated because a single 900$\times$900 kernel matrix exceeded practical wall time; the polypharmacology result reported here uses the state-vector kernel for computational tractability.
...
diverse spaces---as demonstrated by its standalone AUC of \num{0.823} in the QKS binary classification benchmark (\cref{M-tab:qkernel})---rather than 
...
The kernel comparison (\cref{M-tab:qkernel}; per-fold values in \cref{tab:sm_s4_perfold}) follows the exact pipeline used for the hybrid benchmark (\cref{sec:qp_optimisation}): 5-fold stratified cross-validation (\texttt{StratifiedKFold} with \texttt{random\_state=42}), UMAP reduction of the 2048-bit Morgan (radius 2) fingerprints to 6 dimensions (Jaccard metric, \texttt{random\_state=42}, $n_{\text{neighbors}}=15$, $min\_dist=0.1$) followed by scaling to $[-1,1]$, and support vector machines with precomputed kernels. The canonical 6-qubit circuit (IQPEmbedding, 1 repeat) is the Phase-2 winning configuration. Benchmarks were run at two sample sizes: the first \num{5000} molecules of the library by source order (\num{3750} active, \num{1250} inactive; job 12702) and the full canonical panel of \num{19836} molecules (\num{14721} active, \num{5115} inactive; job 12700).
