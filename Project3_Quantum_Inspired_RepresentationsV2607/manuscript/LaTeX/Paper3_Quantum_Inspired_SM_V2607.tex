...
\vspace{2mm}
\noindent\textbf{IMPORTANT:} The per-fold values in this table are from the \textbf{200-molecule development subsample} used during method development, \emph{not} the full \num{19849}-molecule benchmark. Headline AUC values reported in the main manuscript (\cref{M-tab:benchmark}) are from the full benchmark; this table provides fold-level variance estimates for the development set only. Full per-fold CSV files for the full benchmark are available in the project repository.
