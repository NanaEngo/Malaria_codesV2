...
\begin{table}[htbp]
\centering
\caption{ChEMBL experimental validation: expanded search across 77 RRS-TFP compounds against 3 \textit{Plasmodium} targets (231 candidate--target pairs, Tanimoto $\geq 0.25$). Eight structural analogues were identified (3.5\% match rate); three showed experimental activity below 1~$\mu$M (PfATP4), while the remaining five were inactive or intermediate. The low match rate supports the structural novelty of the generated chemical space.}
...
22 & PfDHFR & CHEMBL36 & Intermediate & 2.48 & 1.000 \\
...
\end{tabularx}
\vspace{1mm}
\footnotesize The expanded search queried all 77 RRS-TFP compounds against PfDHFR, PfCRT, and PfATP4 (231 pairs; PfClpP excluded). Eight matches (3.5\% match rate) exceeded the Tanimoto $\geq 0.25$ threshold. Compound 22/PfDHFR has a Tanimoto 1.000 exact match to CHEMBL36 (intermediate, IC$_{50}$ 2.48~$\mu$M) --- the only identical structure found. Three PfATP4 matches are experimentally active (IC$_{50}$ 0.4--0.79~$\mu$M), indicating that generated candidates with structural homology to known PfATP4 actives retain binding potential. The remaining four PfCRT matches are inactive. The low overall match rate (3.5\%), with only one exact structure match, provides preliminary evidence of structural novelty, though direct IC$_{50}$ measurements remain necessary for activity confirmation.
\end{table}
...
\begin{table}[htbp]
\centering
\caption{SOTA topological benchmark: five TDA descriptor strategies with RF ($n = \num{19849}$, full library) and SVM ($n = 5000$, subsample) classifiers (5-fold stratified CV, class-weighted). Highest AUC: PersStats + RF ($0.873$, numerical parity with ECFP4).}
\label{SM-tab:sota}
...
\end{tabularx}
\vspace{1mm}
\footnotesize All classifiers used \texttt{class\_weight=balanced} to mitigate the 75.9\%/24.1\% class imbalance. RF: $n_{\text{estimators}} = 500$. SVM: RBF kernel, $C = 10$, $\gamma = \texttt{scale}$. PersStats (22 features) captures persistence birth, death, persistence, and entropy across H$_0$, H$_1$, H$_2$ dimensions.
\end{table}
