...
\caption{Joint topological analysis linking Paper~3 TFP features with Paper~2 resistance classification. Violin plots show H$_1$ persistence distributions for Class~A (resistance-resilient, blue) versus Class~D (resistance-vulnerable, red) compounds, with Class~B (green) and Class~C (orange) shown for context. Quantitative analysis of the expanded cohort ($n = 77$) confirms a significant association between H$_1$ count and continuous RRS (Spearman $\rho = 0.312$, $p = 0.0057$); the pilot $n = 14$ correlation ($\rho = 0.947$, $p < 0.0001$) is shown for context but should be interpreted cautiously because the Class~D subset contains only one compound.}
\label{fig:h1_rrs}
\end{figure}
...
\cref{tab:sm_s4_perfold} reports per-fold AUC, accuracy, and F1 for the six representation methods for which per-fold development data were retained. ECFP4, TFP, TNE, and Hybrid values are from the 200-molecule development subsample; QKS values are from a 10,000-molecule quantum-kernel subsample; and RBF values are from the gamma-tuned classical kernel benchmark. The full \num{19849}-molecule headline values are in the main manuscript (\cref{M-tab:benchmark}).

\begin{table}[htbp]
\centering
\caption{Per-fold activity prediction statistics for six representation methods. ECFP4, TFP, and TNE values are from the corrected full \num{19849}-molecule classical benchmark (5-fold stratified CV, Random Forest, July 29, 2026). Hybrid values are provisional (development run, pending full rerun); QKS values are from a \num{500}-molecule quantum-kernel subsample (subsampled from the \num{10000}-molecule design target); RBF values are from the gamma-tuned classical kernel benchmark. Headline AUC values in the main manuscript (\cref{M-tab:benchmark}) are from the full \num{19849}-molecule benchmark.}
...
\midrule
ECFP4 & 1 & 0.949 & 0.898 & 0.933 \\
ECFP4 & 2 & 0.949 & 0.895 & 0.931 \\
ECFP4 & 3 & 0.949 & 0.896 & 0.931 \\
ECFP4 & 4 & 0.951 & 0.898 & 0.933 \\
ECFP4 & 5 & 0.947 & 0.897 & 0.932 \\
\midrule
...
\midrule
TFP (TDA) & 1 & 0.879 & 0.841 & 0.898 \\
TFP (TDA) & 2 & 0.880 & 0.844 & 0.900 \\
TFP (TDA) & 3 & 0.876 & 0.838 & 0.897 \\
TFP (TDA) & 4 & 0.881 & 0.837 & 0.896 \\
TFP (TDA) & 5 & 0.867 & 0.832 & 0.893 \\
\midrule
TNE ($d=8$) & 1 & 0.707 & 0.755 & 0.857 \\
TNE ($d=8$) & 2 & 0.725 & 0.752 & 0.856 \\
TNE ($d=8$) & 3 & 0.730 & 0.756 & 0.858 \\
TNE ($d=8$) & 4 & 0.730 & 0.754 & 0.856 \\
TNE ($d=8$) & 5 & 0.719 & 0.754 & 0.856 \\
\midrule
...
\vspace{2mm}
\noindent\textbf{NOTE:} The per-fold values for ECFP4, TFP, and TNE in this table are from the corrected full \num{19849}-molecule classical benchmark (see main manuscript \cref{M-tab:benchmark}). Hybrid values are provisional (development run, pending full rerun). QKS and RBF values are from a \num{500}-molecule quantum-kernel benchmark (subsampled from the \num{10000}-molecule design target). Full per-fold CSV files are available in the project repository.
...
Cohen's $d$ effect sizes for all pairwise AUC comparisons against the corrected full-library ECFP4 baseline are reported in \cref{tab:effect_sizes}. The values are now computed from the canonical 19,849-molecule classical benchmark (Random Forest, 5-fold stratified CV) rather than the earlier 200-molecule development subsample, which contained a copy-paste error in the TNE per-fold values.
...
diverse spaces---as demonstrated by its standalone AUC of \num{0.751} in the QKS binary classification benchmark (\cref{M-tab:qkernel})---rather than 
...
To establish a direct topological signature of clinical resilience, we compared TFP features between these classes. An initial pilot of 14 polypharmacological leads suggested a strong relationship (Spearman $\rho = 0.947$, $p < 0.0001$); expanding the cohort to $n = 77$ compounds yielded $\rho = 0.312$ ($p = 0.0057$), confirming that H$_1$ count is significantly associated with resistance resilience while showing the expected attenuation as sample size increases. In line with this H$_1$ count association, Class~A compounds also exhibit systematically higher H$_1$ persistence (mean $3.74 \pm 0.34$~\AA\ versus $1.73$~\AA\ for Class~D). The Class~D subset in the pilot contains only a single compound ($n = 1$), so the pilot mean should be interpreted cautiously. The attenuated but significant expanded correlation supports the hypothesis that greater ring topological complexity confers conformational stability advantageous against resistance mutations, though the relationship is weaker than the pilot suggested.
