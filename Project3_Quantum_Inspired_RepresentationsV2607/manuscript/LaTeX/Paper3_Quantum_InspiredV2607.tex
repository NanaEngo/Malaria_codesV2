...
A systematic grid search over quantum circuit hyperparameters (bond dimension $d \in \{4, 6, 8\}$, IQPEmbedding repeats $n_{\text{rep}} \in \{1, 2, 3, 4, 6\}$, kernel PCA components $n_{\text{kpca}} \in \{5, 10, 20, 30\}$; 60 combinations, 5-fold CV on $n = 200$ molecules) identified $d = 6$, $n_{\text{rep}} = 1$, $n_{\text{kpca}} = 30$ as the optimal configuration (Hybrid AUC $0.853 \pm 0.049$), approaching the ECFP4 baseline (AUC $0.868$). The default $d = 8$ remains reported throughout for consistency with the full-library benchmark; the optimised configuration is detailed in Supplementary Material, Table~S5 ($n = 1000$ re-benchmarking).
