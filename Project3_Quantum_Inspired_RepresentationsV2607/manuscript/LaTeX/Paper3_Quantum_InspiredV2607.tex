...
Quantum kernels were implemented using PennyLane \citep{pennylane_2024} with an 8-qubit statevector simulator. The circuit (\cref{SM-alg:qkernel}) encodes the overlap between quantum states prepared from two input vectors using \texttt{StronglyEntanglingLayers}, upgrading the classical embedding strategy based on quantum-generative-models QCBM architectures.States were prepared using \texttt{StronglyEntanglingLayers} parameterised by input vectors, and the kernel $K(x_1, x_2) = |\langle \phi(x_1) | \phi(x_2) \rangle|^2$ was estimated by ground-state measurement probability using PennyLane's \texttt{qp.kernels.kernel\_matrix} (full pseudocode in Supplementary Material, \cref{SM-sec:qkernel_algorithm}). Input vectors were computed as Morgan fingerprint atom-pair counts (radius 2, \num{2048} features) reduced to 8 dimensions by UMAP with Jaccard metric, then scaled to $[-1, 1]$. UMAP with Jaccard metric is the appropriate dimensionality reduction for binary/count fingerprints.
...
\footnotesize $^\ddagger$ Hybrid uses kernel PCA (10 components) from the full quantum kernel matrix.
\end{table}% (Duplicate §3.7 "Topological Features and the Scaffold Paradox" removed — content merged into §3.3 "Scaffold paradox resolution")

\subsection{Computational scalability}

The combined TDA + TNE protocol for \num{19849} molecules ran in under 30 minutes on a 12-core workstation (TDA: \SI{6.4}{\minute}, Tucker: \SI{20}{\minute}), practical for libraries up to $10^4$ compounds. Quantum kernel computation is restricted to $\leq \num{10000}$ compounds ($O(N^2)$ scaling).

\subsection{Limitations}

\textbf{First}, TDA does not outperform ECFP4 overall (\cref{tab:benchmark}); PersStats + RF (AUC $0.873$) matches ECFP4 numerically ($p > 0.05$), with TFP adding value at the high-complexity tail ($\geq 3$ fused rings). \textbf{Second}, activity labels are computational; ChEMBL validation (77 compounds, 3 targets, 231 pairs) found 7 analogues (3.0\%), including 3 active PfATP4 matches (IC$_{50}$ 0.40--0.79~$\mu$M; \cref{SM-tab:chembl_validation}). \textbf{Third}, the H$_1$-RRS correlation attenuated from pilot ($\rho = 0.947$, $n = 14$) to expanded cohort ($\rho = 0.361$, $p = 0.001$, $n = 77$, H$_1$ total persistence, 500 compounds processed; \cref{SM-fig:h1_rrs}), identifying class-imbalanced sampling as a methodological confound. \textbf{Fourth}, the quantum kernel was simulated classically (8 qubits, PennyLane \texttt{lightning.qubit}); all three kernels are statistically indistinguishable after proper RBF tuning ($p > 0.05$), an honest NISQ-era benchmark that corrects a prior hyperparameter miscalibration. \textbf{Fifth}, TFP summary statistics capture sufficient topological information; enriched TFP and neural architectures (MLP AUC 0.799 vs. RF 0.860) show no improvement. \textbf{Sixth}, the library is biased toward African NP and synthetic drug seeds; generalisation remains to be tested.

Despite these limitations, TFP resolves the scaffold paradox via H$_1$/H$_0$ decomposition, TNE achieves $5.9\times$ real-atom compression, and the hybrid matches ECFP4 ($p = 0.111$), collectively demonstrating that quantum-inspired tools complement classical representations in African natural product drug discovery.
