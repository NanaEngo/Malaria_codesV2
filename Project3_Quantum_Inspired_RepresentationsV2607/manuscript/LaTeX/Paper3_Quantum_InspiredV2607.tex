...
\textbf{First}, TDA does not outperform ECFP4 overall (\cref{tab:benchmark}); PersStats + RF (AUC $0.873$) matches ECFP4 numerically ($p > 0.05$), with TFP adding value at the high-complexity tail ($\geq 3$ fused rings). \textbf{Second}, activity labels are computational; ChEMBL validation (77 compounds, 3 targets, 231 pairs) found 7 analogues (3.0\%), including 3 active PfATP4 matches (IC$_{50}$ 0.40--0.79~$\mu$M; \cref{SM-tab:chembl_validation}). \textbf{Third}, the H$_1$-RRS correlation attenuated from pilot ($\rho = 0.947$, $n = 14$) to expanded cohort ($\rho = 0.312$, $p = 0.006$, $n = 77$; \cref{SM-fig:h1_rrs}), identifying class-imbalanced sampling as a methodological confound. \textbf{Fourth}, the quantum kernel was simulated classically; the PennyLane $\to$ IBM Quantum pipeline was verified on a 2-qubit circuit (Bell $\langle XX\rangle = +1.0056$, kernel $K = 0.988$, ibm\_fez 156-qubit processor; \cref{SM-sec:nisq_verification}), but free-tier queue times prohibit full 8-qubit benchmarks. \textbf{Fifth}, TFP summary statistics capture sufficient topological information; enriched TFP and neural architectures (MLP AUC 0.799 vs. RF 0.860) show no improvement. \textbf{Sixth}, the library is biased toward African NP and synthetic drug seeds; generalisation remains to be tested.
