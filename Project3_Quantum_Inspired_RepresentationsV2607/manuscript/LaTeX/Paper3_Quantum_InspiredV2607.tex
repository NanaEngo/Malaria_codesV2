...
\textbf{Results:} Classical ECFP4 remains superior for primary screening (AUC \num{0.868}), while a hybrid descriptor achieves comparable discriminative performance (AUC \num{0.842}, $p = \num{0.111}$) and reveals non-linear ring-topology features critical for African natural product scaffolds. H$_1$ topological persistence shows a preliminary correlation with resistance resilience scores in a 14-compound pilot (Spearman $\rho = 0.947$, $p < 0.0001$), which weakened to a non-significant trend upon expansion to 33 compounds ($\rho = 0.305$, $p = 0.085$), warranting further investigation with balanced resistance-class representation. 
\textbf{Conclusions:} Quantum-inspired molecular representations provide interpretable topological features complementary to classical fingerprints, with particular value for natural product scaffolds where ring topology determines bioactivity. A preliminary H$_1$-resistance correlation ($\rho = 0.947$, $n = 14$) did not replicate at $n = 33$ ($\rho = 0.305$, $p = 0.085$), indicating that balanced resistance-class sampling is critical for topological predictor validation.
\end{abstract}
