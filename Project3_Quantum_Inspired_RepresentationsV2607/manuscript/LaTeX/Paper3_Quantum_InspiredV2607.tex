...
\textbf{Results:} Classical ECFP4 remains superior for primary screening (AUC \num{0.868}), while a hybrid descriptor achieves comparable discriminative performance (AUC \num{0.842}, $p = \num{0.111}$) and reveals non-linear ring-topology features critical for African natural product scaffolds. A cross-paper analysis reveals that H$_1$ topological persistence correlated strongly with resistance resilience scores in a 14-compound pilot (Spearman $\rho = 0.947$, $p < 0.0001$), but this correlation attenuated upon expansion to 33 compounds ($\rho = 0.305$, $p = 0.085$), identifying balanced resistance-class sampling as a critical methodological requirement for topological predictor validation. 
\textbf{Conclusions:} Quantum-inspired molecular representations provide interpretable topological features complementary to classical fingerprints, with particular value for natural product scaffolds where ring topology determines bioactivity. The cross-paper H$_1$-resistance correlation ($\rho = 0.947$, $n = 14$ pilot) attenuated at $n = 33$ ($\rho = 0.305$, $p = 0.085$), a methodological finding demonstrating that small, class-imbalanced cohorts can overestimate topological predictor effect sizes and that balanced-class sampling is essential for rigorous validation.
\end{abstract}
