...
\begin{table}[htbp]
\centering
\caption{Kernel comparison for activity prediction (500-molecule representative subsample, 5-fold cross-validation). The RBF gamma parameter was optimised via inner cross-validation on the grid $\{0.5, 1.0, 2.0, 5.0\}$. All three kernels are statistically indistinguishable ($p > 0.05$, uncorrected; \cref{tab:qkernel}). }
\label{tab:qkernel}
...
A key finding is that Quantum, RBF (gamma-tuned), and Linear kernels are statistically indistinguishable (\cref{tab:qkernel}; paired $t$-tests all $p > \num{0.05}$). The apparent earlier advantage (QK~0.936 vs.~RBF~0.105) was an artefact of untuned RBF hyperparameters, which vanishes after proper gamma calibration (see \cref{tab:qkernel}).
