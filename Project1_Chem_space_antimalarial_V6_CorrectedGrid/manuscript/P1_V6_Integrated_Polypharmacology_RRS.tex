...
The previously described PP-11 PfDHFR C59R knockout pattern was not retained as a design rule. PP-11's PfDHFR WT estimate in the separate mutant panel was non-binding under the declared threshold, whereas its PfCRT mutant profile was evaluated on the PfCRT denominator; the four-target V5 matrix is not used to define this RRS eligibility decision. The corrected analysis therefore supports a target-specific rather than a mixed-target interpretation of resilience. The complete mutation profile is shown in \Cref{fig:rrs_profiles}; cross-metric relationships are displayed in \Cref{fig:crossmetrics}. These derived figures are reproducible computational outputs; they should not be interpreted as independent structural validation or experimental evidence.
