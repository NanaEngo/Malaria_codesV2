...
## 9 August 2026 — P1 V4 submission gate corrected after V5 structural findings

The V5 migration established that PDB 4GM2 is PfClpR, not the active PfClpP target, and that the corrected PfClpP receptor is 2F6I. The existing V4 484-centroid numerical panel still contains the historical 4GM2 arm; therefore V4 must not be submitted as a PfClpP-validated study. The V5 17-by-4 Vina runs are raw candidate-level evidence and cannot replace the V4 484-centroid panel. The submission gate is consequently **BLOCKED** until a complete, provenance-bound 2F6I revalidation of the V4 centroid panel is executed and independently checked. This decision changes no historical score and promotes no pending V5 consensus/RRS/PNS result.

The safe backport scope is limited to terminology and caveats: 4GM2 analyses are PfClpR-specific, the historical parent-MD `164` label is not accepted as structural PfClpP validation, and no V5 pending-review result is imported into V4. **Mapping resolution (9 August 2026):** the V4 V5–P2 overlay was rebuilt from the V6 source-bound candidate manifest. Row-wise RDKit canonical-SMILES checks now match each `PP-01`–`PP-17` identifier to its V5 four-target manifest rows and its P2 Set-C source row; the provenance sidecar records the V6 audit/validator hashes. The mapping is no longer order-only, but it remains a computational provenance correction and does not authorize biological or downstream consensus claims. The complete replacement run requires the canonical 484 centroid ligand mapping, 2F6I receptor/PDBQT frame verification, target-specific grid definition, fail-closed per-record accounting, and an updated downstream consensus only after independent review. The uniform array `13451` is currently running in the background; merge `13452` remains afterok, while audit `13972` is afterany and will emit an explicit fail-closed verdict even if the array has failed tasks. No partial result is promoted.

**Computational-rescue authorization (9 August 2026).** The first raw array produced 449 `PASS_RAW_VINA` records and 35 explicit failures (10 deterministic embedding failures, 8 non-finite Gasteiger-charge failures, 7 Vina exit-code failures, 9 composite biological-gate failures, and 1 multi-fragment preparation failure). A versioned rescue array is authorized only for these failed IDs. It may use an alternate deterministic conformer seed and higher Vina exhaustiveness, but it must preserve the canonical SMILES, receptor frame, fixed 2F6I triad-centered box, and the original biological gate. It may not enlarge/translate the box, relax containment/contact thresholds, or overwrite the first-run outputs. Rescue outputs are a sensitivity/provenance layer until the combined 484-record aggregate passes independent review; unresolved failures remain explicit and block promotion. The stale dependency job 12966 is superseded by the independent retry/merge chain documented in `Project1_Chem_space_antimalarial_V4_CorrectedGrid/scripts/p1_v4_retry_clpp_2f6i.sbatch`. To remove protocol heterogeneity rather than merely rescue failed records, a separate uniform 484-centroid run with the rescue protocol is also authorized; it writes to `pfclpp_2f6i_484_uniform_seed20260809` and is aggregated only through the parameterized fail-closed aggregator. The uniform run is a candidate replacement panel, not an accepted manuscript result, until independent review.

**Scope of this action.** This rescue removes avoidable software/runtime limitations, not scientific limitations. It does not create experimental IC50/EC50 evidence, make docking equivalent to binding, or open the V6 independent-review gate.

**Implementation status before full execution (9 August 2026).** A V4-local worker (`scripts/p1_v4_revalidate_clpp_2f6i.py`) and SLURM array wrapper (`scripts/p1_v4_revalidate_clpp_2f6i.sbatch`) were created. The worker binds each zero-based row of the 484-row `cluster_representatives_smiles.csv` to genuine PfClpP structure 2F6I, derives the chain-A catalytic-triad center from the PDB, verifies PDB/PDBQT frame equivalence, uses deterministic RDKit/Meeko preparation and seeded Vina, and writes only raw per-centroid provenance. It has no consensus/RRS/PNS side effects. A centroid-0 smoke initially failed because the worker passed a relative ligand path to Vina; the path handling was corrected, and a fresh smoke then passed (`PASS_RAW_VINA`, raw affinity −6.627 kcal mol−1, composite gate passed). This smoke is a technical gate only and is not a manuscript result. The full 484-task array is now authorized as a background raw-data generation step; promotion remains blocked until complete accounting and independent review.
...
>
> **Résultats (17 candidats polypharm × 3 cibles, tous PASS) :** PfClpP/2F6I job 12854 (aff −5.05 à −7.03, contact triade 3.3-8.9 Å) ; PfCRT/6UKJ job 12855 (aff −5.12 à −7.91, contact Y01 3.0-4.1 Å — Y01 = proxy membranaire, caveat documenté, pas une revendication de puissance) ; PfDHFR/7F3Y job 12859 (aff −4.86 à −6.35, contact MTX A702 3.0-3.5 Å). **PfATP4/9N10 DÉBLOQUÉ le 08/08** (job 12864 TERMINÉ) : ancre biologique phospho-site D451 (CSDKTGT 449-458) + charnière DPPR 751-754, centre `[122.712, 125.545, 91.411]` — **17/17 paires passent le gate composite** (centroïde en boîte, contact ancre ≤ 10 Å, ≥ 90 % atomes en boîte), affinités −4.63 à −7.57 kcal/mol → **table 17×4 COMPLETE** (`results/v5_four_target_vina_affinities.csv`, review table `v5_four_target_vina_review_table.json` : 4 cibles COMPLETE_17). Frames CA PDB/PDBQT identiques (rmsd 0) pour chaque récepteur. Register `results/structural_pocket_independent_review.json` v2 : 3 ancres `EVIDENCE_COMPLETE_AWAITING_REVIEW` + PfATP4 `COMPLETE`. **DÉCISION D'AUTEUR 08/08 (soir) : la revue indépendante est OBLIGATOIRE — le bypass interne proposé en cours de session a été REJETÉ** (« il faut faire absolument la revue indépendante »). Register `authorization_mode = INDEPENDENT_REVIEW_REQUIRED`, `internal_work_authorized=false`, gate `p1_v5_consensus_rrs_gate.py` **strict fail-closed restauré** (fonction partagée `check_gate` : exige `accepted_for_full_run=true` ET les 4 cibles `ACCEPTED` ; utilisé aussi par `p1_v5_consensus_scoring.py`). Le consensus 4 cibles calculé sous le bypass rejeté est **VOID** (`v5_consensus_four_target_provenance.json` status VOID, CSV supprimé) — non utilisable. Gate-opener corrigé : `p1_v5_apply_review_signature.py` pose désormais `decision=ACCEPTED` sur chaque cible PASS (le mode soumission peut passer après signature réelle). **Aucun consensus/RRS/PNS/manuscrit V5 avant signature humaine du register.** Manuscrit V5 : notes de statut de migration mises à jour (gate 4 cibles OUVERT, PfATP4 inclus) ; centres PfDHFR corrigés dans `p1_v5_score_rank1_vina.py` et `p1_v5_run_diffdock.py`. Tracking complet : `Project1_Chem_space_antimalarial_V5_CorrectedGrid/project-tracking.md`.
