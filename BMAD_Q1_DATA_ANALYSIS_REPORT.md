...
> **⚠️ RECONCILIATION — ρ=0.312 vs ρ=0.361 (August 1, 2026).** These are **two different features**, not contradictory results, verified by recomputation on `results/p3_rrs_tfp_final.csv` (n=77, A=46, B=31):
> - `H1_count` = number of ring-like features → **ρ = 0.312 (p=0.006)** — canonical headline (§3.9 / main manuscript)
> - `H1_total_persistence` — **definition-dependent**: product `count × mean_pers` → ρ=0.263 (p=0.021, matches `p3_h1_rrs_correlation_final.txt`); sum `max_pers+mean_pers+entropy+count` (Appendix G "corrected computation") → ρ=0.361 (p=0.001)
>
> **Decision:** The main manuscript must use **H1_count (ρ=0.312)** as the single headline correlation. Appendix G's 0.361 (sum definition) and §3.9's 0.263 (product definition) are both secondary aggregate statistics and must NOT be quoted as the headline. The two definitions must be labelled explicitly wherever they appear (SM caption to be checked for the same ambiguity).

> **🔴🔴 CRITICAL — CONFOUNDING ANALYSIS (follow-up E-2, August 1, 2026):** The H1-RRS association does **NOT** survive control for molecular size. Script `scripts/p3_h1_rrs_partial_corr.py` (output `results/p3_h1_rrs_partial_corr.txt`/`.csv`) computed partial Spearman on the rank-transformed data (n=77), controlling for MW, n_rings, Fsp3, H0_count:
>
...
**Vérification finale (02/08/2026) :** main **18 p. / 7 230 mots**, SM **18 p.**, cover letter **1 p.** — tous 0 erreur / 0 réf. indéfinie (compilation séquentielle main→SM). 2 Overfull hbox négligeables (3.8 pt). Ancien titre conservé uniquement dans `outputs/critical-reviews/*.md` (notes d'audit historiques, intentionnel).
