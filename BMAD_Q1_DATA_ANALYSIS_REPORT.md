...
>
> **Résultats (17 candidats polypharm × 3 cibles, tous PASS) :** PfClpP/2F6I job 12854 (aff −5.05 à −7.03, contact triade 3.3-8.9 Å) ; PfCRT/6UKJ job 12855 (aff −5.12 à −7.91, contact Y01 3.0-4.1 Å — Y01 = proxy membranaire, caveat documenté, pas une revendication de puissance) ; PfDHFR/7F3Y job 12859 (aff −4.86 à −6.35, contact MTX A702 3.0-3.5 Å). **PfATP4/9N10 reste BLOQUÉ** (aucune ancre co-cristallisée, CAVITY_EVIDENCE_REQUIRED) et est exclu de la revendication principale 3 cibles. Frames CA PDB/PDBQT identiques (rmsd 0) pour chaque récepteur. Register `results/structural_pocket_independent_review.json` v2 : 3 ancres `EVIDENCE_COMPLETE_AWAITING_REVIEW`, signature humaine `PENDING` — **aucun consensus/RRS/PNS/manuscrit ne peut être calculé avant signature**. Manuscrit V5 : notes de statut de migration mises à jour (gate 3 cibles OUVERT, PfATP4 exclu) ; centres PfDHFR corrigés dans `p1_v5_score_rank1_vina.py` et `p1_v5_run_diffdock.py`. Tracking complet : `Project1_Chem_space_antimalarial_V5_CorrectedGrid/project-tracking.md`.
