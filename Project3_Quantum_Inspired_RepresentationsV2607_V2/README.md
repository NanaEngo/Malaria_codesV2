...
| Distinction / Novelty Claim |
|----------------------------|
| **Physical Pharmacophore Retention** | Tensor Network Embedding (TNE, 192-dim, 5.9× real-atom compression / 15.6× padded) retains 3D physical binding affinity across 19,913 docked molecules. |
| **Quantum Kernel Benchmarking** | Quantum kernel (QKS, 8-qubit) vs. SVM RBF benchmark on multi-target antimalarial polypharmacology ($\ge 2$ *P. falciparum* targets bound). |
| **Topological Rigidity vs. Promiscuity** | TDA $H_1$ persistent homology correlation maps scaffold ring rigidity to biological target promiscuity ($\rho = -0.190$, $N = 17{,}011$). |
| **Scaffold Paradox Resolution** | Persistent homology explains the 92.6% ECFP4-unreachable gap vs. 69.3% scaffold recovery ($H_1$ preservation vs. $H_0$ divergence). |

---

## 3. Scope & Key Parameters

- **Library**: 65,856 molecules generated from African Natural Product chemical space; 19,849 passed quality control and form the primary TFP/TNE/QKS benchmark set.
- **Physical Oracle**: 19,913 molecules docked $\times$ 3 *P. falciparum* targets (`tartarus_output.csv`; distinct from the 19,849-molecule benchmark set).
- **Descriptors**:
  - **TFP**: 12-dim / 78-dim Topological Fingerprints (Vietoris-Rips persistent homology, $H_0, H_1$).
  - **TNE**: Tucker decomposition tensor embeddings (bond_dim=8, 192 dims, 5.9× real-atom compression / 15.6× padded).
  - **QKS**: 8-qubit Quantum Kernel (`pennylane` with `lightning.qubit`/`lightning.gpu` auto-selection via `best_device()`).
- **Statistical Rigour**: 5-fold Stratified Cross-Validation, Bonferroni-corrected Wilcoxon signed-rank tests ($p < 0.01$).
...
├── manuscript/LaTeX/
│   ├── Paper3_Quantum_InspiredV2607.tex  # Main manuscript
...
```bash
# 1. Environment activation (Python 3.12; see requirements.txt)
