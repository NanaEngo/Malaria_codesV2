...
\title{\textbf{Persistent homology resolves the scaffold paradox in AI-generated African antimalarial candidates: a topological and tensor-network fingerprinting study}}
...
\begin{abstract}
Computational drug discovery for neglected diseases often faces a scaffold paradox where candidate molecules achieve extreme structural diversity (e.g., \qty{92.6}{\percent} Tanimoto diversity) while preserving conserved macrocyclic ring systems essential for bioactivity (\qty{69.3}{\percent} scaffold recovery), a global topological phenomenon that classical extended-connectivity fingerprints fail to capture. To resolve this mathematically, we introduce three quantum-inspired representations---Topological Fingerprint (TFP) using persistent homology, Tensor Network Embedding (TNE) for molecular compression (\num{5.9}$\times$ real compression), and simulated Quantum Kernel Scores (QKS)---and evaluate them on \num{19849} African antimalarial natural product candidates. A corrected classical-only benchmark on the full library shows that ECFP4 remains the strongest single descriptor (AUC \num{0.949}), followed by AP (AUC \num{0.941}), BPF (AUC \num{0.939}), FCFP4 (AUC \num{0.920}), MACCS (AUC \num{0.904}), and PHCO (AUC \num{0.897}). The topological and tensor-network descriptors TFP (AUC \num{0.877}) and TNE (AUC \num{0.722}) capture complementary but lower signal. The full hybrid descriptor (TFP+TNE+QK) is currently being rerun after the original full-library run was found to be non-reproducible; the hybrid results reported below are therefore provisional. Cross-paper validation demonstrates that H$_1$ count correlates with resistance resilience scores (Spearman $\rho = 0.312$, $p = 0.0057$, $n=77$ polypharmacological leads; pilot $n=14$, $\rho = 0.947$, $p < 0.0001$), establishing a computationally inexpensive topological predictor of resistance tolerance.
\end{abstract}
...
\noindent\textbf{Relationship to prior antimalarial screening.} This study builds on our initial antimalarial screening study \citep{temgoua2026antimalarial} and addresses four methodological questions: (i)~whether VAE latent space diversity reflects chemical scaffold diversity, examined in \cref{sec:tfp_vae} (TFP versus VAE Latent Space); (ii)~whether enrichment metrics vary with clustering resolution, examined in \cref{sec:tensor_clustering} (Tensor-Based Clustering); (iii)~the applicability domain of computational activity predictions, examined in \cref{sec:applicability_domain} (Applicability Domain Analysis via Quantum Kernel Density); and (iv)~whether the generative computational protocol generalises beyond African natural product seed space, examined in \cref{sec:paradox} (Scaffold Paradox Resolution). In addressing these questions, the present study introduces new molecular representations and provides the topological and tensor-network justification for the initial screening results \citep{temgoua2026antimalarial}.
...
Quantum kernels were implemented using PennyLane \citep{pennylane_2024} with an 8-qubit statevector simulator. The circuit (\cref{alg:qkernel}) encodes the overlap between quantum states prepared from two input vectors using PennyLane's \texttt{IQPEmbedding} template, a data-encoding strategy that upgrades the classical embedding approach based on quantum-generative-models QCBM architectures.

\begin{algorithm}[htbp]
\caption{Quantum kernel circuit for 8-qubit state overlap using PennyLane's \texttt{IQPEmbedding} template.}
\label{alg:qkernel}
\begin{algorithmic}[1]
\REQUIRE Data points $x_1, x_2 \in [-1, 1]^8$
\STATE Apply $\mathrm{IQPEmbedding}(x_1, \mathrm{wires})$
\STATE Apply $\mathrm{IQPEmbedding}^\dagger(x_2, \mathrm{wires})$
...
The resulting 10-dimensional quantum features $q_i$ preserve the non-linear structure of the quantum Hilbert space substantially better than a single density scalar. Ablation experiments reported in preliminary work suggested that removing $q_i$ from the hybrid degraded mean 5-fold CV AUC from approximately 0.842 to 0.608; however, the original full-library hybrid run could not be reproduced from current files, so these ablation numbers are considered provisional until the rerun completes.

Hyperparameter optimisation followed a two-phase protocol. In Phase~1 ($n = 200$), a grid search evaluated quantum kernel parameters across bond dimensions $d \in \{4, 6, 8\}$, repetition layers $r \in \{1, 2, 3, 4, 6\}$, and KPCA components $k \in \{5, 10, 20, 30\}$ (\num{60} combinations in total, of which \num{50} completed before the job terminated), identifying optimal configuration $d = 6, r = 1, k = 30$ with mean 5-fold CV AUC of \num{0.8534} $\pm$ \num{0.0490}. In Phase~2 ($n = 5000$, Jobs 12340--12342), the top three parameter combinations were re-evaluated on $n = 5000$ molecules using 8-core CPU parallelisation per task and JAX CPU backend (\texttt{JAX\_PLATFORMS=cpu}). This confirmed $d = 6, r = 1, k = 30$ as the optimal hyperparameter configuration (AUC \num{0.8283} $\pm$ \num{0.0371}), outperforming alternative candidate configurations ($d=6, r=6, k=30$: AUC \num{0.8121} $\pm$ \num{0.0396}; $d=6, r=6, k=20$: AUC \num{0.8047} $\pm$ \num{0.0354}).

Feature fusion weights $\alpha$, $\beta$, $\gamma$ were optimised by grid search over $\alpha, \beta \in [0.1, 0.8]$ (step 0.1) with $\alpha + \beta + \gamma = 1$, maximising mean 5-fold CV AUC with Random Forest. The provisional grid search on the preliminary run identified weights $\alpha = 0.10$, $\beta = 0.10$, $\gamma = 0.80$, heavily favouring the quantum kernel PCA component; these weights will be re-evaluated once the reproducible hybrid rerun completes. We interpret this weight distribution mechanistically: the 10 kernel PCA components capture non-linear feature interactions in the quantum Hilbert space that are orthogonal to the local topological features (TFP) and global structural compression (TNE). When concatenated, these orthogonal signal axes provide the Random Forest classifier with maximally complementary information. Default weights ($\alpha = 0.40$, $\beta = 0.35$, $\gamma = 0.25$) are used if grid search is unavailable.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{persistence_diagrams_v2.pdf}
\caption{Population-scale persistent homology for the \num{19849}-molecule ANP-space library (Vietoris--Rips filtration on the weighted interatomic distance matrix).
\textbf{(a)} $H_0$, connected components: distribution of per-molecule mean $H_0$ persistence; birth is identically zero for $H_0$ by construction, and the dashed line marks the median (\SI{1.60}{\angstrom}).
\textbf{(b)} $H_1$, rings and loops: hexbin density of per-molecule mean (birth, death) coordinates on linear axes, coloured by median $H_1$ persistence.
\textbf{(c)} $H_2$, cavities and voids: the 587 molecules (\qty{3.0}{\percent} of the library) carrying at least one $H_2$ feature, sized by $H_2$ count and coloured by mean $H_2$ persistence.
In panels (b) and (c) the dotted diagonal $y = x$ marks zero lifetime; vertical distance from it encodes persistence.}
\label{fig:persistence}
\end{figure}

\subsection{Activity prediction: TDA versus classical fingerprints}

\cref{tab:benchmark} presents the corrected activity-prediction performance of the classical and quantum-inspired representations on the full 19,849-molecule set. Classical ECFP4 achieves the highest AUC (\num{0.949}), confirming that local atom-pair environments remain the most discriminative descriptor for this eos80ch-based binary classification task. The full ranking is: ECFP4 (\num{0.949}), AP (\num{0.941}), BPF (\num{0.939}), FCFP4 (\num{0.920}), MACCS (\num{0.904}), PHCO (\num{0.897}), TFP (\num{0.877}), and TNE (\num{0.722}). The TFP and TNE descriptors capture meaningful but lower signal, which is expected because they encode global topological and compressed structural features rather than the local pharmacophoric substructures that dominate activity determination. Notably, the PHCO (2D pharmacophore) descriptor was corrected after a bug in the bit-vector extraction (`GetOnBits`) had previously forced it to random performance (AUC 0.500); the corrected PHCO now attains AUC \num{0.897}, confirming that 2D pharmacophore features do carry discriminative information. The corrected classical-only benchmark was produced by \texttt{p3\_classical\_benchmark\_19849.py} and is documented in the project data-analysis report (\texttt{BMAD\_Q1\_DATA\_ANALYSIS\_REPORT.md}). The hybrid descriptor (TFP+TNE+QK) is discussed in \cref{tab:hybrid}; because the original full-library hybrid run could not be reproduced, those results are shown provisionally while a rerun is in progress.

\begin{table}[htbp]
\centering
\caption{Activity prediction performance by representation method (\num{19849} molecules, 5-fold stratified cross-validation, Random Forest). Classical ECFP4 remains the primary screening gold standard. Values marked `(p)' are provisional pending a full rerun.}
\label{tab:benchmark}
...
\midrule
ECFP4 (baseline) & 0.949 & 0.897 & 0.932 & {---} \\
AP & 0.941 & 0.891 & 0.929 & -0.008 \\
BPF & 0.939 & 0.888 & 0.927 & -0.010 \\
FCFP4 & 0.920 & 0.870 & 0.914 & -0.029 \\
MACCS & 0.904 & 0.861 & 0.908 & -0.045 \\
PHCO & 0.897 & 0.852 & 0.903 & -0.052 \\
TFP (TDA) & 0.877 & 0.838 & 0.897 & -0.072 \\
TNE ($d=8$) & 0.722 & 0.754 & 0.856 & -0.227 \\
QKS (quantum kernel)$^\dagger$ (p) & 0.751 & 0.680 & 0.710 & -0.198 \\
Hybrid$^\ddagger$ (TFP+TNE+QK) (p) & 0.842 & 0.745 & 0.758 & -0.107 \\
TFP + ECFP4 (p) & 0.865 & 0.780 & 0.787 & -0.084 \\
\bottomrule
\end{tabularx}
\vspace{2mm}
...
\footnotesize $^\ddagger$ Hybrid uses kernel PCA (10 components) from the full quantum kernel matrix. The original full-library hybrid run could not be reproduced; the values shown are provisional while a rerun is in progress.
\end{table}

\subsection{Scaffold paradox resolution}\label{sec:paradox}\label{sec:tfp_vae}

Mechanistically, this paradox is driven by the STONED-SELFIES generative framework employed by Temgoua et al. \citep{temgoua2026antimalarial}. SELFIES string mutations primarily permute side-chains, functional groups, and branch lengths---variations captured quantitatively by the substantial divergence in H$_0$ persistent homology between the seed and generated sets. Concurrently, the SELFIES string syntax inherently preserves valid cyclic macro-structures, fused ring systems, and rigid core fragments from the African NP seeds, which manifests mathematically as the preservation of the H$_1$ topology distributions. Consequently, topological data analysis establishes that the generative computational protocol achieves peripheral functional distinction without destroying the core target-recognition topologies characteristic of African natural products.
...
in fine-grained near-neighbour detection. For applicability domain assessment in generative computational protocols where generated molecules are structurally close to training seeds, classical fingerprint-based distance metrics provide a simpler and more reliable baseline, while quantum kernel methods add value primarily for structurally heterogeneous or out-of-distribution regimes.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.70\textwidth]{p3_ga_discriminator_v2.png}
\caption{Chemical similarity vs.\ quantum kernel density discriminator benchmark. AUC-ROC for separating $N$ STONED-SELFIES-generated molecules from $200$ reference seeds, scoring each molecule either by its maximum ECFP4 Tanimoto similarity to a seed (orange) or by its mean quantum kernel density over the reference set (blue). The dashed line marks chance performance (AUC = \num{0.5}) and the shaded band covers performance at or below chance. ECFP4 separates the two sets perfectly at every $N$, while the quantum kernel stays near or below chance (AUC = \numrange{0.425}{0.511}). Simulated on \texttt{lightning.qubit}.}
...
\cref{tab:hybrid} presents the full benchmark of 10 representation methods on the complete library. The values for the hybrid descriptor (TFP+TNE+QK, AUC \num{0.842}) and its ablations are provisional: the original full-library run could not be reproduced from current code or data files, and a rerun is in progress. The table therefore serves as a record of the preliminary hybrid results and the expected structure of the ablation analysis; the final hybrid-vs-classical comparison will be reported once the rerun is complete.

\begin{table}[htbp]
\centering
\caption{Provisional full activity prediction benchmark (10 representation methods, \num{19849} molecules, 5-fold cross-validation). The original full-library hybrid run could not be reproduced; these values are shown as a place-holder while the rerun is in progress. Statistical comparisons vs.~ECFP4 have been removed because the hybrid/ablation values were generated with the previous 0.868 baseline and are not comparable to the corrected 0.949 baseline.}
\label{tab:hybrid}
\begin{tabularx}{\textwidth}{l S[table-format=1.3] S[table-format=1.3] S[table-format=1.3] S[table-format=+1.3]}
\toprule
Method & {AUC} & {Accuracy} & {F1} & {$\Delta$AUC vs.~ECFP4} \\
...
TFP + ECFP4 (p) & 0.865 & 0.780 & 0.787 & -0.084 \\
\midrule
\multicolumn{5}{l}{\textit{Provisional ablation study (Hybrid minus one component):}} \\
Hybrid $-$ TFP (p) & 0.837 & 0.740 & 0.753 & -0.112 \\
Hybrid $-$ TNE (p) & 0.835 & 0.738 & 0.750 & -0.114 \\
Hybrid $-$ QKS (p) & 0.608 & 0.593 & 0.408 & -0.341 \\
\bottomrule
\end{tabularx}
\vspace{2mm}
...
\footnotesize $^\ddagger$ Hybrid uses kernel PCA (10 components) from the full quantum kernel matrix. These hybrid/ablation values are provisional pending a reproducible rerun.
\end{table}

\subsection{Comparison with existing tools}

To validate whether the TNE compression preserves binding-relevant geometry, we leveraged the Tartarus docking benchmark \citep{tartarus_2024}: \num{19913} primary leads docked with QuickVina against three targets (1SYH/PfDHFR, 6Y2F/PfATP4, 4LDE/PfCRT) on an HPC cluster. Random Forest regressors (\num{100} trees, \texttt{random\_state=42}, 5-fold cross-validation) were trained on TNE-only vectors (\num{192}-dim) to predict each target's docking score, with \num{2048}-bit ECFP4 as baseline, executed via the \texttt{p3\_physical\_validation.py} pipeline.

For PfDHFR (1SYH, $N = \num{11878}$), TNE achieved an $R^2$ of \num{0.473} (Spearman $\rho = \num{0.694}$), outperforming classical ECFP4 ($R^2$ = \num{0.451}, $\rho = \num{0.683}$). For the remaining targets, ECFP4 maintained an advantage: 6Y2F/PfATP4 (TNE $R^2$ = \num{0.464}, $\rho = \num{0.654}$ vs.\ ECFP4 $R^2$ = \num{0.570}, $\rho = \num{0.762}$, $N = \num{17075}$) and 4LDE/PfCRT (TNE $R^2$ = \num{0.334}, $\rho = \num{0.585}$ vs.\ ECFP4 $R^2$ = \num{0.515}, $\rho = \num{0.733}$, $N = \num{17074}$). These results confirm that the \num{5.9}$\times$ real-atom mean TNE compression retains substantial binding-relevant information, competitive with classical \num{2048}-bit fingerprints for specific targets (\cref{fig:tne_regression}).

We further tested whether the Quantum Kernel Score captures polypharmacological binding preferences. Compounds were labelled as polypharmacological if they bound $\geq 2$ targets at $\Delta G \leq -\SI{7.0}{\kcalmol}$ (\qty{10.3}{\percent} prevalence in the merged $N=\num{17011}$ set). The QKS (IQPEmbedding, 8 qubits, \texttt{lightning.qubit}) achieved AUC \num{0.747} vs.\ RBF \num{0.737} in \num{5}-fold cross-validation on a stratified subsample, demonstrating that the quantum kernel extracts polypharmacologically relevant information beyond classical kernel baselines (\cref{tab:qkernel}).

Finally, topological features from persistent homology showed highly significant correlations with multi-target binding promiscuity across $N = \num{17011}$ molecules (\cref{fig:tda_promiscuity}). H$_0$ count (Spearman $\rho = -\num{0.248}$, $p = 2.54 \times 10^{-236}$), H$_0$ entropy ($\rho = -\num{0.243}$, $p = 1.18 \times 10^{-226}$), and H$_1$ entropy ($\rho = -\num{0.190}$, $p = 1.91 \times 10^{-137}$) all negatively correlate with the number of targets bound. This provides a clear topological mechanism: lower topological complexity in ring structures (H$_1$) and structural components (H$_0$) endows candidate molecules with optimal conformational adaptability across multiple malaria targets, reducing steric clash penalties.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.90\textwidth]{p3_tne_regression_v2.png}
\caption{Tartarus docking validation of Tensor Network Embeddings. Random Forest regression performance (\num{100} trees, 5-fold cross-validation) of the \num{192}-dimensional TNE descriptor against the \num{2048}-bit ECFP4 baseline when predicting docking scores for three \textit{Plasmodium} targets. \textbf{(a)} Coefficient of determination $R^2$. \textbf{(b)} Spearman rank correlation between predicted and computed scores. TNE exceeds ECFP4 for PfDHFR ($N = \num{11878}$) and trails it for PfATP4 ($N = \num{17075}$) and PfCRT ($N = \num{17074}$), retaining substantial binding-relevant information at \num{5.9}$\times$ real-atom compression.}
\label{fig:tne_regression}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{p3_tda_promiscuity_v2.png}
\caption{Topological predictors of multi-target binding promiscuity. Spearman correlation between persistent-homology features and the number of \textit{Plasmodium} targets bound at $\Delta G \leq -\SI{7.0}{\kcalmol}$, across $N = \num{17011}$ molecules. The \num{14} interpretable persistence statistics with the largest $|\rho|$ are shown, ordered by $\rho$, with the corresponding Spearman $p$-values listed at right. Lower topological complexity accompanies broader target coverage, the strongest negative associations being H$_0$ count ($\rho = -0.248$), H$_0$ entropy ($\rho = -0.243$) and H$_1$ entropy ($\rho = -0.190$), all $p < 10^{-137}$. A further \num{45} vectorised persistence-image and Betti-curve components were also tested ($\rho$ between $-0.111$ and $+0.139$); they are omitted here because their individual indices are not chemically interpretable. The underlying analysis reports no confidence intervals, so none are drawn.}
\label{fig:tda_promiscuity}
\end{figure}
...
\cref{tab:benchmark} confirms that classical ECFP4 remains the recommended baseline for primary screening, achieving the highest AUC (\num{0.949}) across the full library. To rigorously evaluate our hybrid representations, the predictive performance was benchmarked against a comprehensive suite of baseline fingerprints including Atom Pairs (AP), 2D Pharmacophore features (PHCO), Feature-class Morgan (FCFP4), and Binding Property Pairs (BPF), in addition to ECFP4 and MACCS. The corrected classical-only benchmark shows that ECFP4, AP, BPF, FCFP4, MACCS and PHCO all achieve AUC $\geq 0.897$, while TFP (AUC 0.877) and TNE (AUC 0.722) capture complementary but weaker signal. The previously reported PHCO value of 0.500 was an artefact of a bug in bit-vector extraction (`GetOnBits`); after correction, PHCO performs comparably to the other classical fingerprints (AUC 0.897). The hybrid (TFP+TNE+QK) and its ablations are discussed only provisionally in \cref{tab:hybrid}, because the original full-library hybrid run could not be reproduced and a rerun is in progress. TFP adds value specifically for compounds with complex ring systems ($\geq 3$ fused rings) where topological features diverge most from atom-pair representations, but it does not improve overall classification relative to ECFP4. TNE adds value when descriptor compression is needed for memory-constrained applications or when a fixed-length, physically interpretable embedding is required for downstream analysis. The quantum kernel applicability domain analysis (\cref{fig:domain}) adds independent value by identifying African NP compounds that fall outside the training distribution of existing activity prediction models, highlighting the need for NP-specific model development regardless of whether QKS improves classification. However, the GA-style discriminator benchmark (\cref{tab:ga_discriminator}, \cref{fig:ga_discriminator}) reveals an important boundary condition: when generated molecules are structurally near-identical to training seeds (as in STONED-SELFIES mutations with $\leq 2$ character changes), a simple ECFP4 Tanimoto distance achieves perfect discrimination (AUC = \num{1.000}) while the quantum kernel density achieves near-random to below-random performance (AUC = \numrange{0.425}{0.511}).

\subsection{Mechanistic Explanation for Classical Fingerprint Superiority}

The overall ranking of ECFP4 (AUC \num{0.949}) at the top, followed by AP (\num{0.941}), BPF (\num{0.939}), FCFP4 (\num{0.920}), MACCS (\num{0.904}) and PHCO (\num{0.897}), reflects the dominance of local atom environments at radius 2 for this activity prediction task. These local pharmacophoric patterns---hydrogen bond donors/acceptors, aromatic rings, hydrophobic groups---are the primary determinants of target engagement. TDA captures global topology (ring systems, molecular cavities) that is correlated with but not directly predictive of binding affinity; on this library it achieves AUC 0.877. TNE compresses the molecular tensor along low-rank modes that may not align with activity-relevant features (AUC 0.722). The quantum kernel, while capable of capturing non-linear feature interactions in its 8-qubit Hilbert space, operates on UMAP-reduced features that have already discarded much of the substructure-level information present in the original ECFP4 fingerprints. Whether the hybrid (TFP+TNE+QK) can recover a large fraction of the ECFP4 signal remains to be established by the reproducibility rerun currently in progress; the provisional hybrid AUC of 0.842 reported in \cref{tab:hybrid} is not used to support any conclusion until the rerun completes.

This interpretation is consistent with recent spectral analysis of molecular kernels \citep{jamali2026spectralanalysismolecularfeatures}, which showed that ECFP-based kernels exhibit a positive correlation between spectral richness and generalization, while 3D kernel methods show negative or inconclusive correlations. Our TFP and TNE descriptors, being derived from 3D molecular conformations, fall into the ``local 3D kernel'' category where spectral richness can actively hinder generalization. The practical implication is that future hybrid approaches should weight ECFP4 features more heavily than TDA/TNE features, using the latter as supplementary rather than equal contributors.

\subsection{Kernel Indistinguishability After Proper Classical Tuning}

A key finding of this benchmark is that Quantum, RBF (gamma-tuned), and Linear kernels are statistically indistinguishable for this molecular activity prediction task (QK AUC~0.751 vs.~RBF~0.701 vs.~Linear~0.721; paired $t$-tests all $p > \num{0.05}$; \cref{tab:qkernel}). An earlier benchmark with default RBF gamma appeared to show a striking quantum advantage (QK AUC~0.936 vs.~RBF~0.105, $p = \num{0.0003}$); this was an artefact of the RBF kernel assigning near-uniform similarity values when gamma is far from optimal, producing AUC below random ($< 0.5$). Once the RBF gamma is properly optimised via inner cross-validation on the grid $\{0.5, 1.0, 2.0, 5.0\}$, the classical kernel recovers to AUC~0.701, eliminating the apparent quantum advantage.

Preliminary work suggested that the hybrid (TFP+TNE+QK) could achieve AUC~0.842, substantially higher than a simpler single-scalar quantum kernel density approach (AUC~0.691). However, the original full-library hybrid run could not be reproduced from current files, so the ablation study (\cref{tab:hybrid}) is shown provisionally and no conclusion is drawn until the rerun completes. If confirmed, the key driver would be replacing the single QK density scalar with 10 kernel PCA components from the full kernel matrix, which is expected to preserve more of the non-linear structure of the quantum Hilbert space.

\subsection{Computational Cost and Scalability}

TDA computation for \num{19849} molecules required approximately \SI{6.4}{\minute} on a 12-core workstation using 8 parallel workers (Joblib LokyBackend), dominated by persistent homology computation of 3D conformers. Each molecule required approximately \SI{19}{\milli\second} for full TFP extraction. Tucker decomposition for the same library required approximately \SI{20}{\minute} (sequential, single-threaded per molecule). Per-molecule TNE extraction was \SIrange{50}{60}{\milli\second}. The full computational protocol (TDA + TNE) for \num{19849} molecules ran in under 30 minutes on a standard workstation, demonstrating practical scalability for libraries on the order of $10^4$ compounds. Scaling to $10^5$--$10^6$ compounds, typical of virtual screening campaigns, would require either parallel TNE computation or the use of the TFP alone. Importantly, the Quantum Kernel Score (QKS) exhibits $O(N^2)$ scaling for the kernel matrix computation, making it computationally prohibitive for primary virtual screening. Given the corrected classical ECFP4 baseline (AUC 0.949) and the provisional hybrid value (AUC 0.842), the substantial computational overhead of the QKS is not justified for active compound prioritization unless the hybrid rerun shows a clear, reproducible advantage. Instead, these quantum-inspired descriptors should be strictly reserved for retrospective mechanistic analysis and small-scale lead optimization, where their mathematically interpretable feature extraction provides distinct non-linear insights.
...
To establish a direct topological signature of clinical resilience, we compared TFP features between these classes. An initial pilot of 14 polypharmacological leads suggested a strong relationship (Spearman $\rho = 0.947$, $p < 0.0001$); expanding the cohort to $n = 77$ compounds yielded $\rho = 0.312$ ($p = 0.0057$, H$_1$ count), confirming that H$_1$ count is significantly associated with resistance resilience while showing the expected attenuation as sample size increases. In line with this H$_1$ count association, Class~A compounds also exhibit systematically higher H$_1$ persistence (mean $3.74 \pm 0.34$~\AA\ versus $1.73$~\AA\ for Class~D). The Class~D subset in the pilot contains only a single compound ($n = 1$), so the pilot mean should be interpreted cautiously. The attenuated but significant expanded correlation supports the hypothesis that greater ring topological complexity confers conformational stability advantageous against resistance mutations, though the relationship is weaker than the pilot suggested.

This correlation presents an interesting biophysical paradox: why does the TFP perform poorly at predicting absolute antimalarial activity (AUC 0.587) yet show a strong pilot correlation with resistance resilience ($\rho = 0.947$)? The explanation lies in what topological persistence (specifically H$_1$) measures: cycle rigidity. Having a rigid core conformation (high H$_1$ persistence) is insufficient on its own to establish high-affinity binding (which is dominated by local pharmacophoric interactions captured by ECFP4). However, for compounds that are already established high-affinity binders (like the 14 candidates validated in MD), a highly rigid cycle system reduces the conformational entropy penalty upon binding. This rigidity essentially "locks" the ligand in its bioactive pose, stabilizing the bound complex even when active-site mutations alter the shape of the pocket, thereby conferring resilience.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{h1_rrs_class_violin_v2.png}
\caption{Joint topological analysis linking TFP persistence features with the molecular dynamics resistance classification \citep{temgoua2027md}. \textbf{(a)} Mean H$_1$ persistence and \textbf{(b)} H$_1$ count for the expanded cross-paper cohort ($n = 77$), split by RRS class: Class~A (resistance-resilient, $n = 46$) and Class~B ($n = 31$). Individual compounds are overlaid as points, the thick bar spans the interquartile range, and the white marker is the median. Of the \num{500} compounds processed, \num{423} carried no RRS classification and are excluded. Across this cohort H$_1$ count is significantly associated with the Resistance Resilience Score (Spearman $\rho = 0.312$, $p = 0.0057$); the pilot cohort ($n = 14$, $\rho = 0.947$, $p < 0.0001$) is reported in the text for context and should be read cautiously, because its Class~D subset contains a single compound. Classes~C and~D do not occur in the expanded cohort and are therefore absent here.}
\label{fig:joint_h1_rrs}
\end{figure}

This finding positions ring topology (H$_1$) as a computationally inexpensive descriptor for prioritising resistance-resilient candidates in future generative campaigns. We plan to expand this joint analysis with dynamic persistent homology on full MD trajectories in a follow-up study, pending completion of the polypharmacological MD simulations.

\subsection{Limitations}

Several limitations constrain the interpretation of our results. First, TDA does not outperform ECFP4 overall in activity prediction (\cref{tab:benchmark}), a valid negative result consistent with the known effectiveness of atom-pair representations for synthetic drug-like molecules. TFP adds value primarily at the high-complexity tail of the distribution --- molecules with $\geq 3$ fused rings or $\geq 2$ stereocentres, where topological features diverge most from atom-pair representations. Second, activity labels are computational predictions from Ersilia ML models rather than experimental measurements, limiting biological validity. All claims regarding ``activity,'' ``polypharmacology,'' and ``resistance tolerance'' should therefore be understood as predictions within a computational framework, pending experimental validation. Third, the cross-paper H$_1$ persistence--Resistance Resilience Score (RRS) relationship was first observed in a small pilot ($\rho = 0.947$, $n = 14$) and subsequently confirmed in an expanded cohort ($\rho = 0.312$, $p = 0.0057$, $n = 77$). The pilot involved small sample sizes across resistance classes (Class~A: $n = 3$, Class~D: $n = 1$) and should be treated as hypothesis-generating; the expanded cohort provides stronger evidence but the relationship remains attenuated and requires further validation. The quantum kernel circuit was evaluated on a classical simulator with 8 qubits; scaling to more qubits increases computational cost exponentially. However, we upgraded the quantum kernel computational protocol to use the PennyLane \texttt{IQPEmbedding} template --- a classically hard-to-simulate data encoding strategy --- mitigating the limitations of simpler $R_Y$-$R_Z$ alternating layers. Furthermore, the collapse of the quantum discriminator's performance compared to classical Tanimoto (AUC 0.425--0.511 versus 1.000) represents an inherent resolution bottleneck of the UMAP dimensionality reduction (to 8 dimensions) required for simulation on 8 qubits. While classical fingerprints excel at detecting local atom-level changes (such as 2-character SELFIES mutations), the quantum kernel remains suited for defining global applicability domains across macroscopically heterogeneous spaces. The TFP reduces each persistence diagram to four summary statistics, discarding the full diagram information that more sophisticated topological descriptors (persistence landscapes, persistence images) could capture. However, the enriched TFP (32 features including persistence images and Betti curves) showed no improvement over the 12-feature TFP baseline (AUC 0.587), suggesting that additional topological features do not add discriminative power for this dataset. Finally, the library is biased toward the chemical space of its 396 African NP and 454 synthetic drug seeds; generalization to other chemical domains remains to be tested.

The QKS benchmark revealed an important methodological lesson: an initial run with the RBF gamma at its default value yielded AUC~0.936 for the quantum kernel versus AUC~0.105 for RBF ($p = \num{0.0003}$), appearing to demonstrate a decisive quantum advantage. Re-running with the RBF gamma tuned by inner cross-validation (grid $\{0.5, 1.0, 2.0, 5.0\}$) produced AUC~0.751 versus AUC~0.701 versus AUC~0.721 for quantum, RBF, and linear kernels respectively (all $p > \num{0.05}$), confirming that the apparent advantage was an artefact of hyperparameter miscalibration. This result underscores the importance of rigorous baseline tuning in quantum kernel benchmarks. The corrected classical-only benchmark (10 representation methods, \num{19849} molecules, 5-fold cross-validation) shows ECFP4 at AUC~0.949 (\cref{tab:benchmark}). The provisional hybrid value of AUC~0.842 is discussed in \cref{tab:hybrid}, but no conclusion is drawn until the rerun completes.

Despite these limitations, the most novel finding of this study is that H$_1$ count is significantly associated with resistance tolerance across antimalarial candidates (Spearman $\rho = 0.312$, $p = 0.0057$, $n = 77$; pilot $\rho = 0.947$, $n = 14$; \cref{fig:joint_h1_rrs}) --- suggesting a potential topological link to drug quality that is invisible to classical fingerprints and computationally inexpensive to compute. This cross-paper result provides a preliminary indication that persistent homology captures structural determinants of resistance tolerance that neither ECFP4 nor any classical descriptor can access, serving as a valuable hypothesis-generating basis for future large-scale validations. Two additional contributions support this core finding: (i)~TFP resolves the scaffold paradox via H$_1$/H$_0$ decomposition, proving that VAE generative expansion preserves ring topology while varying substituent chemistry; and (ii)~TNE achieves \num{5.9}$\times$ real-atom compression with competitive Tartarus regression ($R^2 = 0.473$ for PfDHFR). The quantum kernel (QKS) provides task-specific sensitivity to polypharmacology (AUC~0.747 vs.~RBF~0.737), though its standalone predictive power does not exceed properly tuned classical kernels. Classical ECFP4 (AUC~0.949) remains the recommended primary screening baseline. The hybrid (AUC~0.842, $p = \num{0.111}$) is only provisionally reported in \cref{tab:hybrid} and is not used to support any conclusion until the reproducible rerun completes. We therefore position these quantum-inspired representations not as replacements for classical fingerprints, but as complementary diagnostic tools that reveal topological dimensions of molecular quality --- particularly resistance tolerance --- that are inaccessible to conventional cheminformatics.
...
We introduce three quantum-inspired molecular representations --- the Topological Fingerprint (TFP), Tensor Network Embedding (TNE), and Quantum Kernel Score (QKS) --- and evaluate them on a library of \num{19849} antimalarial candidates derived from African natural product chemical space. TFP, derived from Vietoris--Rips persistent homology, resolves the scaffold paradox in the library (\qty{92.6}{\percent} Tanimoto distinction vs.\ \qty{69.3}{\percent} scaffold recovery) by demonstrating that H$_1$ ring topology is preserved (TFP mean H$_1$ count 3.61) while H$_0$ connectivity diverges (mean H$_0$ count 38.05) --- a systematic application of persistent homology to explain a chemical space data inconsistency. TNE compresses the molecular tensor to a \num{192}-element core descriptor (mean real ratio \num{5.9}$\times$ based on \num{38} mean atoms; bond dimension~8; mean reconstruction error 0.113) while preserving activity-relevant information. The full computational protocol processed \num{19849} molecules in \SI{6.4}{\minute} (TDA) and \SI{20}{\minute} (TNE) on a standard workstation, demonstrating practical scalability. The kernel benchmark with a properly tuned RBF baseline found that all three kernels---Quantum (AUC~0.751), RBF (AUC~0.701), and Linear (AUC~0.721)---are statistically indistinguishable (all $p > \num{0.05}$), showing that an earlier apparent quantum advantage was an artefact of untuned RBF hyperparameters. The corrected classical-only benchmark (10 representation methods, \num{5}-fold CV, \num{19849} molecules) confirms that classical ECFP4 (AUC~0.949) is the strongest single descriptor. The provisional hybrid representation value (AUC~0.842, $p = \num{0.111}$) is not used to draw conclusions until the reproducible rerun completes; if confirmed, it would position quantum-inspired methods as effective complementary tools alongside classical fingerprints. The quantum kernel applicability domain analysis identifies African NP compounds that fall outside the training distribution of existing activity prediction models. All representations, benchmark results, and analysis scripts are deposited on Zenodo and GitHub under open-source licences.
...
All data supporting the conclusions of this study are deposited on Zenodo (DOI: 10.5281/zenodo.19608875, reserved July 2026) and mirrored at 
\url{https://github.com/Vital-Sao/Malaria_codes} under the MIT licence. The full deposit includes: (1)~TFP feature matrices for all \num{19849} library 
molecules (H$_0$/H$_1$ persistence diagrams and summary statistics); (2)~TNE embeddings (\num{192}-element Tucker core descriptors, bond dimension 8, 
reconstruction errors); (3)~quantum kernel matrices (8-qubit IQPEmbedding, PennyLane 0.45.1); (4)~all benchmark CSV files (5-fold CV splits, AUC scores per 
descriptor); (5)~the cross-paper H$_1$/RRS analysis dataset (\texttt{p3\_polypharm\_tfp\_rrs.csv}); and (6)~all analysis scripts. The repository will be made 
...
Myke Vital Sao Temgoua: conceptualization, methodology, 
software, formal analysis, investigation, data curation, writing -- original draft. Jean-Pierre Tchapet Njafa: conceptualization, methodology, software, 
supervision, writing -- review \& editing. Serge Guy Nana Engo: methodology, supervision, writing -- review \& editing. Penabei Samafou: data curation, writing 
-- review \& editing. Wilfred Fon Mbacham: conceptualization, validation, writing -- review \& editing. All authors read and approved the final manuscript.
...
During the preparation of this work the authors used AI assistants (Claude, Anthropic) to support code development, data analysis scripting, and manuscript 
drafting and editing. The authors reviewed and edited all AI-assisted output and take full responsibility for the content of this article. No generative AI tool 
is listed as an author. Computational results, statistical analyses, and all quantitative claims were generated, verified, and deposited by the authors.
