...
The primary benchmark dataset consists of \num{19849} molecules drawn from a library of \num{65856} candidates generated from 396 African natural products and 454 synthetic drugs by Cheese API similarity search and STONED-SELFIES generative expansion, as described previously \citep{temgoua2026antimalarial}. Binary activity labels were obtained from the Ersilia model eos80ch (asexual blood stage activity) applied to the full \num{65856}-molecule generated library. The activity threshold was set at the score distribution median (0.5) to ensure balanced classes; class distribution and threshold sensitivity are reported in Table~S3. For reference comparison, we used DrugBank 5.1.10 \citep{drugbank_2018}, COCONUT \citep{coconut_2021}, and the African Natural Products Database (ANPDB, \num{11000} compounds) \citep{anpdb_2026}.
...
The headline Hybrid AUC of \num{0.842} reported throughout this manuscript is the full \num{19849}-molecule benchmark value (default configuration, bond dimension $d = 8$, $n_{\text{rep}} = 2$, $n_{\text{kpca}} = 20$). It is distinct from the development-subsample values reported in the Supplementary Material: the $n = 200$ Phase-1 grid search (best configuration $0.853 \pm 0.049$) and the $n = 5000$ Phase-2 re-benchmark of the optimal configuration ($d = 6$, $n_{\text{rep}} = 1$, $n_{\text{kpca}} = 30$; AUC $0.8283 \pm 0.0371$). These intermediate runs inform hyperparameter selection and are not the full-benchmark headline.
...
\label{tab:tda_stats}
\begin{tabularx}{0.7\textwidth}{l S[table-format=1.4] S[table-format=1.4] S[table-format=1.4] S[table-format=1.4]}
\toprule
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{persistence_diagrams.pdf}
\caption{Vietoris-Rips persistence diagrams for the 19,849-molecule ANP-space library (weighted interatomic distance matrix).
\textbf{Left} ($H_0$, connected components): kernel density estimate of mean persistence across all molecules; birth is identically zero for $H_0$ by construction.
\textbf{Centre} ($H_1$, rings/loops): hexbin density of per-molecule mean (birth, death) coordinates on log--log axes, coloured by median persistence; the orange circle marks a high-persistence ANP (top-20\%, $\bar{p}=0.78$) and the blue triangle a low-persistence synthetic-like molecule (bottom-10\%, $\bar{p}=0.24$).
\textbf{Right} ($H_2$, cavities/voids): scatter of the 587 molecules (3.0\%) that carry at least one $H_2$ feature, sized by $H_2$ count and coloured by mean $H_2$ persistence.
Points above the diagonal $y = x$ correspond to topological features with positive lifetime; vertical distance from the diagonal encodes persistence.}
\label{fig:persistence}
\end{figure}

\subsection{Activity prediction: TDA versus classical fingerprints}

\cref{tab:benchmark} presents the activity prediction performance of all representations. Classical ECFP4 achieves a high AUC (\num{0.868}), while 2D pharmacophore features (PHCO), once a sparse-bit-vector extraction bug was corrected (Supplementary Material), attain AUC \num{0.943}---slightly above ECFP4---confirming that pharmacophore-based bit vectors carry strong discriminative information for this eos80ch-based binary classification task when properly encoded. The quantum-inspired representations rank as follows: Hybrid (\num{0.842}), QKS (\num{0.751}), TNE (\num{0.606}), and TFP (\num{0.587}). The standalone TFP and TNE descriptors perform substantially below the classical baselines, which is expected given that they encode global topological and compressed structural features respectively, rather than the local pharmacophoric substructures that dominate activity determination. We note that this ranking depends on the evaluation configuration: the Supplementary Material reports a class-weighted topological benchmark in which PersStats (22 persistence-statistic features) and TFP-12 reach AUCs of \num{0.873} and \num{0.867}, respectively---comparable to ECFP4 (\num{0.868}; $\Delta\text{AUC}=+0.005$, $p > \num{0.05}$)---indicating that topological features carry ECFP4-comparable discriminative signal once the \qty{75.9}{\percent}/\qty{24.1}{\percent} class imbalance is addressed via class-weighted training. The Hybrid descriptor (AUC \num{0.842}) narrows the gap to within \num{0.026} AUC units of ECFP4, which we analyse further in \cref{tab:hybrid}.

\begin{table}[htbp]
\centering
\caption{Activity prediction performance by representation method (\num{19849} molecules, 5-fold stratified cross-validation). Classical ECFP4 remains the primary screening gold standard.}
\label{tab:benchmark}
\begin{tabularx}{0.8\textwidth}{l S[table-format=1.3] S[table-format=1.3] S[table-format=1.3] S[table-format=+1.3]}
\toprule
...
\bottomrule
\multicolumn{5}{m{0.8\textwidth}}{
\footnotesize $^\dagger$ QKS evaluated on a representative subsample (10,000~mol); the RBF baseline was optimised by inner cross-validation (see 
\cref{tab:qkernel}).}\\
\multicolumn{5}{m{0.8\textwidth}}{\footnotesize $^\ddagger$ Hybrid uses kernel PCA (10 components) from the full quantum kernel matrix.}
\end{tabularx}
\end{table}
