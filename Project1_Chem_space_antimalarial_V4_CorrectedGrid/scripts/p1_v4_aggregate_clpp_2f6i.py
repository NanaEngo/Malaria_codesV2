#!/usr/bin/env python3
"""Fail-closed aggregator for the V4 PfClpP/2F6I 484-centroid array.

The aggregator independently re-derives identity, file hashes, and the
composite pose gate from raw files. It never trusts worker-reported gate values
alone and never computes consensus, MPO, RRS, PNS, or manuscript tables.
"""
...
RAW = Path(os.environ.get("P1_CLPP_RAW_DIR", str(ROOT / "Project1_Chem_space_antimalarial_V4_CorrectedGrid/results/pfclpp_2f6i_484_raw")))
OUT = Path(os.environ.get("P1_CLPP_AGG_OUT", str(ROOT / "Project1_Chem_space_antimalarial_V4_CorrectedGrid/results/pfclpp_2f6i_484_aggregate")))
...
def fail(message: str) -> int:
    ...
    (OUT / "aggregation_failure.json").write_text(json.dumps({
        "schema": "p1-v4-clpp-2f6i-aggregate/v2",
...
def main() -> int:
    # Never promote an incomplete array: the expected canonical panel is fixed at 484.
    if not RAW.is_dir():
    ...
    symlink_dirs = [p for p in observed_dirs if p.is_symlink()]
    if symlink_dirs:
        return fail(f"symlinked centroid directories are forbidden: {symlink_dirs[:4]}")
    if unexpected:
        return fail(f"unexpected centroid directories: {sorted(str(p) for p in unexpected)[:4]}")
    if missing_dirs:
        return fail(f"incomplete raw array: {len(missing_dirs)} centroid directories missing")
    ...
    for i, row in enumerate(rows):
        ...
        if failure_path.exists():
            failures.append((i, "worker failure.json present")); continue
        if not result_path.is_file():
            failures.append((i, "result.json missing")); continue
        try:
    ...
    if len(records) != 484 or failures:
        return fail(f"incomplete/invalid array: {len(records)}/484 independently PASS; failures={len(failures)}; first failures={failures[:8]}")
    OUT.mkdir(parents=True, exist_ok=True)
    ...
    summary = {"schema": "p1-v4-clpp-2f6i-aggregate/v2",
               "status": "RAW_ARRAY_COMPLETE_PENDING_INDEPENDENT_REVIEW",
               "target": "PfClpP", "pdb_id": "2F6I", "records": len(records),
               "source_smiles_sha256": source_sha,
               "aggregate_csv_artifact": str(csv_path),
               "aggregate_csv_sha256": sha256(csv_path),
               "created_utc": datetime.now(timezone.utc).isoformat(),
