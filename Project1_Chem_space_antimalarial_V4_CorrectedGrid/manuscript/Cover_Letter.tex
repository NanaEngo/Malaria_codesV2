...
The second contribution is an explicit target-anchored polypharmacology layer. A predefined cohort of \num{17} polypharmacology candidates was evaluated against PfDHFR, PfCRT, PfClpP, and PfATP4 using corrected target-specific anchors and fixed Vina boxes. The \num{68} candidate--target computations passed the declared pose-and-anchor gate, yielding preliminary mean docking scores of \qty{-5.76}{\kilo\calorie\per\mole} (PfDHFR), \qty{-6.19}{\kilo\calorie\per\mole} (PfCRT), \qty{-5.94}{\kilo\calorie\per\mole} (PfClpP), and \qty{-6.31}{\kilo\calorie\per\mole} (PfATP4) in a raw, unreviewed computational artifact rather than experimental binding free energies.
