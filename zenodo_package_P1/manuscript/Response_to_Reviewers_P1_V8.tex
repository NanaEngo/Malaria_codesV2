...
% minting (author account action): repo public, MIT license, release tag v1.0.0,
% staged package zenodo_package_P1/ (sha256-verified). Insert the DOI in Data
% Availability (main) and SM reproducibility before resubmission.
...
sha256-verified manifest). A permanent Zenodo DOI is being minted for this release and will be
inserted into the Data Availability statement (main) and the SI reproducibility section
(\Cref{SM-sec:sm_repro}) before resubmission. In parallel, the WT/mutant score table required to
...
\texttt{v1.0.0}, GitHub release), with the staged Zenodo package ready; only the Zenodo DOI
minting (author account action) remains before resubmission.
