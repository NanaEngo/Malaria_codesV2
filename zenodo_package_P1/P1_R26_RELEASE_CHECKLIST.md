...
| 1.2 | Commit all canonical V8 changes on `data-results` | `git status --short` must show only intended files; `git diff --check` clean | ⬜ |
...
| 1.5 | **Add a software license** | e.g. MIT or CC-BY-4.0 (recommended for code+data mix); add `LICENSE` at repo root; state it in the README | ⬜ |
...
| 3.3 | Upload the staged package + tarball | Follow P5 (`zenodo_package_20260827/` + tarball, 31/31 verified) | ⬜ |
