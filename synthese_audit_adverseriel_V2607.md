...
| **Projet 3 (P3) — code** | Bug du descripteur PHCO (AUC=0.500 exactement aléatoire = feature nulle, pas un résultat négatif authentique). | Corriger l'extraction via `GetOnBits()` et relancer le benchmark hybride. | ✅ PHCO corrigé (GetOnBits). AUC rétabli 0.500→0.801. Benchmark 5K mols en cours (job 7943). |
