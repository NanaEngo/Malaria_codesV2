...
|---|---|---|---|
| **Projet 1 (P1)** | Cohérence des grilles de docking et redocking MTX. | Grilles V2 déjà recalculées (✅) pour le panel 1815-mols ; **la quasi-totalité des autres résultats P1/P2/P3 est datée avant le diagnostic de grille (16 juillet)** et doit être revérifiée — cartographie complète en §3.1c. **Complexités structurales supplémentaires** (indépendantes du recentrage) pour PfCRT (isoforme 7G8 sud-américaine, pas la plus africaine-pertinente) et PfATP4 (structure apo sans ligand, cycle conformationnel E1/E2) — revue de littérature en §3.3. | ✅ Recalcul fait (panel 1815) / 🔴 ~15 analyses en aval à revérifier / 🟡 Limites structurales PfCRT/PfATP4 à documenter |
| **Projet 2 (P2)** | Biais de centralité PfCRT et MM-GBSA de PfATP4. | Corriger l'imputation de centralité PfCRT par la moyenne NetworkX et documenter l'exclusion MM-GBSA. | 🔄 Code prêt, à relancer |
| **Projet 3 (P3) — écriture** | Ratio de compression TNE gonflé (15.6× vs 5.9× réel) ; incohérence du chiffre QKS canonique (0.751/0.701 vs 0.936/0.105). | Réécrire l'Abstract/§3.3 avec le ratio réel (5.9×) et fixer un seul chiffre QKS sourcé sur `p3_qks_summary.txt`. | ✅ Aucune recompute requise |
| **Projet 3 (P3) — code** | Bug du descripteur PHCO (AUC=0.500 exactement aléatoire = feature nulle, pas un résultat négatif authentique). | Corriger l'extraction via `GetOnBits()` et relancer le benchmark hybride. | 🔄 Code prêt, à relancer |
| **Projet 3 (P3) — statistique** | Faiblesse statistique de la corrélation H₁/RRS ($N=14$, classe D à $n=1$). | Court terme : test de permutation + bootstrap exact sur les 14 points existants. Moyen terme (levier prioritaire) : relancer TDA+QKS sur les 1 815 molécules de la congeneric series P1 pour sortir de la petite taille d'échantillon. | 🔄 HPC requis (~4h/job) |
...
|---|---------|-------------|----------|--------|
| C1 | Sélection des 20 candidats (docking consensus 4 cibles) | §2.1 | 4 cibles incl. PfDHFR | 🔴 **Critique** — la composition même du top-20 pourrait changer si PfDHFR est re-scoré avec V2 |
| C2 | Polypharmacologie / promiscuité (§2.2) | §2.2 | 4 cibles | 🔴 Hérite de C1 |
| C3 | Corrélation DiffDock–Vina par cible (§2.3) | §2.3 | PfDHFR notamment | 🔴 Corrélation calculée contre un Vina potentiellement erroné pour PfDHFR |
| C4 | Consensus de docking par cible (§2.4) | §2.4 | 4 cibles | 🔴 Idem C3 |
| C5 | Construction des complexes MD, dont 201_PfDHFR (200 ns production) | §2.9, 14 juil. | PfDHFR | 🔴 **Très critique** — le BMAD confirme explicitement que la fixation "allostérique" observée en MD s'explique par la grille V1 ; la pose de départ MD est probablement structurellement fausse |
| C6 | RRS (docking mutants WT/mutant, C59R/S108N PfDHFR, "7G6" PfCRT — nomenclature à vérifier, §3.3) | `mutant_docking_results.csv`, 102 lignes, non analysé | PfDHFR + PfCRT | 🔴 **Critique** — le RRS dérive directement du ΔG de docking ; vérifier la version de grille utilisée pour le docking mutant avant le Job B2 (§4.1), et confirmer l'identité réelle du mutant PfCRT ("7G6" ne correspond à aucun isoforme publié trouvé) |
| C7 | Corrélation croisée PNS/ACSI/RRS/dG_WT | 10 juil. | Hérite de C6 | 🔴 Hérite de C6 |
| C8 | Calibration Tartarus (19 913 × 3 cibles : 1SYH/PfDHFR, 6Y2F/PfCRT homologue, 4LDE/PfClpP homologue) | §2.8, 8 juil. | PfDHFR + PfCRT (homologue) | 🟡 Pipeline externe (QuickVina, grille propre au benchmark Tartarus) — à confirmer qu'il n'hérite pas de `config.txt` V1. Cibles 6Y2F/4LDE désignées "homologues" dans le brouillon P3 — vérifier qu'il s'agit bien de structures PDB distinctes de 6UKJ/4GM2 et non d'une confusion de nommage. PfATP4 hors périmètre Tartarus (absent des 3 cibles) |
...
| # | Analyse | Source BMAD | Hérite de | Statut |
|---|---------|-------------|-----------|--------|
| D1 | Régression TNE sur scores Tartarus (§3.8.1) | 8 juil. | C8 | 🟡 Hérite du statut de C8 |
| D2 | QKS polypharmacologie sur Tartarus (§3.8.2) | — | C8 | 🟡 Hérite de C8 |
| D3 | TDA H0 persistence vs promiscuité de liaison (§3.8.3) | — | C8 (labels ΔG ≤ −7.0 kcal/mol) | 🟡 Hérite de C8 |
| D4 | H1 persistence vs RRS inter-papiers (§3.9, ρ=0.916) | 15 juil. | C6/C7 (classes RRS A–D) | 🔴 Hérite de C6 — si le RRS change après re-docking mutant, cette corrélation phare doit être recalculée, indépendamment du problème de taille d'échantillon déjà traité en §5.5 |
| D5 | Panel 1 815 mols relance TDA/QKS (§5.5) | — | A7 | ✅ Confirmé V2 |
...
### 3.2. Cas du Méthotrexate (MTX) — Écart de Pose Accepté, Pas de Correctif Algorithmique
...
*   **Décision retenue (BMAD F2, "no new simulations") :** ne pas chercher à forcer une RMSD < 2 Å par un docking multi-conformationnel supplémentaire — **réinsérer le MTX avec sa RMSD réelle (~30 Å)** dans le Tableau S30 et le reformuler comme justification du recours au consensus DiffDock plutôt que comme un échec à corriger.
*   **Remède :** édition du Supplementary Material (Table S30) — pas de nouveau calcul.
*   **Fichier Cible :** `manuscript/Antimalarial_Candidates_African_NP_V2607.tex` (Table S30).
*   **Note :** une version antérieure de ce document proposait un docking multi-conformationnel restreint (50 conformères, torsions gelées, script `dock_flexible_ligand.py`) pour ramener la RMSD sous 2 Å. Cette approche est **abandonnée** au profit de la décision BMAD ci-dessus, plus honnête scientifiquement (la limitation de Vina sur les gros ligands flexibles est documentée, pas contournée).
...
**Action recommandée (écriture seule, non bloquante pour le Q1) :** ajouter un paragraphe de Discussion/Limitations dans les manuscrits P1/P2 reconnaissant (i) le caractère inféré (non observé) du site de liaison PfATP4, et (ii) le choix de l'isoforme 7G8 pour PfCRT comme template disponible plutôt que comme haplotype nécessairement le plus pertinent pour le contexte africain. Un nouveau docking sur un modèle Dd2/africain par homologie est une piste de travail future, pas un prérequis Q1.
...
|--------|---------------|---------------------|----------------|
| P1 | [config.txt](${MALARIA_ROOT}/Project1_Chem_space_antimalarialV2607/Docking/Docking_7F3Y/config.txt) | Docking de référence | Aligner coordonnées de centrage V2. |
| P1 | [r8b_fullcluster_rescoring.py](${MALARIA_ROOT}/Project1_Chem_space_antimalarial_V2_CorrectedGrid/scripts/r8b/r8b_fullcluster_rescoring.py) | Docking local (1815 mols) | Aligner le dictionnaire `TARGETS` V2. |
