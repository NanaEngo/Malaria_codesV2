...
|---|---------|-------------|----------|--------|
| C1 | Sélection des 20 candidats (docking consensus 4 cibles) | §2.1 | 4 cibles incl. PfDHFR | ✅ Composition top-20 inchangée (classement par `weighted_mpo_score` physico-chimique). ⚠️ **MPO par cible (MPO_7f3y etc.) issus de V1** — le fichier source `ANP_MPO_Ranked_Final_refined.csv` date du 5 juillet, soit 11 jours avant le diagnostic de grille V2 (16 juillet). Aucune version V2 n'existe. Les scores MPO par cible rapportés dans le manuscrit peuvent être basés sur des affinités V1 erronées. **Action recommandée :** recalculer les MPO par cible avec les scores V2 corrigés avant publication (impact rédactionnel, pas de changement de classement). |
