...
| v3-h | Aug 8, 2026 | **Fusion topologique random COMPLÈTE (GIN-TFP / GIN-TNE)** | random | **GIN-TFP random = 0.9084 ± 0.0060, GIN-TNE random = 0.8918 ± 0.0060 (25 fold×seed, jobs 12841/12842)** — complète les barres random manquantes de la figure benchmark. Tous les bras random sont significativement sous ECFP4-RF 0.9433 (paired t sur 5 per-seed means, df=4, BH-FDR) : GIN-TFP Δ=−0.035 (t(4)=−60.7, p<0.0001), GIN-TNE Δ=−0.052 (t(4)=−60.3, p<0.0001), GIN Δ=−0.034 (t(4)=−27.9, p<0.0001), ChemBERTa Δ=−0.031 (t(4)=−65.9, p<0.0001) — tous passent BH-FDR (adj p < 0.0001). **Hiérarchie random finale : ECFP4-RF 0.9433 > ChemBERTa 0.9121 > GIN 0.9098 > GIN-TFP 0.9084 > GIN-TNE 0.8918.** Honest-negative verrouillé sur les 2 splits : chaque arm GNN/transformer est significativement pire que les fingerprints. Learning curves random GIN-TFP/GIN-TNE capturées (clé `curve` aux ckpts) → figure F4 régénérée avec les 2 splits complets. | `p5_{GIN-TFP,GIN-TNE}_random_results.csv` + ckpts (25/25, jobs 12841/12842) |
| v3-i | Aug 8, 2026 | **Actions validation indépendante (2/3 lancées)** | random+scaffold | **(3) Redérivation statistique indépendante ✅ `p5_replicate_stats.py`** : recompute paired-t (df=4) + BH-FDR depuis les per-seed means des CSVs commités → **reproduit EXACTEMENT les valeurs du manuscrit** (random : tous p<0.0001, BH-FDR adj p<0.0001 ; scaffold : GIN 0.018→adj 0.0330, GIN-TFP 0.025→adj 0.0330, GIN-TNE 0.038→adj 0.0378, ChemBERTa p<0.0001→adj 0.0002). Sorties : `results/p5_replication_stats.csv/.json`. **(2a) Réplication computationnelle ECFP4-RF ✅ (job 12843)** : re-run ECFP4-RF complet sur splits figés (RDKit Morgan r=2 2048 bits, RF 500 arbres, 25 fold×seed) → **PASS parfait, déviation 0.0000 sur les 2 splits** (random 0.9433=0.9433, scaffold 0.8300=0.8300, tolérance 0.005) → déterminisme du baseline verrouillé (`results/p5_replication_verification.json`). **(2b) Réplication GIN ✅ (jobs 12844+12845, GPU)** : re-run GIN random+scaffold via `--tag _replic` (sorties séparées, canoniques intactes) → **PASS sur les 2 splits** (random 0.9097 vs 0.9098, Δ=−0.0001 ; scaffold 0.8015 vs 0.8047, Δ=−0.0032, ρ=0.70). Le 1er run scaffold a crashé sur un import deepchem (jax/numpy incompatibles dans l'env) → **guard ajouté dans `p5_data.py`** (deepchem optionnel, toute exception capturée — ECFP4 reste via RDKit pur) puis re-run propre. Critère de verdict = Δmean ≤ 0.01 + ρ ≥ 0.7 quand la variance inter-seed est résolvable (le max per-seed est informationnel : non-déterminisme GPU documenté — random inter-seed var 5e-6 < seuil donc ρ non informatif, scaffold seed-1 Δ=0.0176 dans le bruit GPU). `p5_GIN_replication_verification.json` : **PASS**. **(4) Benchmark public antimalarial 🔄 (dataset construit, benchmark en cours)** : dataset S3 MoleculeNet privatisé (403), TDC 1.1.15 sans malaria, miroirs HF/figshare/Zenodo/chemprop/OGB sans le fichier → **dataset public construit depuis ChEMBL** (`p5_public_malaria_chembl.py`, cible CHEMBL364 P. falciparum, IC50/EC50 pChEMBL, binarisation actif ≥6.0 / inactif <5.0, dédup canonique) — build job 12846 : **22 447 molécules (19 321 actives / 3 126 inactives), 31 649 composés canoniques uniques, 64 279 activités brutes** ; **overlap panel P5 vérifié = 180 molécules (0.9 % du P5)** → **dataset disjoint `p5_public_chembl_malaria_disjoint.csv` (22 267) excluant tout chevauchement** pour un vrai test de généralisation externe (les labels P1/P3/P5 sont des prédictions ML Ersilia, PAS les IC50/EC50 ChEMBL — vérifié sur les fichiers sources). | `p5_replicate_stats.py`, `p5_replication_verification.py`, `p5_replication.sbatch`, `p5_gin_replication.sbatch`, `p5_gin_scaffold_replication.sbatch`, `p5_verify_gin_replication.py`, `p5_public_benchmark.py`, `p5_public_malaria_chembl.py`, `p5_public_chembl_build.sbatch` |
| v3-j | Aug 8, 2026 | **Benchmark public ChEMBL COMPLET (job 12848, 2 splits)** | random+scaffold | **Dataset disjoint (22 267 mol., excl. 180 overlap panel P5 ; labels IC50/EC50 expérimentaux ChEMBL, cible CHEMBL364, 86 % actifs — déséquilibre identique pour les 2 bras). VERDICT : ECFP4-RF bat GIN sur les DEUX splits.** Random : ECFP4-RF 0.9547 ± 0.0033 vs GIN 0.9237 ± 0.0040 → Δ=+0.031, paired-t p=6e-05 (per-seed ECFP4 [0.9509,0.9589,0.9512,0.9579,0.9544] ; GIN [0.9265,0.9278,0.9184,0.9265,0.9193]). Scaffold : ECFP4-RF 0.9190 ± 0.0004 vs GIN 0.8843 ± 0.0021 → Δ=+0.0346, p<0.0001 (per-seed ECFP4 [0.9191,0.9186,0.9195,0.9185,0.9192] ; GIN [0.8859,0.8839,0.8834,0.8813,0.8872]). **Le verdict « les fingerprints classiques gagnent » se REPRODUIT sur le dataset expérimental externe indépendant** → la conclusion P5 n'est PAS spécifique aux labels Ersilia computationnels du panel : c'est une preuve de généralisation externe (action 4 du plan validation indépendante) — le manuscrit P5 peut citer cette validation dans Discussion/Limitations. ⚠️ Note : le runner n'écrivait que le dernier split dans le JSON ; les valeurs random ont été récupérées du log job 12848 et consolidées. | `results/p5_public_malaria_report.json` (2 splits) ; dataset : `results/p5_public_chembl_malaria.csv` + `_disjoint.csv` + `p5_public_chembl_malaria_provenance.json` |
