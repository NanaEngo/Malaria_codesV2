...
\subsection{Polypharma high-RRS subset calibration and AUC (available-target)}
\label{sec:polypharma_si}

We further stratified the post-hoc calibration and ROC-AUC analyses by the resistance-relevant RRS classification from the P2 Set-C docking study (docking-RRS $\ge 80\%$ on available targets). Using the P2 docking-RRS classification\citep{temgoua2027md}, we identified 6 candidates with A*/A RRS class on available targets (PP-01, PP-02, PP-06, PP-11, PP-13, PP-15), corresponding to \num{30} test samples per configuration (out of \num{99180} pooled). This available-target definition includes candidates that may lack scores for one target but retain high RRS on the other, in contrast to the complete two-target definition (n=2 candidates: PP-01, PP-15). Calibration by RRS class (complete and available) is provided in the released data package.

\begin{table}[h]
\centering
\caption{Available-target polypharma subset ROC-AUC on the scaffold split (secondary analysis; n=6 candidates A*/A available RRS class, 30 test samples/config vs 99,150 single-target). $\Delta$AUC = polypharma $-$ single-target. Low polypharma sample size precludes bootstrap CI95; values are descriptive.}
...
On the scaffold split, the available-target polypharma AUC is consistently below the single-target AUC for all models ($\Delta$AUC $= -0.173$ to $-0.668$). ChemBERTa and ECFP4--RF show the smallest deficit (-0.173), while GIN, GIN-TFP, and GIN-TNE show larger deficits (-0.52 to -0.67). The complete two-target polypharma subset (n=2 candidates) shows wider variance (AUC 0.00–0.84) and larger deficits for most models. These secondary analyses are descriptive; the honest-negative scaffold result remains unchanged.
...
The main text reports the paired model-minus-reference differences with their permutation-based intervals. For completeness, \Cref{tab:s4_paired_ci} archives the audited paired seed-level contrasts with 95\% intervals derived from the paired $t$ statistics (\num{5} per-seed means per arm). All eight contrasts are negative and exclude zero at the 95\% level after Benjamini--Hochberg correction\citep{benjamini1995}.
