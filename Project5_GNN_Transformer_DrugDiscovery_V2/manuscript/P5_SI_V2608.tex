...
The main text summarises a phenotype-only benchmark on the public LISH mechanism-of-action (MoA) release; the protocol and results are reported in full here. The benchmark is pharmacological rather than chemical: it tests how well gene-expression and viability readouts predict a drug's MoA. It does not address whether the molecular representations examined in the main analysis capture that pharmacology, because no structural information links the two data sets.
...
\begin{table}[h]
\centering
\caption{Post-hoc calibration metrics, pooled across \num{25} fold--seed evaluations per configuration. Estimates describe the deployed-checkpoint family, not any single fold, and should be read as trend diagnostics rather than deployment-grade calibration. ECE and MCE rise from the random split to the scaffold and novel-partition splits for every model family, indicating that confidence degrades under scaffold shift even when ranking (ROC-AUC) is only moderately affected.}
...
On the scaffold split, ROC-AUC on the available-target polypharmacology subset falls below the single-target value for every model, with deficits ranging from $-\num{0.173}$ to $-\num{0.668}$. ChemBERTa and ECFP4--RF hold up best (a deficit of $-\num{0.173}$ each), while GIN, GIN-TFP and GIN-TNE lose considerably more ($-\num{0.52}$ to $-\num{0.67}$). The stricter complete two-target subset, built from only two candidates ($n=2$, ten test records per arm), does not support meaningful AUC estimates; values from that subset are reported only as technical outputs and are not interpreted as evidence. The available-target analysis above is the only subset with interpretable ROC-AUC. These numbers are descriptive only: the small candidate count prevents any inferential claim, and the honest-negative scaffold result that anchors the main analysis stands unchanged.
...
The scaffold contrast could mask a regime in which the ranking of arms reverses. To test this, each test molecule was paired with its highest-Tanimoto training neighbour (ECFP4, radius \num{2}, \num{2048} bits) and the test set was binned by decile of that nearest-neighbour similarity. Per-decile mean ROC-AUC (averaged across five seeds and five folds) was recomputed from the stored per-fold prediction files for each arm on the canonical scaffold split (\num{25} fold--seed records per decile--arm cell). Decile \numrange{1}{10} span NN-Tanimoto means of $\num{0.306} \to \num{0.631}$ (D\num{1}: $\num{0.306}$, D\num{10}: $\num{0.631}$); the D\num{5}--D\num{8} bins correspond to a familiar chemical neighbourhood and the D\num{1}--D\num{4} bins to out-of-distribution extrapolation. D\num{9} and D\num{10} carry the highest similarity and are examined separately below.
