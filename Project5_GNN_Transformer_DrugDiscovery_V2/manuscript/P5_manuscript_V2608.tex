...
The molecular benchmark was the primary analysis. We also evaluated a phenotype-only baseline on the public LISH mechanism-of-action (MoA) training release. This task uses gene-expression, viability, dose, and time features rather than molecular structures; after drug-level aggregation, it comprised \num{3289} drugs and \num{206} scored MoA labels. Because no versioned drug--SMILES mapping was available, LISH cannot test transfer of ECFP4, GNN, or transformer representations. An unweighted per-label logistic baseline was assessed with five seeds and five drug-grouped folds, using mean column-wise log loss as the primary metric and macro-AUPRC, macro-AUROC, Brier score, and expected calibration error as secondary metrics.

With vehicle controls retained, the phenotype-only baseline achieved mean column-wise log loss \num{0.02378}, macro-AUROC \num{0.6435}, macro-AUPRC \num{0.1428}, mean Brier score \num{0.00374}, and expected calibration error \num{0.00376} across \num{25} fold--seed evaluations. Removing vehicle controls changed macro-AUROC by only $-\num{0.0018}$. These metrics summarize the phenotype-only reference; they are not numerically comparable with the molecular ROC-AUC results because the task, labels, features, aggregation unit, and primary metric differ. The labels indicate MoA-associated prediction rather than causal target engagement \citep{Trapotsi2022MoA}.
