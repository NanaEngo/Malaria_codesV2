...
> ⚠️ **Dépendance Set-C (10/08) : CGenFF INDISPONIBLE — workaround OpenFF documenté.** `cgenff`/`cgenff_charmm2gmx` est un binaire ParamChem **licencié**, non installable via pip/conda (vérifié 09/08). **16 systèmes Set-C préparés avec OpenFF 2.2.0 (AM1-BCC via AmberTools) + protéine CHARMM36m**, déviation de politique `DOCUMENTED_DEVIATION_OPENFF_2_2_AM1BCC` déclarée **et approuvée PI** dans chaque `forcefield_manifest.json` (`policy_deviation.declared=true`) et dans `results/md_systems/set_c/preparation_manifest.json` (cohort `P2_SET_C_POLYPHARM_17`, 16/16 préparés). Smoke validé de bout en bout sur PP-01_PfDHFR_WT (EM restreint + libre + NVT + NPT rc=0, EM final −5.21e6 kJ/mol, NPT 310.15 K/1 bar) ; **équilibration array 15054 en cours**. `p2_setc_md_workflow.py::validate_forcefield` n'accepte cette déviation que si `policy_deviation.approved=true` (fail-closed sinon). Doc : `results/set_c_md/set_c_preparation_remediation_20260809.md`.
...
| 15054 (array 0-15) | `p2_setc_equilibrate` | 🔄 **En cours (10/08)** | EM restreint + libre + NVT + NPT par système (CPU-only, 310.15 K/1 bar) ; smoke validé rc=0 ; 4 tâches actives au moment du commit |
