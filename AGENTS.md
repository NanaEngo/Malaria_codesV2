...
| Set-C MD (pilot RRS/polypharm, 16 systèmes PP-01/PP-02 × PfDHFR/PfCRT WT+mutants) | 🔄 **16/16 préparés ; équilibration job 15106 en cours (16 systèmes, coordonnées régénérées propres)** | Workaround OpenFF 2.2.0 (AM1-BCC) à la place de CGenFF (binaire licencié indisponible — **déviation DOCUMENTÉE + approuvée PI** dans `forcefield_manifest.json::policy_deviation`). ⚠️ **Cause racine finale (10/08)** : **désalignement ordre `[molecules]` vs coordonnées** → genion corrompait les structures (anneaux effondrés, chaînes explosées) → segfault pair-search. **Fix : alignement d'ordre + régénération** → job 15106, wave-0 propre, EM doux `emstep=0.0001` conservé |
...
| 15106 (array 0-15 `%4`) | `p2_setc_equilibrate_final` | 🔄 **En cours (10/08)** | **Structures régénérées propres (0 anneau effondré, 16/16 vérifiés)** + ordre `[molecules]` corrigé + EM doux `emstep=0.0001` ; wave-0 (K76A/K76T/WT/C59R) à em 979-1585 pas, 0 failure json |
