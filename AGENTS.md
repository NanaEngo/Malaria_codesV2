...
| Set-C MD (pilot RRS/polypharm, 16 systèmes PP-01/PP-02 × PfDHFR/PfCRT WT+mutants) | 🔄 **16/16 préparés ; équilibration job 15097 en cours (16 systèmes, EM doux sans POSRES)** | Workaround OpenFF 2.2.0 (AM1-BCC) à la place de CGenFF (binaire licencié indisponible — **déviation DOCUMENTÉE + approuvée PI** dans `forcefield_manifest.json::policy_deviation`). ⚠️ **Cause racine finale (10/08)** : le POSRES figeait la protéine → clashs massifs (Pot step-0 ~9e8) non relaxables → eau en NaN → segfault pair-search (non-déterministe, pas un problème de chimie ni de ressources). **Fix : EM doux sans POSRES (`emstep=0.0001`)** → job 15097, 4 premières tâches au-delà de la zone de crash |
...
| 15054/15070/15081/15090 (arrays) | `p2_setc_equilibrate(_retry/_final)` | ❌ **Crashs successifs (10/08)** | Cause racine finale : POSRES figeant la protéine → eaux en NaN → segfault pair-search (non-déterministe). Arrays interrompus et remplacés par 15097 |
| 15097 (array 0-15 `%4`) | `p2_setc_equilibrate_final` | 🔄 **En cours (10/08)** | **Protocole EM doux sans POSRES (`emstep=0.0001`)** + `-ntomp 8` ; 4 premières tâches à em 3328-4742 pas (zone de crash 40-250 dépassée), 0 failure json |
