...
\begin{table}[t]
\centering
\caption{Mean test ROC AUC $\pm$ std across \num{25} fold--seed replicates, random and scaffold splits. Paired $t$-tests (\num{5} per-seed means) vs.\ ECFP4--RF under scaffold.}
\label{tab:h1}
\begin{tabular}{lccc}
\toprule
...
\cref{tab:h1} reports mean test ROC AUC across \num{25} fold--seed replicates for each model, under both splits. Under the scaffold split, the fingerprint baseline reaches \num{0.8300}. Every graph-based and transformer arm is significantly \emph{worse}: the best GNN arm, GIN--TFP, reaches \num{0.8138} ($\Delta = -\num{0.016}$, paired $t$, $p = \num{0.025}$); GIN--TNE reaches \num{0.8090} ($\Delta = -\num{0.021}$, $p = \num{0.038}$); plain GIN reaches \num{0.8047} ($\Delta = -\num{0.025}$, $p = \num{0.018}$); ChemBERTa reaches \num{0.7867} ($\Delta = -\num{0.043}$, $p < \num{0.0001}$). All four comparisons survive Benjamini--Hochberg correction (adjusted $p \leq \num{0.038}$). Under the random split, GIN reaches \num{0.9098}, still \num{0.034} below ECFP4--RF (\num{0.9433}). \emph{No graph-based arm outperforms the fingerprint baseline under either split.}
...
\caption{Mean test ROC AUC (bars) with \qty{95}{\percent} confidence intervals, random (left) and scaffold (right) splits, \num{25} fold--seed replicates per model. The ECFP4--RF baseline (red) is not beaten by any GNN or transformer arm. CIs for the baseline are computed from the \num{5} per-seed means; for models, across all \num{25} fold--seed AUCs.}
\label{fig:benchmark}
\end{figure}
...
Per model, \num{25} fold--seed AUCs. Model-vs-baseline comparisons use paired $t$-tests on the \num{5} per-seed means (each a mean over the \num{5} folds); $p$-values are Benjamini--Hochberg--corrected across the \num{4} scaffold comparisons. Significance threshold $\alpha = \num{0.05}$; effect sizes reported as mean difference. Salience is aggregated across the \num{25} replicates as the per-dimension mean absolute projection weight.
